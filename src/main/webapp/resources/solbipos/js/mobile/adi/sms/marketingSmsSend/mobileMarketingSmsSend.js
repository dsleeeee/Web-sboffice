/****************************************************************
 *
 * 파일명 : mobileMarketingSmsSend.js
 * 설  명 : (모바일) 부가서비스 > 마케팅용 SMS전송 JavaScript
 *
...
function marketingSmsSendMsgShow(title, message) {
    ...
    var scope = agrid.getScope('mobileMarketingSmsSendCtrl');
    var params = {};
    params.title = title;
 ...
 */
app.controller('mobileMarketingSmsSendCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileMarketingSmsSendCtrl', $scope, $http, false));
    ...
    $scope.memberSaleList = [
        {value: "0", name: messages["mobile.marketingSmsSend.saveSale"]},
        {value: "1", name: messages["mobile.marketingSmsSend.saveSaleAmount"]}
    ];
    $scope.memberPointList = [
        {value: "0", name: messages["mobile.marketingSmsSend.membrPoint.add"] + messages["mobile.marketingSmsSend.membrPoint"]},
        {value: "1", name: messages["mobile.marketingSmsSend.membrPoint.use"] + messages["mobile.marketingSmsSend.membrPoint"]},
        {value: "2", name: messages["mobile.marketingSmsSend.membrPoint.adj"] + messages["mobile.marketingSmsSend.membrPoint"]},
        {value: "3", name: messages["mobile.marketingSmsSend.membrPoint.ava"] + messages["mobile.marketingSmsSend.membrPoint"]}
    ];
    ...
    // <-- 검색 호출 -->
    $scope.$on("mobileMarketingSmsSendCtrl", function(event, data) {
    ...
    //     $scope._postJSONQuery.withOutPopUp('/adi/sms/marketingSmsSend/marketingSmsSend/getVerifyChk.sb', params, function (response) {
    //         if (response.data.data.list !== 0) {
    //             $scope._popMsg(messages["mobile.marketingSmsSend.verifyChk"]);
    //             return false;
    //
    //         } else {
    ...
    $scope.tellNumChk = function() {
        ...
        $scope._postJSONQuery.withOutPopUp('/adi/sms/smsSend/smsSend/getSmsTelNoComboList.sb', params, function (response) {
            if (response.data.data.list.length > 0) {
                ...
                $scope._popMsg(messages["mobile.marketingSmsSend.telNoVerifyAlert"]); // 발신번호은 웹화면에서 진행하여 주십시오.
                return;

                // 등록된 발신번호가 없습니다. <br/> 문자메세지 발송을 위해서는 <br/> 발신번호를 사전 등록하셔야 합니다. <br/> 등록을 진행하시겠습니까?
                // if (confirm(messages["mobile.marketingSmsSend.telNoConfirm"])) {
                //     // 발신번호 사전등록 팝업
    ...
    //
    //             if ($scope.telNoCombo == "") {
    //                 $scope._popMsg(messages["mobile.marketingSmsSend.telNoAlert"]); // 사전등록된 발신번호가 없습니다. <br/> [발신번호 추가] 버튼으로 발신번호 사전등록하여 주십시오.
    //                 return;
    //             }
    //
    //             if ($("#marketingSmsSendMessageContent").val() == "") {
    //                 $scope._popMsg(messages["mobile.marketingSmsSend.messageContentAlert"]); // 메세지를 입력해주세요.
    //                 return false;
    //             }
    //
    ...
    //                     $scope._popMsg(messages["mobile.marketingSmsSend.txtByteOverAlert"]); // 2000 바이트 이상 전송이 불가합니다.
    //                     return;
    //                 }
    ...
    //                 $scope._popMsg(messages["mobile.marketingSmsSend.smsAmtAlert"]); // 전송가능한 금액이 없습니다.
    //                 return;
    //             }
    //
    //             // 조회한 회원수
    ...
    //                 // 조회한 회원수
    //                 smsSendListCnt = $("#lblMarketingSmsSendListCnt").text();
    //
    //                 if (parseInt(smsAmt) < (parseInt(smsSendListCnt) * parseInt(msgOneAmt))) {
    //                     $scope._popMsg(messages["mobile.marketingSmsSend.smsAmtOverAlert"] + (parseInt(smsSendListCnt) * parseInt(msgOneAmt)) + messages["mobile.marketingSmsSend.smsAmtOverAlert2"]); // 전송시 필요한 잔여금액이 부족합니다. 000원의 잔여금액이 필요합니다.
    //                     return;
    //                 }
    //
    //             } else {
    ...
    //                             $scope._popMsg(messages["mobile.marketingSmsSend.telNoBlank"]); // 수신번호를 입력해주세요.
    //                             return false;
    //                         }
    //                     }
    //                 }
    ...
    //                 param.smsSendListCnt = smsSendListCnt;
    //
    //                 $scope.setSelectedMarketingSmsSend(param);
    //                 $scope.wjMobileSmsReserveLayer.show(true);
    //                 event.preventDefault();
    //             } else {
    //                 // 전송 저장
        ...
        $scope._postJSONQuery.withOutPopUp('/adi/sms/marketingSmsSend/marketingSmsSend/getVerifyChk.sb', params, function (response) {
            if (response.data.data.list === 0) {
                ...
                if ($scope.telNoCombo == "") {
                    ...
                    $scope.smsSendReserve2(reserveYn);
                }
            }
        });
    };
    // 발신번호 공통코드에 등록된지 확인
    $scope.smsSendReserve2 = function(reserveYn) {
        var params = {};
        params.telNo = $scope.telNoCombo;

        $scope._postJSONQuery.withOutPopUp('/adi/sms/marketingSmsSend/marketingSmsSend/getTelNoNmCodeChk.sb', params, function (response) {
            var telNoNmCodeChk = response.data.data.result;
            $scope.telNoNmCodeChk = telNoNmCodeChk;

            if($scope.telNoNmCodeChk.telNoCount != "0") {
                $scope.smsSendReserve3(reserveYn);
            } else {
                $scope._popMsg(messages["marketingSmsSend.telNoNmCodeChk"]); // 발신번호 최종승인 대기중 입니다.(최대 3일 소요됩니다.) <br/> 승인 지연될 경우 관리자에게 문의하십시오.(1644-5195)
                return false;
            }
        });
    };
    $scope.smsSendReserve3 = function(reserveYn) {
        if ($("#marketingSmsSendMessageContent").val() == "") {
            $scope._popMsg(messages["mobile.marketingSmsSend.messageContentAlert"]); // 메세지를 입력해주세요.
            return false;
        }
    ...
    // 전송 저장
    $scope.smsSendSave = function(reserveYn, reserveDate, msgType, msgOneAmt, smsSendListCnt) {
        // 첨부파일 개수
        var fileCount = 0;
        // MMS 첨부파일 체크
        if(msgType == "3") {
            ...
            if (!isNull($("#marketingSmsSendFileSms1")[0].files[0])) {
                ...
                if (fileSize > maxSize) {
                    $scope._popMsg(messages["mobile.marketingSmsSend.fileSizeChk.300.msg"]); // 첨부파일은 300KB 이내로 등록 가능합니다.
                    return;
                }
                // 파일명 형식 체크
                ...
                if(1 > imgFullNm.lastIndexOf('.')){
                    $scope._popMsg(messages["mobile.marketingSmsSend.fileNmChk.msg"]); // 파일명 또는 확장자가 올바르지 않습니다. 다시 확인해주세요.
                    return;
                }
                // 확장자 체크
                var reg = /(.*?)\.(jpg|JPG)$/;
                if(! $("#marketingSmsSendFileSms1").val().match(reg)) {
                    $scope._popMsg(messages["mobile.marketingSmsSend.fileExtensionChk.msg"]); // 확장자가 .jpg .JPG 인 파일만 등록가능합니다.
                    return;
                }
        ...
        var msg = $("#lblMarketingSmsSendMsgType").text() + messages["mobile.marketingSmsSend.smsSendConfirm"]  + " " + smsSendListCnt + messages["mobile.marketingSmsSend.smsSendConfirm2"];
        if (confirm(msg)) {
            // 전송가능 시간 체크(09~21시)
            var date = new Date();
            var hh = new String(date.getHours());
            if(parseInt(hh) < 9 || parseInt(hh) > 21) {
                $scope._popMsg(messages["mobile.marketingSmsSend.smsSendTimeAlert"]); // 전송가능한 시간대가 아닙니다. 09~21시만 전송가능합니다.
                return;
            }
            ...
            if(msgType == "3") {
                // 첨부파일 저장
                $scope.smsSendFileSave(reserveYn, reserveDate, msgType, msgOneAmt, smsSendListCnt, fileCount);
                // SMS, LMS
            } else {
                // 전송 저장 save
                $scope.smsSendRealSave(reserveYn, reserveDate, msgType, msgOneAmt, smsSendListCnt, 0, "");
            }
        }
    };

    // 전송 저장 save
    $scope.smsSendRealSave = function(reserveYn, reserveDate, msgType, msgOneAmt, smsSendListCnt, fileCount, contentData) {
        // 1000건 이상
        if(parseInt(smsSendListCnt) >= 1000) {
            // 파라미터 설정
            var params = {};
            ...
            params.smsSendListCnt = smsSendListCnt; // 조회한 회원수
            ...
            for (var i = 0; i < $scope.flexMarketingSmsSend.collectionView.items.length; i++) {
                if($scope.flexMarketingSmsSend.collectionView.items[i].gChk) {
                    ...
                    $scope.flexMarketingSmsSend.collectionView.items[i].smsSendListCnt = smsSendListCnt; // 조회한 회원수
    ...
    // 첨부파일 저장
    $scope.smsSendFileSave = function(reserveYn, reserveDate, msgType, msgOneAmt, smsSendListCnt, fileCount) {
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (result.status === "OK") {
                    ...
                    // 전송 저장 save
                    $scope.smsSendRealSave(reserveYn, reserveDate, msgType, msgOneAmt, smsSendListCnt, fileCount, contentData);
                }
                else if (result.status === "FAIL") {
                    $scope._popMsg('Ajax Fail By HTTP Request');
                    $scope.$broadcast('loadingPopupInactive');
    ...
    angular.element(document).ready(function () {
        ...
        // SMS예약 팝업 핸들러 추가
        $scope.wjMobileSmsReserveLayer.shown.addHandler(function (s) {
            setTimeout(function() {
                $scope._broadcast('mobileSmsReserveCtrl', $scope.getSelectedMarketingSmsSend());
            }, 50)
        });
    });
 ...
 *  팝업 조회 그리드 생성
 */
app.controller('mobileMarketingSmsSendPopupCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileMarketingSmsSendPopupCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("mobileMarketingSmsSendPopupCtrl", function(event, data) {
        // 닫았다 다시 호출하면 안떠서
        // 날짜 비교하여 팝업 띄우기
