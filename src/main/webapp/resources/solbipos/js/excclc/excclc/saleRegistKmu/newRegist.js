...
app.controller('selectProdCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.searchRegProd = function(){
        ...
        $scope._inquiryMain("/excclc/excclc/saleRegistKmu/saleRegistKmu/getSelectProdList.sb", params, function() {});
    };
...
app.controller('newRegistCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.memberSelectApply = function () {
        $scope.wjSaleRegistKmuMemberLayer.show(true);
        event.preventDefault();
    };
    ...
    angular.element(document).ready(function () {
        ...
        // 회원선택 팝업 핸들러 추가
        $scope.wjSaleRegistKmuMemberLayer.shown.addHandler(function (s) {
            setTimeout(function() {
                $scope._broadcast('saleRegistKmuMemberCtrl', null);
            }, 50)
        });
    });
