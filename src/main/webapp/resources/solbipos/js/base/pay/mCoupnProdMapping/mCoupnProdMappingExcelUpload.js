/****************************************************************
 *
 * 파일명 : mCoupnProdMappingExcelUpload.js
 * 설  명 : 모바일쿠폰상품매핑 엑셀업로드 팝업 JavaScript
 *
...
/**
 *  모바일쿠폰상품매핑 엑셀업로드 팝업 조회 그리드 생성
 */
app.controller('mCoupnProdMappingExcelUploadCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mCoupnProdMappingExcelUploadCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("mCoupnProdMappingExcelUploadCtrl", function(event, data) {
    ...
    $scope.deleteExl = function () {
        ...
        $scope._postJSONSave.withOutPopUp("/base/pay/mCoupnProdMapping/mCoupnProdMappingExcelUpload/getMCoupnProdMappingExcelUploadCheckDeleteAll.sb", params, function(){
            // 엑셀 업로드
            $scope.excelUpload();
        });
    };
    ...
    $scope.excelUpload = function () {
        ...
        // 선택한 파일이 있으면
        if ($('#mCoupnProdMappingExcelUpFile')[0].files[0]) {
            var file          = $('#mCoupnProdMappingExcelUpFile')[0].files[0];
            ...
            if (fileExtension.toLowerCase() === '.xlsx' || fileExtension.toLowerCase() === '.xlsm') {
                ...
                // $timeout(function () {
                //     var flex = $scope.flex;
                //     wijmo.grid.xlsx.FlexGridXlsxConverter.loadAsync(flex, $('#mCoupnProdMappingExcelUpFile')[0].files[0], {includeColumnHeaders: true}
                //         , function () {
                //             $timeout(function () {
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            ...
                            renameKey(item, '쿠폰사', 'mcoupnNm');
                            renameKey(item, '매핑코드', 'mcoupnProdCd');
    ...
    $scope.save = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/pay/mCoupnProdMapping/mCoupnProdMappingExcelUpload/getMCoupnProdMappingExcelUploadCheckSave.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    ...
                    // 팝업 호출
                    $scope.wjMCoupnProdMappingExcelUploadResultLayer.show(true);
                    event.preventDefault();
                }
            }
    ...
    angular.element(document).ready(function () {
        ...
        // 모바일쿠폰상품매핑 엑셀업로드 결과 팝업 핸들러 추가
        $scope.wjMCoupnProdMappingExcelUploadResultLayer.shown.addHandler(function (s) {
            setTimeout(function() {
                $scope._broadcast('mCoupnProdMappingExcelUploadResultCtrl', null);
            }, 50)
        });
    });
