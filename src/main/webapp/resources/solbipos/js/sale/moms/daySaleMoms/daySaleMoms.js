...
app.controller('daySaleMomsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.searchDaySaleMomsList = function () {
        ...
        $.postJSON("/sale/moms/daySaleMoms/daySaleMoms/getDaySaleMomsList.sb", params, function (response){
            var grid = $scope.flex;
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "일별매출_" + wijmo.Globalize.format(startDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format(endDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
...
app.controller('daySaleMomsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/moms/daySaleMoms/daySaleMoms/getDaySaleMomsExcelList.sb", params, function(response) {
