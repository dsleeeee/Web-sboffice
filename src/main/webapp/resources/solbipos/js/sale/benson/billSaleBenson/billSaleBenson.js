/****************************************************************
 *
 * 파일명 : billSaleBenson.js
 * 설  명 : (벤슨) 간소화화면 > 영수건별매출 JavaScript
 *
 ...
 *  영수건별매출 그리드 생성
 */
app.controller('billSaleBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('billSaleBensonCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("billSaleBensonCtrl", function (event, data) {
        $scope.searchBillSaleBensonList();
        event.preventDefault();
    });

    $scope.searchBillSaleBensonList = function () {
        ...
        params.storeCds = $("#billSaleBensonStoreCd").val();
        params.prodCds = $("#billSaleBensonSelectCd").val();
        ...
        $.postJSON("/sale/benson/billSaleBenson/billSaleBenson/getBillSaleBensonList.sb", params, function(response) {
            var grid = $scope.flex;
    ...
    $scope.billSaleBensonSelectShow = function () {
        $scope._broadcast('billSaleBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function (excelType) {
        ...
        if(params.excelType === '1') {
            ...
            $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
                $scope._broadcast('billSaleBensonExcelCtrl', params);
            });
        }else{
            // 분할 엑셀다운로드 사용자 제한 체크
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('billSaleBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('billSaleBensonExcelCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("billSaleBensonExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/benson/billSaleBenson/billSaleBenson/getBillSaleBensonExcelList.sb", params, function(response) {
                ...
                function delay(x){
                    return new Promise(function(resolve, reject){
                        ...
                        $.postJSON("/sale/benson/prodSaleDayStoreBenson/prodSaleDayStoreBenson/getDivisionExcelDownloadSaveUpdate.sb", params, function(response){
                            ...
                            // ajax 통신 설정
                            $.postJSON("/sale/benson/billSaleBenson/billSaleBenson/getBillSaleBensonExcelDivisionList.sb", params, function(response) {
