...
app.controller('rtnDstbCloseProdDtlCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('rtnDstbCloseProdDtlCtrl', $scope, $http, true));

  $scope.procFgMap = new wijmo.grid.DataMap([
    {id: "00", name: messages["rtnDstbCloseProd.dtl.procFgReg"]},
    {id: "10", name: messages["rtnDstbCloseProd.dtl.procFgMd"]},
    {id: "20", name: messages["rtnDstbCloseProd.dtl.procFgDstbClose"]},
    {id: "30", name: messages["rtnDstbCloseProd.dtl.procFgSlip"]}
  ], 'id', 'name');
  ...
  $scope.$on("rtnDstbCloseProdDtlCtrl", function (event, data) {
    ...
    $scope.searchRtnDstbCloseProdDtlList();
    // 기능수행 종료 : 반드시 추가
  ...
  $scope.save = function () {
    ...
    for (var i = 0; i < $scope.flex.collectionView.itemsEdited.length; i++) {
      ...
      if (item.mgrEtcQty !== null && (parseInt(item.mgrEtcQty) >= parseInt(item.poUnitQty))) {
        $scope._popMsg(messages["rtnDstbCloseProd.dtl.not.mgrEtcQty"]); // 낱개수량은 입수량보다 작아야 합니다.
        return false;
      }
    ...
    $scope._save("/iostock/orderReturn/rtnDstbCloseProd/rtnDstbCloseProdDtl/save.sb", params, function () {
      $scope.saveRtnDstbCloseProdDtlCallback()
    });
  };


  // 저장 후 콜백 함수
  $scope.saveRtnDstbCloseProdDtlCallback = function () {
    var rtnDstbCloseProdScope = agrid.getScope('rtnDstbCloseProdCtrl');
    rtnDstbCloseProdScope.searchRtnDstbCloseProdList();

    $scope.wjRtnDstbCloseProdDtlLayer.hide(true);
  };
