...
app.controller('mcoupnCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("mcoupnCtrl");
  ...
  $scope.searchCashBillList = function () {
    ...
    if( isEmptyObject( $("#mcoupnStoreCd").val()) ) {
    ...
    // 파라미터
    var params       = {};
    params.storeCds   = $("#mcoupnStoreCd").val();
    ...
    $scope._inquiryMain("/sale/appr/mcoupn/mcoupn/getMcoupnList.sb", params);
  };
  ...
  $scope.mcoupnStoreShow = function () {
    $scope._broadcast('mcoupnStoreCtrl');
  };
  ...
  $scope.excelDownload = function () {
    ...
    $scope._broadcast('mcoupnExcelCtrl',params);
  }
}]);

app.controller('mcoupnExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('mcoupnExcelCtrl', $scope, $http, true));
  ...
  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("mcoupnExcelCtrl", function (event, data) {
  ...
  $scope.searchExcelList = function (data) {
    ...
    // 조회 수행 : 조회URL, 파라미터, 콜백함수
    $scope._inquiryMain("/sale/appr/mcoupn/mcoupn/getMcoupnExcelList.sb", params, function() {
      ...
      $timeout(function () {
        ...
        }, messages["mcoupnMoms.mcoupnMoms"]+getToday()+'.xlsx', function () {
          $timeout(function () {
