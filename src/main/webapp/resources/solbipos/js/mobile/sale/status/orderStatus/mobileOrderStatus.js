 ...
 *  주문현황 그리드 생성
 */
app.controller('mobileOrderStatusCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    // 주문현황 엑셀다운로드
    $("#btnExcelMobileOrderStatus").on("click", function(event) {

        if ($scope.flexMobileOrderStatus.rows.length <= 0) {
        ...
        $timeout(function () {
          wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flexMobileOrderStatus, {
            includeColumnHeaders: true,
            includeCellStyles: true,
            includeColumns: function (column) {
              return column.visible;
            }
          },  messages["mobile.orderStatus"] + '_' + getToday() + '.xlsx', function () {
            $timeout(function () {
