/****************************************************************
 *
 * 파일명 : stockAcinsDtl.js
 * 설  명 : 국민대 > 재고관리 > 재고실사 > 실사상세 팝업 JavaScript
 *
...
app.controller('stockAcinsDtlCtrl', ['$scope', '$http', function ($scope, $http) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('stockAcinsDtlCtrl', $scope, $http, true));
    ...
    $scope.$on("stockAcinsDtlCtrl", function (event, data) {
        // 그리드 초기화
        ...
        $http({
            ...
            if ($scope._httpStatusCheck(response, true)) {
                ...
                if (!$.isEmptyObject(response.data.data)) {
                    ...
                    $("#stockAcinsDtlVendrCd").val(response.data.data.acinsVendrCd);
                    $("#stockAcinsDtlVendrNm").val('[' + response.data.data.acinsVendrCd + ']' + response.data.data.vendrNm);
                    $("#stockAcinsDtlStoreCd").val(response.data.data.acinsStoreCd);
                    $("#stockAcinsDtlStoreNm").val('[' + response.data.data.acinsStoreCd + ']' + response.data.data.storeNm);
                    $("#stockAcinsDtlStorebtnCancelStoreCd").attr("disabled", true);
                    $("#stockAcinsDtlStoreNm").attr("disabled", true);
                    $scope.stockAcinsDtlVendrNmDisabled  = true; // 반출 거래처선택 모듈 input
                    $scope.stockAcinsDtlVendrBtnDisabled = true; // 반출 선택취소
//          $scope.adjStorageCd = response.data.data.adjStorageCd;
                }
            }
            ...
            // "complete" code here
            $scope.wjStockAcinsDtlLayer.show(true);
    ...
    // 실사상품 리스트 조회
    $scope.getSearchAcinsDtlList = function () {
        // 파라미터
        var params       = {};
        params.acinsDate = $scope.acinsDate;
        params.seqNo     = $scope.seqNo;
//    params.adjStorageCd     = $scope.adjStorageCd;
        params.adjStorageCd     = $scope.acins.dtl.adjStorageCd;
        params.listScale = 50;

        if(params.seqNo !== null && params.seqNo !== undefined){
            // 조회 수행 : 조회URL, 파라미터, 콜백함수
            $scope._inquiryMain("/kookmin/stock/stockAcins/stockAcinsDtl/getSearchAcinsDtlList.sb", params, function () {
            });
        }
    ...
    $scope.saveAcinsDtl = function (confirmFg) {
        ...
        $scope._save("/kookmin/stock/stockAcins/stockAcinsDtl/saveAcinsDtl.sb", params, function () {
    ...
    $scope.saveRegistCallback = function (confirmFg) {
        ...
        if (confirmFg === 'Y') {
            $scope.wjStockAcinsDtlLayer.hide(true);
        }
    };
    ...
    $scope.addProd = function () {
        ...
        $scope._broadcast('stockAcinsRegistCtrl', params);
    };
