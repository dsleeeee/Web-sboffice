/****************************************************************
 *
 * 파일명 : excelUploadStoreErrInfo.js
 * 설  명 : 공통팝업 실사/조정/폐기 엑셀업로드 에러내역 JavaScript
 *
 ...
 *  상품 입고현황 그리드 생성
 */
app.controller('excelUploadStoreErrInfoCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('excelUploadStoreErrInfoCtrl', $scope, $http, true));

    // 그리드 상품상태
    $scope.prodStatusMap = new wijmo.grid.DataMap([
        {id: "0", name: messages["excelUploadMPSErrInfo.prodStatusNoReg"]}, // 미등록상품
        {id: "1", name: messages["excelUploadMPSErrInfo.prodStatusReg"]}    // 등록상품
    ], 'id', 'name');
    ...
    // <-- 검색 호출 -->
    $scope.$on("excelUploadStoreErrInfoCtrl", function(event, data) {
        // 그리드 초기화
        ...
        $scope.wjExcelUploadStoreErrInfoLayer.show(true);
        $scope.gridVisible();
    ...
    $scope.searchExcelUploadErrInfoList = function () {
        ...
        $scope._inquiryMain("/iostock/cmmExcelUpload/excelUploadStore/excelUploadStoreErrInfo/list.sb", params);
    };
