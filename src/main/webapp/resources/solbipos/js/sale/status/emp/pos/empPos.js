...
app.controller('empPosExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
	// 상위 객체 상속 : T/F 는 picker
	angular.extend(this, new RootController('empPosExcelCtrl', $scope, $http, true));
  ...
  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("empPosExcelCtrl", function (event, data) {
  ...
  // 판매자포스별 리스트 조회
  $scope.searchEmpPosExcelList = function () {
    ...
    // 조회 수행 : 조회URL, 파라미터, 콜백함수
    $scope._inquiryMain("/sale/status/emp/pos/excelList.sb", params, function() {
		...
		$timeout(function () {
			...
			}, messages["day.dayTotal.saleInfo"]+'_'+messages["empsale.empsale"]+'_'+messages["empsale.pos"]+'_'+getToday()+'.xlsx', function () {
				$timeout(function () {
  ...
  $scope.getEmpNmList = function () {
	    ...
	    $scope._postJSONQuery.withOutPopUp("/sale/status/emp/pos/empList.sb", params, function(response) {
		  ...
		  var length = response.data.data.list.length;
		  var grid = wijmo.Control.getControl("#empPosExcelGrid");
