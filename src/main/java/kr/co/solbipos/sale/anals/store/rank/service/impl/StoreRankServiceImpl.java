    ...
    public List<DefaultMap<String>> getStoreRankList(StoreRankVO storeRankVO, SessionInfoVO sessionInfoVO) {
        ...
        if(storeRankVO.getChkPay().equals("Y")) {
            List<DefaultMap<String>> payCd = storeRankMapper.getPayColList(storeRankVO);
            	sQuery1 +=", M.PAY0" + "\n";
            	sQuery2 +=", SUM(TSDP.TOT_PAY_AMT) AS PAY0" + "\n";
            for(int i = 0; i < payCd.size(); i++) {
            	int j = Integer.parseInt(payCd.get(i).get("payCd"));
            	sQuery1 +=", M.PAY" + j + "\n";
            	sQuery2 +=", SUM(CASE WHEN TSDP.PAY_CD = "+ j + " THEN TSDP.TOT_PAY_AMT ELSE NULL END) AS PAY" + j +  "\n";
            }
        }else {
        	sQuery1 +=", M.PAY0" + "\n";
        	sQuery1 +=", M.PAY1" + "\n";
        	...
        	sQuery1 +=", M.PAY3" + "\n";
        	sQuery2 +=", SUM(TSDP.TOT_PAY_AMT) AS PAY0" + "\n";
        	sQuery2 +=", SUM(CASE WHEN TSDP.PAY_CD = '01' THEN TSDP.TOT_PAY_AMT ELSE NULL END) AS PAY1" + "\n";
        	sQuery2 +=", SUM(CASE WHEN TSDP.PAY_CD = '02' THEN TSDP.TOT_PAY_AMT ELSE NULL END) AS PAY2" + "\n";	
        	sQuery2 +=", SUM(CASE WHEN TSDP.PAY_CD != '01' AND TSDP.PAY_CD != '02' THEN TSDP.TOT_PAY_AMT ELSE NULL END) AS PAY3" + "\n";	
