/****************************************************************
 *
 * 파일명 : stockAdjustDtl.js
 * 설  명 : 국민대 > 재고관리 > 재고조정 > 재고조정 상세 팝업 JavaScript
 *
...
app.controller('stockAdjustDtlCtrl', ['$scope', '$http', function ($scope, $http) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('stockAdjustDtlCtrl', $scope, $http, true));
    ...
    $scope.$on("stockAdjustDtlCtrl", function (event, data) {
        // 그리드 초기화
        ...
        $http({
            ...
            if ($scope._httpStatusCheck(response, true)) {
                ...
                if (!$.isEmptyObject(response.data.data)) {
                    ...
                    $("#stockAdjustDtlVendrCd").val(response.data.data.adjVendrCd);
                    $("#stockAdjustDtlVendrNm").val('[' + response.data.data.adjVendrCd + ']' + response.data.data.vendrNm);
                    $("#stockAdjustDtlStoreCd").val(response.data.data.adjStoreCd);
                    $("#stockAdjustDtlStoreNm").val('[' + response.data.data.adjStoreCd + ']' + response.data.data.storeNm);
                    $("#stockAdjustDtlStorebtnCancelStoreCd").attr("disabled", true);
                    $("#stockAdjustDtlStoreNm").attr("disabled", true);
                    $scope.stockAdjustDtlVendrNmDisabled  = true; // 반출 거래처선택 모듈 input
                    $scope.stockAdjustDtlVendrBtnDisabled = true; // 반출 선택취소
                }
            }
            ...
            // "complete" code here
            $scope.wStockjAdjustDtlLayer.show(true);
    ...
    // 조정상품 리스트 조회
    $scope.getSearchAdjustDtlList = function () {
        // 파라미터
        var params       = {};
        params.adjDate   = $scope.adjDate;
        params.seqNo     = $scope.seqNo;
        ...
        $scope._inquiryMain("/kookmin/stock/stockAdjust/stockAdjustDtl/getSearchAdjustDtlList.sb", params);
    };
    ...
    $scope.saveAdjDtl = function (confirmFg) {
        if (nvl($scope.adjTitle, '') === '') {
            var msg = messages["stockAdjust.reg.adjTitle"] + messages["cmm.require.text"];
            $scope._popMsg(msg);
            return false;
        }
        ...
        $scope._save("/kookmin/stock/stockAdjust/stockAdjustDtl/saveAdjustDtl.sb", params, function () {
    ...
    $scope.saveRegistCallback = function (confirmFg) {
        ...
        if (confirmFg === 'Y') {
            $scope.wStockjAdjustDtlLayer.hide(true);
        }
    };
    ...
    $scope.confirm = function () {
        ...
        var msg = messages["stockAdjust.dtl.confirmMsg"]; // 확정하시겠습니까?
        var checkLenth = 0;
    ...
    $scope.addProd = function () {
        ...
        $scope._broadcast('stockAdjustRegistCtrl', params);
    };
