/****************************************************************
 *
 * 파일명 : dlvrExcelUploadAdd.js
 * 설  명 : 배달지엑셀업로드 팝업 JavaScript
 *
...
/**
 *  배달지엑셀업로드 팝업 조회 그리드 생성
 */
app.controller('dlvrExcelUploadAddCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dlvrExcelUploadAddCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("dlvrExcelUploadAddCtrl", function(event, data) {
    ...
    $scope.excelUpload = function () {
        // 선택한 파일이 있으면
        if ($('#dlvrExcelUpFile')[0].files[0]) {
            var file          = $('#dlvrExcelUpFile')[0].files[0];
    ...
    $scope.save = function (jsonData) {
        ...
        // 업로드시 임시테이블 저장
        $scope._postJSONSave.withOutPopUp("/dlvr/manage/info/dlvrExcelUpload/getDlvrExcelUploadCheckSave.sb", jsonData, function () {
            $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
            // 저장기능 수행후 재조회
            var scope = agrid.getScope('dlvrExcelUploadDlvrCtrl');
