/****************************************************************
 *
 * 파일명 : saleAnalysisByTimeReport.js
...
app.controller('saleAnalysisByTimeReportCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('saleAnalysisByTimeReportCtrl', $scope, $http, true));

    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("saleAnalysisByTimeReportCtrl", function (event, data) {
        $scope.data = data;

        $scope.wjSaleAnalysisByTimeReportLayer.show(true);


        // saleAnalysisByTimeReport html 내용 초기화
        $("#saleAnalysisByTimeReport").html('');
    ...
    $scope.getSaleAnalysisByTimeList = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/saleAnalysis/saleAnalysisByTime/saleAnalysisByTime/getSaleAnalysisByTimeList.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (!$.isEmptyObject(response.data.data)) {
                    ...
                    var arrSaleAnalysisListHeaderHtml = '<table class="w100 mt5 small-text-table">' // 시간대별 매출분석리스트 header html
                        + '<colgroup>'
                        + '<col style="width:4%;">'
                        + '<col style="width:4%;">'
                        + '<col style="width:8%;">'
                        ...
                        for (var j = 0; j < payHistList.length; j++) {
                            ...
                            if(item.storeCd !== null && item.storeCd !== '') {
                                arrSaleAnalysisListHtml += '<tr class="h25">'
                                    ...
                                    + '<td class="tc">' + nvl(item.saleDate, '') + '</td>'
                                    + '<td class="tc">' + nvl(item.yoil, '') + '</td>'
                                    + '<td class="tc">' + item.rowFg + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH6) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH7) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH8) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH9) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH10) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH11) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH12) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH13) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH14) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH15) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH16) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH17) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH18) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH19) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH20) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH21) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH22) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH23) + '</td>'
                                    + '<td class="tc">' + addComma(item.totSaleAnals) + '</td>'
                                    + '</tr>';
                            }else{
                                arrSaleAnalysisListHtml += '<tr class="h25">'
                                    + '<td class="tc">' + (++rowNo) + '</td>'
                                    + '<td class="tc" colspan="5">' + item.rowFg + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH6) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH7) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH8) + '</td>'
                                    + '<td class="tc">' + addComma(item.saleAnalsH9) + '</td>'
                    ...
                    $('#saleAnalysisByTimeReport').append(arrSaleAnalysisReportList);
    ...
    $scope.print = function () {
        ...
        // add content to it
        var view = document.querySelector('#saleAnalysisByTimeReport');
        doc.append(view);
