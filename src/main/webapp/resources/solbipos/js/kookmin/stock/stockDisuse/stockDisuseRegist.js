/****************************************************************
 *
 * 파일명 : stockDisuseRegist.js
 * 설  명 : 국민대 > 재고관리 > 재고폐기 > 폐기등록 팝업 JavaScript
 *
...
app.controller('stockDisuseRegistCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('stockDisuseRegistCtrl', $scope, $http, true));
    ...
    $scope.$on("stockDisuseRegistCtrl", function (event, data) {
        ...
        if (!$.isEmptyObject(data)) {
            ...
            $("#stockDisuseRegistVendrCd").val('');
            $("#stockDisuseRegistVendrNm").val('');
            $("#stockDisuseRegistStoreCd").val('');
            $("#stockDisuseRegistStoreNm").val('');
            $("#stockDisuseRegistStorebtnCancelStoreCd").attr("disabled", false);
            $("#stockDisuseRegistStoreNm").attr("disabled", false);
            $scope.stockDisuseRegistVendrNmDisabled  = false; // 반출 거래처선택 모듈 input
            $scope.stockDisuseRegistVendrBtnDisabled = false; // 반출 선택취소
            $scope.getSearchDisuseRegistList();
        }
    ...
    $scope.procFgCheck = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/stock/stockDisuse/stockDisuseRegist/getProcFgCheck.sb', /* 통신할 URL */
            params : params, /* 파라메터로 보낼 데이터 */
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
            if ($scope._httpStatusCheck(response, true)) {
                ...
                if (!$.isEmptyObject(response.data.data)) {
                    ...
                    $scope.disuse.reg.procFg = response.data.data.procFg;
                    ...
                    // 거래처, 매장 값 세팅
                    $scope.disuseVendrCd = response.data.data.disuseVendrCd;
                    $scope.disuseVendrNm = '[' + response.data.data.disuseVendrCd + ']' + response.data.data.vendrNm;
                    $scope.disuseStoreCd = response.data.data.disuseStoreCd;
                    $scope.disuseStoreNm = '[' + response.data.data.disuseStoreCd + ']' + response.data.data.storeNm;
                }
            }
        }, function errorCallback(response) {
    ...
    $scope.layerShow = function () {
        $scope.wjStockDisuseRegistLayer.show(true);
        ...
        if($scope.callParent === "disuse") {
            ...
            $("#stockDisuseRegistVendrCd").val($scope.disuseVendrCd);
            $("#stockDisuseRegistVendrNm").val($scope.disuseVendrNm);
            $("#stockDisuseRegistStoreCd").val($scope.disuseStoreCd);
            $("#stockDisuseRegistStoreNm").val($scope.disuseStoreNm);
            $("#stockDisuseRegistStorebtnCancelStoreCd").attr("disabled", true);
            $("#stockDisuseRegistStoreNm").attr("disabled", true);
            $scope.stockDisuseRegistVendrNmDisabled  = true; // 반출 거래처선택 모듈 input
            $scope.stockDisuseRegistVendrBtnDisabled = true; // 반출 선택취소
        }
    };


    // 폐기상품 리스트 조회
    $scope.getSearchDisuseRegistList = function () {
        // 파라미터
        var params        = {};
        params.disuseDate   = $scope.disuseDate;
        params.seqNo        = $scope.seqNo;
        ...
        // params.vendrCd     = $("#stockDisuseRegistVendrCd").val();
        // params.storeCd     = $("#stockDisuseRegistStoreCd").val();
        params.listScale    = $scope.listScale;
        params.storageCd    = $scope.disuse.reg.disuseStorageCd;

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquirySub("/kookmin/stock/stockDisuse/stockDisuseRegist/getSearchDisuseRegistList.sb", params);
    };
    ...
    $scope.saveDisuseRegist = function () {
        ...
        if($scope.flex.collectionView.items.length > 0) {
            ...
            for (var i = 0; i < $scope.flex.collectionView.itemsAdded.length; i++) {
                ...
                if (item.disuseQty === null || item.disuseQty === '') {
                    item.status = "R";
                } else if (item.gChk === true && item.disuseProdStatus === 'U') {
                    item.status = "D";
                } else {
                    item.status = "U";
                }
                ...
                item.vendrCd = $("#stockDisuseRegistVendrCd").val();
                item.storeCd = $("#stockDisuseRegistStoreCd").val();
        ...
        // {
        $scope._save("/kookmin/stock/stockDisuse/stockDisuseRegist/saveDisuse.sb", params, function () {
    ...
    $scope.saveRegistCallback = function () {
        // 신규등록인 경우
        if ($scope.callParent === "disuse") {
            var disuseScope = agrid.getScope('stockDisuseCtrl');
            disuseScope.getSearchStockDisuseList();
            ...
            // 조회 수행 : 조회URL, 파라미터, 콜백함수
            $.postJSON("/kookmin/stock/stockDisuse/stockDisuse/getSearchStockDisuseList.sb", params, function (result){
                $scope.seqNo = result.data.list[0].seqNo;
                // 진행구분 세팅
                $scope.disuse.reg.procFg = '0';
                $scope.getSearchDisuseRegistList();
            });
        }
        // 폐기상세내역 페이지에서 호출한 경우
            ...
            var disuseDtlScope = agrid.getScope('stockDisuseDtlCtrl');
            disuseDtlScope._setPagingInfo('curr', 1); // 페이지번호 1로 세팅
            disuseDtlScope.getSearchDisuseDtlList();
            disuseDtlScope.procFgCheck(false);
            $scope.getSearchDisuseRegistList();
        }

        // $scope.wjStockDisuseRegistLayer.hide(true);
    };
    ...
    $scope.excelTextUpload = function (prcsFg) {
        ...
        if(nvl($scope.disuse.reg.procFg,'') === ''){
        ...
        if (prcsFg === 'excelFormDown') {
            ...
            s_alert.popConf(msg, function () {
                ...
                excelUploadScope.parentCtrl = 'stockDisuseRegistCtrl';
                // 엑셀 업로드
                if (prcsFg === 'excelUp') {
                    $("#excelUpFile").val('');
    ...
    $scope.uploadCallBack = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/stock/stockDisuse/stockDisuseRegist/excelUpload.sb', /* 통신할 URL */
            params : params, /* 파라메터로 보낼 데이터 */
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
            if ($scope._httpStatusCheck(response, true)) {
    ...
    $scope.stockDisuseRegistVendrShow = function () {
        $scope._broadcast('stockDisuseRegistVendrCtrl');
    };
    ...
    angular.element(document).ready(function () {
        ...
        // 팝업 핸들러 추가
        $scope.wjStockDisuseRegistLayer.shown.addHandler(function () {
            setTimeout(function() {
                var params = {};
