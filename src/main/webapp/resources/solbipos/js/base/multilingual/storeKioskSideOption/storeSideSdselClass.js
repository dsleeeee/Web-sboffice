/****************************************************************
 *
 * 파일명 : storeSideSdselClass.js
 * 설  명 : 다국어관리(키오스크/사이드/옵션)(매장) - 사이드(선택분류명) 탭 JavaScript
 *
 ...
 * 사이드(선택분류명) 조회 그리드 생성
 */
app.controller('storeSideSdselClassCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeSideSdselClassCtrl', $scope, $http, true));
    ...
    $scope.$on("storeSideSdselClassCtrl", function (event, data) {
    ...
    $scope.getSideSdselClassList = function () {
        ...
        // 사이드(선택분류명) 탭 리스트 조회
        $scope._inquiryMain("/base/multilingual/storeKioskSideOption/getStoreSideSdselClassList.sb", params);
    };
        ...
        $scope._popConfirm(messages["cmm.choo.save"], function() {
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                if (nvl(item.sdselClassEnNm + '', '').getByteLengthForOracle() > 50) {
                    $scope._popMsg(messages["storeKioskSideOption.en"] + messages["cmm.overLength"] + " 50 " +
                ...
                if (nvl(item.sdselClassCnNm + '', '').getByteLengthForOracle() > 50) {
                    $scope._popMsg(messages["storeKioskSideOption.cn"] + messages["cmm.overLength"] + " 50 " +
                ...
                if (nvl(item.sdselClassJpNm + '', '').getByteLengthForOracle() > 50) {
                    $scope._popMsg(messages["storeKioskSideOption.jp"] + messages["cmm.overLength"] + " 50 " +
                        ", 현재 : " + item.sdselClassJpNm.getByteLengthForOracle() + messages["cmm.bateLengthInfo"]); // 일문 데이터 중 문자열의 길이가 너무 긴 데이터가 있습니다.
                    return false;
                }
            }

            $scope._save("/base/multilingual/storeKioskSideOption/saveStoreSideSdselClass.sb", params, function(result) {
                // 재조회
                $scope.getSideSdselClassList();
            });
        });
    ...
    $scope.sampleDownload = function () {
        ...
        var vScope = agrid.getScope('storeSideSdselClassExcelCtrl');
 ...
 */
app.controller('storeSideSdselClassExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeSideSdselClassExcelCtrl', $scope, $http, false));
    ...
    $scope.sampleDownload = function (data) {
        ...
        $scope._inquiryMain("/base/multilingual/storeKioskSideOption/getStoreSideSdselClassList.sb", params, function (){
            ...
            $timeout(function()	{
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(	$scope.flex,
                    ...
                    messages["storeKioskSideOption.sideSdselClass"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            ...
                            renameKey(item, '선택분류코드', 'sdselClassCd');
                            renameKey(item, '선택분류명', 'sdselClassNm');
                            renameKey(item, '선택분류명(영문)', 'sdselClassEnNm');
                            renameKey(item, '선택분류명(중문)', 'sdselClassCnNm');
                            renameKey(item, '선택분류명(일문)', 'sdselClassJpNm');
                        });
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/storeKioskSideOption/saveStoreSideSdselClass.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('storeSideSdselClassCtrl').getSideSdselClassList();
                }
            }
