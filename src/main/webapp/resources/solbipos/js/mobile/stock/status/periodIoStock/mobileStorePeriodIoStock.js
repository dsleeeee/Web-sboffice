 ...
 * 설  명 : (모바일) 재고현황 > 기간수불현황(매장) JavaScript
 *
...
app.controller('mobilePeriodIoStockCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobilePeriodIoStockCtrl', $scope, $http, true));
    ...
    $scope.mobilePeriodIoStockSelectVendrShow = function () {
        $scope._broadcast('mobilePeriodIoStockCtrlSelectVendrCtrl');
    };

    // grid 초기화 : 생성되기전 초기화되면서 생성된다
    $scope.initGrid = function (s, e) {
        // picker 사용시 호출 : 미사용시 호출안함
        $scope._makePickColumns("mobilePeriodIoStockCtrl");
    ...
    $scope.$on("mobilePeriodIoStockCtrl", function (event, data) {

        $scope.searchMobilePeriodIoStockList(true);
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    //다른 컨트롤러의 broadcast 받기(페이징 초기화)
    $scope.$on("mobilePeriodIoStockCtrlSrch", function (event, data) {
        $scope.comboArray =  $scope.comboArrayForSrc; // ?????????????
        $scope.searchMobilePeriodIoStockList(false);
    ...
    // 매장기간수불 리스트 조회
    $scope.searchMobilePeriodIoStockList = function (isPageChk) {
        // 파라미터
        var params     = {};
        ...
        params.vendrCd		= $("#mobilePeriodIoStockSelectVendrCd").val(); // 거래처
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquirySub("/mobile/stock/status/periodIoStock/mobilePeriodIoStock/periodIoStockList.sb", params, function (){
            gridShowMsgNoData("mobileStorePeriodIoStock", $scope.flex, "Y");
        },true);
    };
    ...
    $scope.srchOptionView = function(){
        ...
        for(var i=0; i<columns.length-2; i++){
            ...
            if(includeWord !== "" && includeWord !== "poUnitQty" && columns[i].binding != 'setInQty'){ // poUnitQty(입수)는 조회옵션에 따라 visible처리를 해야하는 컬럼이 아니라 무조건 표시해야하는 컬럼
                // 기초/마감재고
    ...
    $scope.excelDownload = function () {
        ...
        $scope._broadcast('mobilePeriodIoStockExcelCtrl',params);
    };
}]);

app.controller('mobilePeriodIoStockExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobilePeriodIoStockExcelCtrl', $scope, $http, $timeout, true));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("mobilePeriodIoStockExcelCtrl", function (event, data) {
        if(data != undefined && $scope.isSearch) {
            $scope.searchMobilePeriodIoStockExcelList(true);
            // 기능수행 종료 : 반드시 추가
            event.preventDefault();
        } else{
    ...
    // 전체 엑셀 리스트 조회
    $scope.searchMobilePeriodIoStockExcelList = function (isPageChk) {// 파라미터
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/mobile/stock/status/periodIoStock/mobilePeriodIoStock/periodIoStockExcelList.sb", params, function(){
