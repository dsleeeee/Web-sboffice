/****************************************************************
 *
 * 파일명 : mobileHqMonthIoStock.js
 * 설  명 : (모바일) 재고현황 > 월수불현황 JavaScript
 *
...
app.controller('mobileHqMonthIoStockCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileHqMonthIoStockCtrl', $scope, $http, true));
    ...
    $scope.mobileHqMonthIoStockSelectVendrShow = function () {
        $scope._broadcast('mobileHqMonthIoStockSelectVendrCtrl');
    };
...
/** 주문대비 입고현황 그리드 controller */
app.controller('mobileHqMonthIoStockMainCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileHqMonthIoStockMainCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("mobileHqMonthIoStockMainCtrl");
        ...
        s.formatItem.addHandler(function (s, e) {
            if (e.panel === s.cells) {
                ...
                if (col.binding === "prodCd" || (col.binding.slice(-3) == 'Qty' && s.cells.getCellData(e.row,e.col,false) != null) && col.binding !== "poUnitQty" && col.binding !== "setInQty" && col.binding !== "closeQty" && col.binding !== "baseQty") { // 상품코드, 각 수량별
    ...
    $scope.$on("mobileHqMonthIoStockMainCtrl", function (event, data) {
        $scope.searchMobileMonthIoStockList(true);
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    //다른 컨트롤러의 broadcast 받기
    $scope.$on("mobileHqMonthIoStockMainCtrlSrch", function (event, data) {
        $scope.searchMobileMonthIoStockList(false);
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    // 월수불현황 리스트 조회
    $scope.searchMobileMonthIoStockList = function (isPageChk) {
        ...
        params.vendrCd 	   = $("#mobileHqMonthIoStockSelectVendrCd").val();
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquirySub("/mobile/stock/status/monthIoStock/mobileMonthIoStock/viewList.sb", params, function () {
            $scope.displayChg($scope.srchOption);
            gridShowMsgNoData("mobileHqMonthIoStock", $scope.flex, "Y");
        },true);
        $scope.excelFg = true;
    };
    ...
    $scope.excelDownloadMonthIostock = function () {
        ...
        $scope._broadcast('mobileHqMonthIoStockExcelCtrl', params);
    };
...
/** 주문대비 입고현황 그리드 controller */
app.controller('mobileHqMonthIoStockExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileHqMonthIoStockExcelCtrl', $scope, $http, true));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("mobileHqMonthIoStockExcelCtrl", function (event, data) {
    ...
    $scope.searchMobileMonthIoStockExcelList = function (data) {
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/mobile/stock/status/monthIoStock/mobileMonthIoStock/viewExcelList.sb", params, function () {
