/****************************************************************
 *
 * 파일명 : excelUploadMPS.js
 * 설  명 : 공통팝업 수불/재고 엑셀업로드 JavaScript
 *
 ...
 *  엑셀업로드 그리드 생성
 */
app.controller('excelUploadMPSCtrl', ['$scope', '$http','$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('excelUploadMPSCtrl', $scope, $http, true));
    ...
    // <-- 검색 호출 -->
    // $scope.$on("excelUploadMPSCtrl", function(event, data) {
    //
    ...
    $scope.deleteExl = function (upFg) {
        ...
        $http({
            method : 'POST', //방식
            url    : "/iostock/cmmExcelUpload/excelUploadMPS/excelUploadMPS/delete.sb", /* 통신할 URL */
    ...
    $scope.save = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/iostock/cmmExcelUpload/excelUploadMPS/excelUploadMPS/save.sb', /* 통신할 URL */
    ...
    $scope.saveUpdateProdCd = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/iostock/cmmExcelUpload/excelUploadMPS/excelUploadMPS/saveUpdateProdCd.sb', /* 통신할 URL */
            params : params, /* 파라메터로 보낼 데이터 */
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
            if ($scope._httpStatusCheck(response, true)) {
