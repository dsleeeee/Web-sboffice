...
app.controller('popUpStoreSalePriceCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.$on("popUpStoreSalePriceCtrl", function(event, key) {
        ...
        $scope._setComboData("storeTukeyGrp", tukeyGrpData);
        // 매장브랜드 콤보박스
        $scope._setComboData("srchStoreHqBrandCd", brandList);
        // 상품브랜드 콤보박스
        $scope._setComboData("srchProdHqBrandCd", brandList);

        $timeout(function () {
            var scope = agrid.getScope("touchKeyCtrl");
            $scope.storeTukeyGrpCombo.selectedValue = scope.touchKeyGrp;
        }, 10);
        // 판매가 조회
    ...
    $scope.searchSalePriceList = function(){
        ...
        params.tukeyGrpCd = $scope.storeTukeyGrp;
        ...
        $scope._inquirySub("/base/prod/touchKey/touchKey/getStoreSalePrice.sb", params, function () {});
