/****************************************************************
 *
 * 파일명 : salePerfCompareBensonStore.js
 * 설  명 : 벤슨 > 배출분석 > 대비기간 매출실적 > 선택점포 탭 JavaScript
 *
...
app.controller('salePerfCompareBensonStoreCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('salePerfCompareBensonStoreCtrl', $scope, $http, $timeout, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        s.refresh();
        // picker 사용시 호출 : 미사용시 호출안함
        $scope._makePickColumns("salePerfCompareBensonStoreCtrl");
        ...
        // 첫째줄 헤더 생성
        var dataItem         = {};
        dataItem.storeCd            = messages["salePerfCompareBenson.storeCd"];
        dataItem.storeNm            = messages["salePerfCompareBenson.storeNm"];
        dataItem.compSaleCntComm    = messages["salePerfCompareBenson.compDate"];
        dataItem.compSaleAmtComm    = messages["salePerfCompareBenson.compDate"];
        dataItem.compSaleCntDlvr    = messages["salePerfCompareBenson.compDate"];
        dataItem.compSaleAmtDlvr    = messages["salePerfCompareBenson.compDate"];
        dataItem.compSaleCntPack    = messages["salePerfCompareBenson.compDate"];
        dataItem.compSaleAmtPack    = messages["salePerfCompareBenson.compDate"];
        dataItem.srchSaleCntComm    = messages["salePerfCompareBenson.srchDate"];
        dataItem.srchSaleAmtComm    = messages["salePerfCompareBenson.srchDate"];
        dataItem.srchSaleCntDlvr    = messages["salePerfCompareBenson.srchDate"];
        dataItem.srchSaleAmtDlvr    = messages["salePerfCompareBenson.srchDate"];
        dataItem.srchSaleCntPack    = messages["salePerfCompareBenson.srchDate"];
        dataItem.srchSaleAmtPack    = messages["salePerfCompareBenson.srchDate"];
        dataItem.grSaleCntComm      = messages["salePerfCompareBenson.growthRate"];
        dataItem.grSaleAmtComm      = messages["salePerfCompareBenson.growthRate"];
        dataItem.grSaleCntDlvr      = messages["salePerfCompareBenson.growthRate"];
        dataItem.grSaleAmtDlvr      = messages["salePerfCompareBenson.growthRate"];
        dataItem.grSaleCntPack      = messages["salePerfCompareBenson.growthRate"];
        dataItem.grSaleAmtPack      = messages["salePerfCompareBenson.growthRate"];
    ...
    $scope.$on("salePerfCompareBensonStoreCtrl", function (event, data) {
        $scope.searchSalePerfCompareBensonStoreList();
        // 기능수행 종료 : 반드시 추가
    ...
    // 매출실적비교 리스트 조회
    $scope.searchSalePerfCompareBensonStoreList = function () {


        if(isEmptyObject($("#salePerfCompareBensonStoreSelctStoreCd").val())){
            $scope._popMsg(messages["cmm.require.selectStore"]); // 매장을 선택해주세요
            return false;
        }
        if($("#salePerfCompareBensonStoreSelctStoreCd").val().split(",").length > 10) {
            $scope._popMsg(messages["salePerfCompareBenson.storeCntAlert"]); // 매장은 최대 10개 선택 가능합니다.
            return false;
        }
        ...
        params.storeCd          = $("#salePerfCompareBensonStoreSelctStoreCd").val();

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/anals/salePerfCompareBenson/salePerfCompareBenson/getSalePerfCompareBensonStoreList.sb", params, function (){

            // 그리드 상단에 조회일자 대비일자 표시
            var grid = wijmo.Control.getControl("#salePerfCompareBensonStoreMainGrid");
            var dataItem         = {};
            var srchDate = '';
            var compDate = '';
            if(dateFg === 'day' || dateFg  === 'month' || dateFg === 'year'){
            ...
            dataItem.storeCd            = messages["salePerfCompareBenson.storeCd"];
            dataItem.storeNm            = messages["salePerfCompareBenson.storeNm"];
            dataItem.compSaleCntComm    = messages["salePerfCompareBenson.compDate"] + compDate;
            dataItem.compSaleAmtComm    = messages["salePerfCompareBenson.compDate"] + compDate;
            dataItem.compSaleCntDlvr    = messages["salePerfCompareBenson.compDate"] + compDate;
            dataItem.compSaleAmtDlvr    = messages["salePerfCompareBenson.compDate"] + compDate;
            dataItem.compSaleCntPack    = messages["salePerfCompareBenson.compDate"] + compDate;
            dataItem.compSaleAmtPack    = messages["salePerfCompareBenson.compDate"] + compDate;
            dataItem.srchSaleCntComm    = messages["salePerfCompareBenson.srchDate"] + srchDate;
            dataItem.srchSaleAmtComm    = messages["salePerfCompareBenson.srchDate"] + srchDate;
            dataItem.srchSaleCntDlvr    = messages["salePerfCompareBenson.srchDate"] + srchDate;
            dataItem.srchSaleAmtDlvr    = messages["salePerfCompareBenson.srchDate"] + srchDate;
            dataItem.srchSaleCntPack    = messages["salePerfCompareBenson.srchDate"] + srchDate;
            dataItem.srchSaleAmtPack    = messages["salePerfCompareBenson.srchDate"] + srchDate;
            dataItem.grSaleCntComm      = messages["salePerfCompareBenson.growthRate"];
            dataItem.grSaleAmtComm      = messages["salePerfCompareBenson.growthRate"];
            dataItem.grSaleCntDlvr      = messages["salePerfCompareBenson.growthRate"];
            dataItem.grSaleAmtDlvr      = messages["salePerfCompareBenson.growthRate"];
