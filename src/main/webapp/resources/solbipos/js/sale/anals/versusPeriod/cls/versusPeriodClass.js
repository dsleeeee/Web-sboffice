...
app.controller('versusPeriodClassCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  // 대비일자 세팅
  $scope.compStartDate = wcombo.genDateVal("#compClassStartDate", result);
  $scope.compEndDate   = wcombo.genDateVal("#compClassEndDate", result);
...
app.controller('versusPeriodClassDtlCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.$on("versusPeriodClassDtlCtrl", function (event, data) {
	  ...
	  $scope.compStartDate = [data.compStartDate.slice(0, 4), "-", data.compStartDate.slice(4, 6), "-", data.compStartDate.slice(6, 8)].join('');
	  $scope.compEndDate = [data.compEndDate.slice(0, 4), "-", data.compEndDate.slice(4, 6), "-", data.compEndDate.slice(6, 8)].join('');
