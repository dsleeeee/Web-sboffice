...
app.controller('storeDayPayCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('storeDayPayCtrl', $scope, $http, true));
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("storeDayPayCtrl");
  ...
  $scope.$on("storeDayPayCtrl", function (event, data) {
    $scope.searchDayList();
    // 기능수행 종료 : 반드시 추가
  ...
  $scope.searchDayList = function () {
    ...
    $scope.searchedStoreCd = $("#storeDayPaySelectStoreCd").val();
    // 파라미터
    var params       = {};
  ...
  $scope.storeDayPaySelectStoreShow = function () {
    $scope._broadcast('storeDayPayStoreCtrl');
  };
    ...
    $timeout(function () {
      wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
        ...
        includeColumns: function (column) {
          ...
          messages["storeDayPayMoms.storeDayPayMoms"] + '_' + getCurDateTime() +'.xlsx', function () {
        $timeout(function () {
