...
app.controller('storageHqCurrCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.excelDownloadStorageHqCurr = function () {
	    // 파라미터
	    var params     = {};
		$scope._broadcast('storageHqCurrExcelCtrl',params);
	    ...
	    $http({
	    	...
	    	if ($scope._httpStatusCheck(response, true)) {
	    		if (!$.isEmptyObject(response.data.data.list)) {
	    			...
	    			//엑셀다운 그리드 반영 파라메터
	    		    $scope.excelGridStorageCd   = storageCd;
	    		    $scope.excelGridStorageNm   = storageNm;	
...
app.controller('storageHqCurrExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('storageHqCurrExcelCtrl', $scope, $http, $timeout, true));
  ...
  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("storageHqCurrExcelCtrl", function (event, data) {
		if(data != undefined && $scope.excelFg) {
			$scope.searchstorageHqCurrList();
		}else{
...
//	$scope.getReStorageNmList(true, true); //(동적 그리드 그리기, 페이징)
  ...
  // 창고별현재고현황 리스트 조회
  $scope.searchstorageHqCurrList = function () {
    // 파라미터
    var params         = {};
	...
	// 조회 수행 : 조회URL, 파라미터, 콜백함수
	$scope._inquiryMain("/stock/curr/storageHqCurr/storageHqCurr/excelList.sb", params, function() {
		...
		$timeout(function () {
			...
			}, messages["cmmStockStatus.stockStatus"]+'_'+messages["storageHqCurr.storageHqCurrList"]+'_'+getToday()+'.xlsx', function () {
				$timeout(function () {
  ...
  //상품분류 항목표시 함수
  $scope.fnDisplay = function() {
	  var grid = wijmo.Control.getControl("#storageHqCurrExcelGrid");
      var columns = grid.columns;
      var length  = grid.columns.length;
      var isDisplay  = $scope.isDisplay;
	  if(isDisplay){
	  ...
	  $scope.makeDataGrid = function () {
		  ...
		  var excelStorageCd = $scope.excelGridStorageCd;
		  var excelStorageNm = $scope.excelGridStorageNm;
		  
		  var arrStorageCd = excelStorageCd.split(',');
		  var arrStorageNm = excelStorageNm.split(',');
		  var colLength = grid.columns.length;
		  var addLength = arrStorageCd.length;
		  if($scope.orgnFg == "H"){
			  if(colLength-6 > 11){
