/****************************************************************
 *
 * 파일명 : prepaidCardUseStatus.js
 * 설  명 : 선불카드 사용 현황 JavaScript
 *
 ...
 *  선불카드 사용 현황 그리드 생성
 */
app.controller('prepaidCardUseStatusCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prepaidCardUseStatusCtrl', $scope, $http, false));

    // 조회일자 셋팅
    $scope.srchStartDate = wcombo.genDateVal("#srchStartUseDate", gvStartDate);
    $scope.srchEndDate   = wcombo.genDateVal("#srchEndUseDate", gvEndDate);

    //
    $scope._setComboData("useYn", useYnAllFgData);
        ...
        s.addEventListener(s.hostElement, 'mousedown', function (e) {
            ...
            if (ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if (col.binding === "storeCd" || col.binding === "saleDate") {
                    ...
                    $scope._broadcast('prepaidCardUseStatusDtlCtrl', params);
                    $scope.prepaidCardUseStatusDtlLayer.show(true);
    ...
    $scope.$on("prepaidCardUseStatusCtrl", function (event, data) {

        // 조회
        $scope.searchPrepaidCardUseStatus();
    ...
    // 조회
    $scope.searchPrepaidCardUseStatus = function () {
        ...
        params.saleYn = $scope.useYnCombo.selectedValue;
        params.storeCds = $("#prepaidCardUseStatusStoreCd").val();
        params.listScale = 50;
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "선불카드 사용 현황(현재화면)" + '_' + getCurDateTime() + '.xlsx', function () {
                    $timeout(function () {
    ...
    $scope.excelDownload = function () {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('prepaidCardUseStatusExcelCtrl', params);
        });
    };

}]);

app.controller('prepaidCardUseStatusExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prepaidCardUseStatusExcelCtrl', $scope, $http, false));
    ...
    $scope.$on("prepaidCardUseStatusExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/status/prepaidCardStatus/getPrepaidCardUseStatusExcelList.sb", params, function() {
            ...
            $timeout(function () {
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.excelFlex, {
                    includeColumnHeaders: true,
                    includeCellStyles: false
                }, "선불카드 사용 현황(조회조건)" + '_' + getCurDateTime() + '.xlsx', function () {
                    $timeout(function () {
                        $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
                    }, 10);
                });
