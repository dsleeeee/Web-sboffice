    ...
    <div id="_brmkLayer" class="layer" style="display: none;">
        <div class="layer_inner">
            ...
            <div class="title w870">
                ...
                <div class="btnSet">
                    <span><a href="javascript:void(0);" class="btn_blue bkmkClose">확인</a></span>
                </div>
            </div>
            <!--//layerContent-->
        </div>
...
function setFav(resrceCd) {
  var jsonUrl = "/com/bkmk/regist.sb";
  if ( $("#fav_" + resrceCd).hasClass("ic_fav on") ) {
    jsonUrl = "/com/bkmk/remove.sb";
  }

  var param = {};
  param.resrceCd = resrceCd;
  $.postJSON(jsonUrl, param, function(result) {
    if ( $("#fav_" + resrceCd).hasClass("ic_fav off") ) {
      $("#fav_" + resrceCd).removeClass("ic_fav off").addClass("ic_fav on");
    } else {
      $("#fav_" + resrceCd).removeClass("ic_fav on").addClass("ic_fav off");
    }
  },
  function(result){
...
$(document).ready(function() {
  ...
  $(".bkmkClose").click(function(e){
    closeBkmkLayer();
  });

  function closeBkmkLayer() {

    var param = {};
    $.postJSON("/com/bkmk/refresh.sb", param, function(result) {
      bkmkMenu.refresh;
    },
    function(result){
      s_alert.pop(result.data.msg);
                ...
                if ( second ) {
                  ...
                  if ( second.items.length > 0 ) {
                    second.items.forEach(function(third) {
                      ...
                      menuHtml += '<a href="javascript:setFav(\'' + third.cd + '\');" id="fav_' + third.cd + '" class="ic_fav off"></a>';
