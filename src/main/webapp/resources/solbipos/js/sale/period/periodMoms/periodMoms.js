...
/** controller */
app.controller('periodMomsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('periodMomsCtrl', $scope, $http, true));
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("periodMomsCtrl");
    ...
    dataItem.totSaleQty	      = messages["periodMoms.srchDate"];
    ...
    dataItem.totSaleQty1	  = messages["periodMoms.compDate"];
    ...
    dataItem.rateSaleAmt      = messages["periodMoms.ratePer"];
  ...
  $scope.$on("periodMomsCtrl", function (event, data) {
  ...
  $scope.searchCashBillList = function () {
    ...
    // 파라미터
    var params       = {};
    params.storeCds   = $("#periodMomsStoreCd").val();
    ...
    $scope._inquiryMain("/sale/period/periodMoms/periodMoms/getPeriodMomsList.sb", params);
  };

  // 매장선택 모듈 팝업 사용시 정의
  // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
  // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
  $scope.periodMomsStoreShow = function () {
    $scope._broadcast('periodMomsStoreCtrl');
  };
  ...
  $scope.excelDownload = function () {
    ...
    $scope._broadcast('periodMomsExcelCtrl',params);
  }
}]);


app.controller('periodMomsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('periodMomsExcelCtrl', $scope, $http, true));
  ...
  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("periodMomsExcelCtrl", function (event, data) {
  ...
  $scope.searchExcelList = function (data) {
    ...
    // 조회 수행 : 조회URL, 파라미터, 콜백함수
    $scope._inquiryMain("/sale/period/periodMoms/periodMoms/getPeriodMomsExcelList.sb", params, function() {
