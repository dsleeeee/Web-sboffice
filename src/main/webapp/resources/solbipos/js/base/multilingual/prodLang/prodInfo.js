 ...
 *
 * 파일명 : prodInfo.js
 * 설  명 : 다국어관리(상품) - 상품설명 탭 JavaScript
 *
 ...
 * 상품설명 조회 그리드 생성
 */
app.controller('prodInfoCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodInfoCtrl', $scope, $http, true));
    ...
    $scope.getProdInfoList = function () {
        ...
        // 상품설명 탭 리스트 조회
        $scope._inquiryMain("/base/multilingual/prodLang/getProdInfoList.sb", params, function() {
    ...
    $scope.saveRow = function(){
        $scope._popConfirm(messages["cmm.choo.save"], function() {
            ...
            $scope._save("/base/multilingual/prodLang/saveProdInfo.sb", params, function(result) {
                // 재조회
                $scope.getProdInfoList();
            });
        });
    ...
    $scope.excelDownload = function(){
        ...
        $scope.$broadcast('loadingPopupActive', messages["cmm.progress"]); // 데이터 처리중 메시지 팝업 열기
            $timeout(function () {
                ...
                }, messages["prodLang.prodInfo"]  + '_' +  getCurDateTime() + '.xlsx', function () {
                    $timeout(function () {
    ...
    $scope.sampleDownload = function () {
        ...
        var vScope = agrid.getScope('prodInfoExcelCtrl');
...
app.controller('prodInfoExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
        ...
        $scope._inquiryMain("/base/multilingual/prodLang/getProdInfoList.sb", params, function (){
            ...
            $timeout(function()	{
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(	$scope.flex,
                    ...
                    messages["prodLang.prodInfo"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/prodLang/saveProdInfo.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('prodInfoCtrl').getProdInfoList();
                }
            }
