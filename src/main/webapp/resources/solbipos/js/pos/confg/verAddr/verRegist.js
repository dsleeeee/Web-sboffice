...
app.controller('verRegistCtrl', ['$scope', '$http', function ($scope, $http) {
  ...
  $scope.regist = function(){
    ...
    if($("#verSerNo").val() === null || $("#verSerNo").val() === undefined || $("#verSerNo").val() === "") {
      $scope._popMsg(messages["verAddr.verSerNo"] + " " + messages["cmm.require.text"]);
      return;
    }

    // 버전적용명 체크
    if($("#verSerNm").val() === null || $("#verSerNm").val() === undefined || $("#verSerNm").val() === "") {
      $scope._popMsg(messages["verAddr.verSerNm"] + " " + messages["cmm.require.text"]);
      return;
    }
    ...
    if($("#file").val() !== null && $("#file").val() !== undefined && $("#file").val() !== "") {
      ...
      if (1 > imgFullNm.lastIndexOf('.')) {
        $scope._popMsg(messages["verAddr.fileNmChk.msg"]);
        return;
      }
    }
    ...
    if( isEmptyObject($scope.getSelectVersion()) ) {
      url = '/pos/confg/verAddr/verInfo/regist.sb';
    } else {
      url = '/pos/confg/verAddr/verInfo/modify.sb';
    }
  ...
  $scope.getVersionInfo = function(){
    ...
    $scope._postJSONQuery.withOutPopUp( "/pos/confg/verAddr/verInfo/dtlInfo.sb", params, function(response){
