/****************************************************************
 *
 * 파일명 : inOutStockProdReg.js
 * 설  명 : 국민대 > 매입처관리 > 매입전표 등록 > 상품추가/변경 팝업JavaScript
 *
...
app.controller('inOutStockProdRegCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('inOutStockProdRegCtrl', $scope, $http, true));

    $scope._setComboData("safeStockFg", [
        {"name": messages["cmm.all"], "value": ""},
        {"name": messages["acquireSlipRegist.reg.safeStockY"], "value": "Y"}
    ]);

    $scope._setComboData("addQtyFg", [
        {"name": messages["acquireSlipRegist.reg.addQtyFgApply"], "value": "apply"} ,
        {"name": messages["acquireSlipRegist.reg.addQtyFgAdd"], "value": "add"}
    ]);

    // $scope._setComboData("prodRegTradeFgComboData", tradeFgComboData);
    // $scope._setComboData("prodRegTradeFormComboData", tradeFormComboData);
...
//    comboParams         = {};
        ...
        s.cellEditEnded.addHandler(function (s, e) {
            if (e.panel === s.cells) {
                ...
                if(col.binding === "costUprc"){
                    var item = s.rows[e.row].dataItem;
                    item.acquireRate = (item.costUprc * 100) / item.saleUprc;
                    $scope.calcAmt(item);
                }
            }
        ...
        //첫째줄 Header 생성
        var dataItem = {};
        dataItem.prodCd         = messages["acquireSlipRegist.reg.prodCd"         ];	//상품코드
        dataItem.prodNm         = messages["acquireSlipRegist.reg.prodNm"         ];	//상품명
        dataItem.lastCostUprc   = messages["acquireSlipRegist.reg.lastCostUprc"   ];	//최종원가
        dataItem.poUnitFg       = messages["acquireSlipRegist.reg.poUnitFg"       ];   //주문단위
        dataItem.poUnitQty      = messages["acquireSlipRegist.reg.poUnitQty"      ];   //입수
        dataItem.prevInUnitQty  = messages["acquireSlipRegist.reg.prevInUnitQty"  ];   //기수량
        dataItem.prevInEtcQty   = messages["acquireSlipRegist.reg.prevInUnitQty"  ];   //기수량
        dataItem.costUprc    = messages["acquireSlipRegist.reg.acquireUprc"       ];	//매입가
        dataItem.inUnitQty      = messages["acquireSlipRegist.reg.inUnitQty"      ];	//수량
        dataItem.inEtcQty       = messages["acquireSlipRegist.reg.inUnitQty"      ];	//수량
        dataItem.prevInTotQty   = messages["acquireSlipRegist.reg.prevInTotQty"   ];	//이전총수량
        dataItem.inTotQty       = messages["acquireSlipRegist.reg.inTotQty"       ];	//총수량
        dataItem.inAmt          = messages["acquireSlipRegist.reg.inAmt"          ];	//금액
        dataItem.splyUprc       = messages["acquireSlipRegist.reg.splyUprc"          ];	//VAT
        dataItem.saleUprc       = messages["acquireSlipRegist.reg.saleUprc"          ];	//VAT
        dataItem.acquireRate    = messages["acquireSlipRegist.reg.acquireRate"          ];	//VAT
        dataItem.inVat          = messages["acquireSlipRegist.reg.inVat"          ];	//VAT
        dataItem.inTot          = messages["acquireSlipRegist.reg.inTot"          ];	//합계(VAT포함)
        dataItem.vatFg          = messages["acquireSlipRegist.reg.vatFg"          ];	//상품부가세구분
        dataItem.vendrVatFg01   = messages["acquireSlipRegist.reg.vendrVatFg01"   ];	//부가세포함여부
//          dataItem.storeSplyUprc  = messages["acquireSlipRegist.reg.storeSplyUprc"  ];	//매장공급가
    ...
    $scope.$on("inOutStockProdRegCtrl", function (event, data) {
        ...
        if (!$.isEmptyObject(data)) {
            ...
            $scope.wjInOutStockProdRegLayer.show(true);

        }
        // 페이징처리에서 broadcast 호출시
        else {
            $scope.getSearchInOutStockRegList();
        }
        // 기능수행 종료 : 반드시 추가
    ...
    // 상품 리스트 조회
    $scope.getSearchInOutStockRegList = function () {
        // 파라미터
        var params       = {};
        params.slipNo    = $scope.slipNo;
        params.slipFg    = $scope.slipFg;
        params.vendrCd   = $scope.vendrCd;
        // params.tradeFg   = $scope.tradeFg;
        // params.trade     = $scope.tradeForm;
        params.listScale = 50;

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/kookmin/acquire/acquireSlipRegist/inOutStockProdReg/getSearchInOutStockRegList.sb", params, function () {
    ...
    $scope.save = function () {
        ...
        for (var i = 0; i < $scope.flex.collectionView.itemsEdited.length; i++) {
            ...
            if (item.inUnitQty !== null && (0 > parseInt(item.inUnitQty))) {
                $scope._popMsg(messages['acquireSlipRegist.reg.above.zero.inUnitQty']); // 주문단위수량은 0이상 입력합니다.
                return false;
            }
            if (item.inEtcQty !== null && (0 > parseInt(item.inEtcQty))) {
                $scope._popMsg(messages['acquireSlipRegist.reg.above.zero.inEtcQty']); //낱개수량은 0이상 입력합니다.
                return false;
            }
            if (item.inEtcQty !== null && (parseInt(item.inEtcQty) >= parseInt(item.poUnitQty))) {
                $scope._popMsg(messages['acquireSlipRegist.reg.not.inEtcQty']); // 낱개수량은 입수량보다 작아야 합니다.
                return false;
            }
            if (item.inTot !== null && (parseInt(item.inTot) > 9999999999)) {
                $scope._popMsg(messages['acquireSlipRegist.reg.not.overInTot']); // 주문금액이 너무 큽니다.
                return false;
            }
...
//      item.outStorageCd	= $scope.save.dtl.outStorageCd;
        ...
        $scope._save("/kookmin/acquire/acquireSlipRegist/inOutStockProd/saveInOutStockProd.sb", params, function () {
    ...
    $scope.saveRegistCallback = function () {
        ...
        var inOutStockProdScope = agrid.getScope('inOutStockProdCtrl');
        inOutStockProdScope.procFgCheck();
    };
    ...
    $scope.excelTextUpload = function (prcsFg) {
        ...
        if (prcsFg === 'excelFormDown') {
            ...
            s_alert.popConf(msg, function () {
                ...
                excelUploadScope.parentCtrl = 'inOutStockProdRegCtrl';
                // 엑셀 업로드
                if (prcsFg === 'excelUp') {
                    $("#excelUpFile").val('');
    ...
    $scope.uploadCallBack = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/acquire/acquireSlipRegist/inOutStockProdReg/excelUpload.sb', /* 통신할 URL */
    ...
    angular.element(document).ready(function () {
        ...
        // 팝업 핸들러 추가
        $scope.wjInOutStockProdRegLayer.shown.addHandler(function () {
            setTimeout(function() {
                var params = {};
