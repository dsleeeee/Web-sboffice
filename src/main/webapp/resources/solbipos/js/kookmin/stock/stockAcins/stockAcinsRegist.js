/****************************************************************
 *
 * 파일명 : stockAcinsRegist.js
 * 설  명 : 국민대 > 재고관리 > 재고실사 > 실사등록 팝업 JavaScript
 *
...
app.controller('stockAcinsRegistCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('stockAcinsRegistCtrl', $scope, $http, true));
    ...
    $scope.$on("stockAcinsRegistCtrl", function (event, data) {
        ...
        if (!$.isEmptyObject(data)) {
            ...
            // 거래처, 매장 세팅 초기화
            $("#stockAcinsRegistVendrCd").val('');
            $("#stockAcinsRegistVendrNm").val('');
            $("#stockAcinsRegistStoreCd").val('');
            $("#stockAcinsRegistStoreNm").val('');
            $("#stockAcinsRegistStorebtnCancelStoreCd").attr("disabled", false);
            $("#stockAcinsRegistStoreNm").attr("disabled", false);
            $scope.stockAcinsRegistVendrNmDisabled  = false; // 반출 거래처선택 모듈 input
            $scope.stockAcinsRegistVendrBtnDisabled = false; // 반출 선택취소
            $scope.getSearchAcinsRegistList();
        }
    ...
    $scope.procFgCheck = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/stock/stockAcins/stockAcins/getProcFgCheck.sb', /* 통신할 URL */
            params : params, /* 파라메터로 보낼 데이터 */
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
            if ($scope._httpStatusCheck(response, true)) {
                ...
                if (!$.isEmptyObject(response.data.data)) {
                    ...
                    // 거래처, 매장
                    $scope.acinsVendrCd = response.data.data.acinsVendrCd;
                    $scope.acinsVendrNm = '[' + response.data.data.acinsVendrCd + ']' + response.data.data.vendrNm;
                    $scope.acinsStoreCd = response.data.data.acinsStoreCd;
                    $scope.acinsStoreNm = '[' + response.data.data.acinsStoreCd + ']' + response.data.data.storeNm;
                }
            }
        }, function errorCallback(response) {
    ...
    $scope.layerShow = function () {
        $scope.wjStockAcinsRegistLayer.show(true);
        ...
        if($scope.callParent === "acins") {
            ...
            // 거래처, 매장 값 셋팅
            $("#stockAcinsRegistVendrCd").val($scope.acinsVendrCd);
            $("#stockAcinsRegistVendrNm").val($scope.acinsVendrNm);
            $("#stockAcinsRegistStoreCd").val($scope.acinsStoreCd);
            $("#stockAcinsRegistStoreNm").val($scope.acinsStoreNm);
            $("#stockAcinsRegistStorebtnCancelStoreCd").attr("disabled", true);
            $("#stockAcinsRegistStoreNm").attr("disabled", true);
            $scope.stockAcinsRegistVendrNmDisabled = true; // 반출 거래처선택 모듈 input
            $scope.stockAcinsRegistVendrBtnDisabled = true; // 반출 선택취소
        }
    };


    // 실사상품 리스트 조회
    $scope.getSearchAcinsRegistList = function () {
        // 파라미터
        var params         = {};
        params.acinsDate   = $scope.acinsDate;
        params.seqNo       = $scope.seqNo;
        ...
        // params.vendrCd     = $("#stockAcinsRegistVendrCd").val();
        // params.storeCd     = $("#stockAcinsRegistStoreCd").val();
        params.listScale   = $scope.listScale;
        params.storageCd    = $scope.acins.reg.adjStorageCd;

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquirySub("/kookmin/stock/stockAcins/stockAcinsRegist/getSearchAcinsRegistList.sb", params);
    };
    ...
    $scope.saveAcinsRegist = function () {
        ...
        if($scope.flex.collectionView.items.length > 0) {
            ...
            for (var i = 0; i < $scope.flex.collectionView.itemsAdded.length; i++) {
                ...
                item.vendrCd = $("#stockAcinsRegistVendrCd").val();
                item.storeCd = $("#stockAcinsRegistStoreCd").val();
        ...
        // {
        $scope._save("/kookmin/stock/stockAcins/stockAcinsRegist/saveAcins.sb", params, function () {
    ...
    $scope.saveRegistCallback = function () {
        // 신규등록인 경우
        if ($scope.callParent === "acins") {
            var acinsScope = agrid.getScope('stockAcinsCtrl');
            acinsScope.getSearchAcinsList();

            var params       = {};
            params.startDate = $scope.acinsDate;
            params.endDate   = $scope.acinsDate;

            $.postJSON("/kookmin/stock/stockAcins/stockAcins/getSearchAcinsList.sb", params, function(result) {
        ...
        // 실사상세내역 페이지에서 호출한 경우
            ...
            var acinsDtlScope = agrid.getScope('stockAcinsDtlCtrl');
            acinsDtlScope._setPagingInfo('curr', 1); // 페이지번호 1로 세팅
            acinsDtlScope.getSearchAcinsDtlList();
            acinsDtlScope.procFgCheck(false);
            $scope.getSearchAcinsRegistList();
        }

//    $scope.wjStockAcinsRegistLayer.hide(true);
    };
    ...
    $scope.searchProdKeyEvt = function (event) {
        if (event.keyCode === 13) { // 이벤트가 enter 이면
            ...
            if (searchFg) {
                ...
                var url = "/kookmin/stock/stockAcins/stockAcins/getAcinsInfo.sb";
    ...
    $scope.excelTextUpload = function (prcsFg) {
        ...
        if (prcsFg === 'excelFormDown') {
            ...
            s_alert.popConf(msg, function () {
                ...
                excelUploadScope.parentCtrl = 'stockAcinsRegistCtrl';
                // 엑셀 업로드
                if (prcsFg === 'excelUp') {
                    $("#excelUpFile").val('');
    ...
    $scope.uploadCallBack = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/stock/stockAcins/stockAcinsRegist/excelUpload.sb', /* 통신할 URL */
            params : params, /* 파라메터로 보낼 데이터 */
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
            if ($scope._httpStatusCheck(response, true)) {
    ...
    $scope.stockAcinsRegistVendrShow = function () {
        $scope._broadcast('stockAcinsRegistVendrCtrl');
    };
    ...
    angular.element(document).ready(function () {
        ...
        // 팝업 핸들러 추가
        $scope.wjStockAcinsRegistLayer.shown.addHandler(function () {
            setTimeout(function() {
                var params = {};
