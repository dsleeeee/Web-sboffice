/****************************************************************
 *
 * 파일명 : monthProdBenson.js
 * 설  명 : (벤슨) 월별상품매출현황 JavaScript
 *
...
app.controller('monthProdBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('monthProdBensonCtrl', $scope, $http, true));
    ...
    $scope.$on("monthProdBensonCtrl", function (event, data) {
    ...
    $scope.searchMonthProdBensonList = function () {
        ...
        params.storeCds = $("#monthProdBensonStoreCd").val();
        ...
        $scope._inquiryMain("/sale/prod/monthProdBenson/monthProdBenson/getMonthProdBensonList.sb", params, function (){
            // <-- 그리드 visible -->
    ...
    $scope.monthProdBensonSelectShow = function () {
        $scope._broadcast('monthProdBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function () {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('monthProdBensonExcelCtrl', params);
        });
    };
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('monthProdBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('monthProdBensonExcelCtrl', $scope, $http, false));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("monthProdBensonExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/prod/monthProdBenson/monthProdBenson/getMonthProdBensonExcelList.sb", params, function() {
            if ($scope.flex.rows.length <= 0) {
                $scope._popMsg(messages["excelUpload.not.downloadData"]); // 다운로드 할 데이터가 없습니다.
                return false;
            }
