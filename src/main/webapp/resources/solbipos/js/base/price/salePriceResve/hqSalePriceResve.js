...
app.controller('hqSalePriceResveCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.delProdPrice = function(){
        $scope._popConfirm(messages["kioskKeyMapResve.delConfirm"], function() {
            ...
            $scope._save('/base/price/salePriceResve/hqSalePriceResve/delHqSalePriceResve.sb', params, function(){
                $scope.searchHqSalePriceResveList();
            });
        });
    }
    ...
    $scope.saveProdPrice = function(){
        ...
        if($scope.storeSaveFgCombo.selectedValue === "choice" && $("#choiceSaveStoreCd").val() === "") {
            $scope._popMsg("매장을 선택해주세요");
            return false;
        }
                ...
                if (($scope.flex.collectionView.items[i].orgStartDate !== $scope.flex.collectionView.items[i].startDate) ||
                    ...
                    if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                        if ($scope.flex.collectionView.items[i].saleUprc < 0) {
                            $scope._popMsg(messages["salePriceResve.saleUprcTypeChk"]); // 판매가는 숫자(양수)만 입력해주세요.
                            return false;
                        }
                    } else {
                        ...
                        if ($scope.flex.collectionView.items[i].stinSaleUprc !== "" && $scope.flex.collectionView.items[i].stinSaleUprc !== null) {
                            ...
                            if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                if ($scope.flex.collectionView.items[i].stinSaleUprc < 0) {
                                    $scope._popMsg(messages["salePriceResve.stinSaleUprcTypeChk"]); // 내점판매가는 숫자(양수)만 입력해주세요.
                                    return false;
                                }
                            } else {
                                ...
                                if ($scope.flex.collectionView.items[i].stinSaleUprc <= -1000000000) {
                                    $scope._popMsg(messages["salePriceResve.stinSaleUprcInChk"]); // 변경내점-판매가 숫자만(정수9자리) 입력해주세요.
                                    return false;
                                }
                            }
                        }
                        ...
                        if ($scope.flex.collectionView.items[i].dlvrSaleUprc !== "" && $scope.flex.collectionView.items[i].dlvrSaleUprc !== null) {
                            ...
                            if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                if ($scope.flex.collectionView.items[i].dlvrSaleUprc < 0) {
                                    $scope._popMsg(messages["salePriceResve.dlvrSaleUprcTypeChk"]); // 배달판매가는 숫자(양수)만 입력해주세요.
                                    return false;
                                }
                            } else {
                                ...
                                if ($scope.flex.collectionView.items[i].dlvrSaleUprc <= -1000000000) {
                                    $scope._popMsg(messages["salePriceResve.dlvrSaleUprcInChk"]); // 변경배달-판매가 숫자만(정수9자리) 입력해주세요.
                                    return false;
                                }
                            }
                        }
                        ...
                        if ($scope.flex.collectionView.items[i].packSaleUprc !== "" && $scope.flex.collectionView.items[i].packSaleUprc !== null) {
                            ...
                            if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                if ($scope.flex.collectionView.items[i].packSaleUprc < 0) {
                                    $scope._popMsg(messages["salePriceResve.packSaleUprcTypeChk"]); // 포장판매가는 숫자(양수)만 입력해주세요.
                                    return false;
                                }
                            } else {
                    ...
                    console.log($scope.flex.collectionView.items[i].applyFg);
                    $scope.flex.collectionView.items[i].orgStartDate = $scope.flex.collectionView.items[i].orgStartDate.replaceAll('-', ''); // 키값
