...
/**
 * @Class Name : FnkeyCmNmcdMapper.java
...
@Mapper
@Repository
public interface FnkeyCmNmcdMapper {

    /** 기능키(공통) 탭 리스트 조회 */
    List<DefaultMap<String>> getCmPosFnkeyList(FnkeyCmNmcdVO fnkeyCmNmcdVO);
    ...
    int saveStoreFnkey(FnkeyCmNmcdVO fnkeyCmNmcdVO);

    /** 공통코드 탭 공통코드 그룹코드 조회(콤보박스용) */
    List<DefaultMap<String>> getNmcodeGrpCdList();

    /** 공통코드 탭 리스트 조회 */
    List<DefaultMap<String>> getCmNmcdList(FnkeyCmNmcdVO fnkeyCmNmcdVO);

    /** 공통코드 탭 영문, 중문, 일문 저장 */
    int saveCmNmcd(FnkeyCmNmcdVO fnkeyCmNmcdVO);
