...
app.controller('standMoveDtlCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('standMoveDtlCtrl', $scope, $http, true));
...
//	        item.standTotQty   		= arrTotQty;   //총입고수량     
  ...
  $scope.$on("standMoveDtlCtrl", function (event, data) {
    // 그리드 초기화
  ...
  $scope.searchStandMoveDtlList = function () {
    ...
    $scope._inquirySub("/iostock/move/standMove/standMoveDtl/list.sb", params, function () {
