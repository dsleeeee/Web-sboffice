 ...
 *
 * 파일명 : storeFnkey.js
 * 설  명 : 다국어관리(기능키/공통코드) - 기능키(매장) 탭 JavaScript
 *
 ...
 * 기능키(매장) 조회 그리드 생성
 */
app.controller('storeFnkeyCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeFnkeyCtrl', $scope, $http, true));

    // 콤보박스 데이터 Set
    $scope._setComboData("fnkeyFg2", fnkeyFgData); // 기능구분
    ...
    $scope.$on("storeFnkeyCtrl", function (event, data) {
        // 조회
        $scope.getStoreFnkeyList();
        event.preventDefault();
    });

    // 조회
    $scope.getStoreFnkeyList = function () {

        // 파라미터
        var params = {};
        params.storeCd = $("#multilingualStoreFnkeyCd").val();
        params.fnkeyFg = $scope.srchFnkeyFg2Combo.selectedValue;
        params.fnkeyNo = $("#srchFnkeyNo2").val();
        params.fnkeyNm = $("#srchFnkeyNm2").val();

        // 기능키(공통) 탭 리스트 조회
        $scope._inquiryMain("/base/multilingual/fnkeyCmNmcd/getStoreFnkeyList.sb", params);
    };
    ...
    $scope.saveRow = function(){
        $scope._popConfirm(messages["cmm.choo.save"], function() {
            ...
            $scope._save("/base/multilingual/fnkeyCmNmcd/saveStoreFnkey.sb", params, function(result) {
                // 재조회
                $scope.getStoreFnkeyList();
            });
        });
    ...
    $scope.excelDownload = function(){
        ...
        $scope.$broadcast('loadingPopupActive', messages["cmm.progress"]); // 데이터 처리중 메시지 팝업 열기
            $timeout(function () {
                ...
                }, messages["fnkeyCmNmcd.storeFnkey"]  + '_' +  getCurDateTime() + '.xlsx', function () {
                    $timeout(function () {
    ...
    $scope.excelUpload = function () {
        ...
        $scope._popConfirm(msg, function() {
            ...
            $("#storeFnkeyExcelUpFile").trigger('click');
    ...
    $scope.multilingualStoreFnkeyShow = function () {
        $scope._broadcast('multilingualStoreFnkeyCtrl');
    };
...
app.controller('storeFnkeyExcelDownCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.sampleDownload = function (data) {
        ...
        $scope._inquiryMain("/base/multilingual/fnkeyCmNmcd/getStoreFnkeyList.sb", params, function (){
            ...
            $timeout(function()	{
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(	$scope.flex,
                    ...
                    messages["fnkeyCmNmcd.storeFnkey"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
...
app.controller('storeFnkeyExcelUploadCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.excelUpload = function () {
        ...
        // 선택한 파일이 있으면
        if ($('#storeFnkeyExcelUpFile')[0].files[0]) {
            var file = $('#storeFnkeyExcelUpFile')[0].files[0];
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/fnkeyCmNmcd/saveStoreFnkey.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('storeFnkeyCtrl').getStoreFnkeyList();
                }
            }
