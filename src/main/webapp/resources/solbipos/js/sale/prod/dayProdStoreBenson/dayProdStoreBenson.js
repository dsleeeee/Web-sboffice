/****************************************************************
 *
 * 파일명 : dayProdStoreBenson.js
 * 설  명 : (벤슨) 일별상품매출현황(매장별) JavaScript
 *
...
app.controller('dayProdStoreBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dayProdStoreBensonCtrl', $scope, $http, true));
    ...
    $scope.$on("dayProdStoreBensonCtrl", function (event, data) {
        $scope.searchDayProdStoreBensonList();
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    // 일별상품매출현황(매장별) 리스트 조회
    $scope.searchDayProdStoreBensonList = function () {
        ...
        params.storeCds = $("#dayProdStoreBensonStoreCd").val();
        params.prodCds = $("#dayProdStoreBensonSelectCd").val();
        ...
        $scope._inquiryMain("/sale/prod/dayProdStoreBenson/dayProdStoreBenson/getDayProdStoreBensonList.sb", params, function (){
            // <-- 그리드 visible -->
    ...
    $scope.dayProdStoreBensonSelectShow = function () {
        $scope._broadcast('dayProdStoreBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function (excelType) {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('dayProdStoreBensonExcelCtrl', params);
        });
    };
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('dayProdStoreBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dayProdStoreBensonExcelCtrl', $scope, $http, false));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("dayProdStoreBensonExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/prod/dayProdStoreBenson/dayProdStoreBenson/getDayProdStoreBensonExcelList.sb", params, function() {
    ...
    $scope.searchExcelDivisionList = function (data) {
        ...
        params.limit = 1;
        params.offset = 1;
        $.postJSON( "/sale/prod/dayProdStoreBenson/dayProdStoreBenson/getDayProdStoreBensonList.sb", params, function(response){
