...
app.controller('apprPartnerCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  //조회조건 승인처리 데이터 Set
  $scope._setComboData("srchPartnerApprProcFgDisplay", [
    {"name": messages["cmm.all"], "value": ""},
  ...
  $scope.searchApprPartnerList = function (isPageChk) {
	  ...
	  var startDt = new Date(wijmo.Globalize.format($scope.srchApprPartnerStartDate.value, 'yyyy-MM-dd'));
	  var endDt = new Date(wijmo.Globalize.format($scope.srchApprPartnerEndDate.value, 'yyyy-MM-dd'));
    ...
    $scope.srchPosNo  	  = $("#apprPartnerSelectPosCd").val();
    $scope.srchCornrCd    = $("#apprPartnerSelectCornerCd").val();
  ...
  $scope.apprPartnerSelectStoreShow = function () {
	...
	$scope.excelDownloadPartner = function () {
		...
		// 파라미터
		var params       = {};
		$scope._broadcast('apprPartnerExcelCtrl',params);
	};
...
app.controller('apprPartnerExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

	// 상위 객체 상속 : T/F 는 picker
	angular.extend(this, new RootController('apprPartnerExcelCtrl', $scope, $http, $timeout, true));
	...
	// 다른 컨트롤러의 broadcast 받기
	$scope.$on("apprPartnerExcelCtrl", function (event, data) {

		if(data != undefined && $scope.isSearch) {
			$scope.searchapprPartnerExcelList();
			// 기능수행 종료 : 반드시 추가
			event.preventDefault();
		} else{
	...
	// 제휴카드 승인현황 리스트 조회
	  $scope.searchapprPartnerExcelList = function () {
		...
		// 조회 수행 : 조회URL, 파라미터, 콜백함수
		$scope._inquiryMain("/sale/status/appr/partner/excelList.sb", params, function() {
			...
			$timeout(function () {
				...
				}, messages["dailyReport.appr"]+'_'+messages["dailyReport.appr"]+'_'+messages["dailyReport.apprPartner"]+'_'+getToday()+'.xlsx', function () {
					$timeout(function () {
	...
	// 선택한 승인구분에 따른 리스트 항목 visible
		$scope.editDataGrid = function () {
	        var grid = wijmo.Control.getControl("#apprPartnerExcelGrid");
	        var columns = grid.columns;
