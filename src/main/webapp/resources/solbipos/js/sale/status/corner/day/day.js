...
app.controller('cornerDayCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.searchCornerDayList = function (isPageChk) {
	  ...
	  var startDt = new Date(wijmo.Globalize.format($scope.srchCornerDayStartDate.value, 'yyyy-MM-dd'));
	  var endDt = new Date(wijmo.Globalize.format($scope.srchCornerDayEndDate.value, 'yyyy-MM-dd'));
