...
app.controller('apprCashCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  //조회조건 승인처리 데이터 Set
  $scope._setComboData("srchCashApprProcFgDisplay", [
    {"name": messages["cmm.all"], "value": ""},
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("apprCashCtrl");
    ...
    s.addEventListener(s.hostElement, 'mousedown', function (e) {
      ...
      if (ht.cellType === wijmo.grid.CellType.Cell) {
        ...
        if (col.binding === "storeNm") { // 매장명
            ...
            $scope._broadcast('saleApprCashCtrl', params);
        }
      }
    }, true);
  ...
  $scope.searchApprCashList = function (isPageChk) {
	  ...
	  var startDt = new Date(wijmo.Globalize.format($scope.srchApprCashStartDate.value, 'yyyy-MM-dd'));
	  var endDt = new Date(wijmo.Globalize.format($scope.srchApprCashEndDate.value, 'yyyy-MM-dd'));
    ...
    $scope.srchPosNo  	  = $("#apprCashSelectPosCd").val();
    $scope.srchCornrCd    = $("#apprCashSelectCornerCd").val();
  ...
  $scope.isChkDt = function() {
    $scope.srchApprCashStartDate.isReadOnly = $scope.isChecked;
    $scope.srchApprCashEndDate.isReadOnly = $scope.isChecked;
  };

  //매장선택 모듈 팝업 사용시 정의
  // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
  // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
  $scope.apprCashSelectStoreShow = function () {
    $scope._broadcast('apprCashSelectStoreCtrl');
  };

  	//포스선택 모듈 팝업 사용시 정의
	// 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
	// _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
	$scope.apprCashSelectPosShow = function () {
		$scope._broadcast('apprCashSelectPosCtrl');
	};

	//코너선택 모듈 팝업 사용시 정의
	//함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
	//_broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
	$scope.apprCashSelectCornerShow = function () {
		$scope._broadcast('apprCashSelectCornerCtrl');
	};

//엑셀 다운로드
  $scope.excelDownloadCash = function () {
	...
	// 파라미터
	    var params       = {};
	    $scope._broadcast('apprCashExcelCtrl',params);
  };
  ...
  //매장의 포스(pos) 리스트 조회
	$scope.getPosNmList = function () {
		...
		comboParams.storeCd = $("#posCashSelectStoreCd").val();
	};
...
app.controller('apprCashExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

	// 상위 객체 상속 : T/F 는 picker
	angular.extend(this, new RootController('apprCashExcelCtrl', $scope, $http, $timeout, true));
	...
	// 다른 컨트롤러의 broadcast 받기
	$scope.$on("apprCashExcelCtrl", function (event, data) {

		if(data != undefined && $scope.isSearch) {
			$scope.searchApprCashExcelList();
			// 기능수행 종료 : 반드시 추가
			event.preventDefault();
		} else{
	...
	// 신용카드 승인현황 리스트 조회
	  $scope.searchApprCashExcelList = function () {
		...
		// 조회 수행 : 조회URL, 파라미터, 콜백함수
		$scope._inquiryMain("/sale/status/appr/cash/excelList.sb", params, function() {
			...
			$timeout(function () {
				...
				}, messages["dailyReport.appr"]+'_'+messages["dailyReport.appr"]+'_'+messages["dailyReport.apprCash"]+'_'+getToday()+'.xlsx', function () {
					$timeout(function () {
	...
	// 선택한 승인구분에 따른 리스트 항목 visible
		$scope.editDataGrid = function () {
			var grid = wijmo.Control.getControl("#apprCashExcelGrid");
	        var columns = grid.columns;
