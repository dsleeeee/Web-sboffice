...
public interface AcinsMapper {
    ...
    int deleteAcinsToExcelUploadData(ExcelUploadStoreVO excelUploadStoreVO);

    /** 실사관리 엑셀업로드 - 엑셀업로드 한 수량을 실사수량으로 입력 */
    int insertAcinsToExcelUploadData(ExcelUploadStoreVO excelUploadStoreVO);

    /** 실사관리 엑셀업로드 - 정상 입력된 데이터 TEMP 테이블에서 삭제 */
    int deleteExcelUploadCompleteData(ExcelUploadStoreVO excelUploadStoreVO);

    /** 사유값 */
    List<DefaultMap<String>> getAcinsReason(SessionInfoVO sessionInfoVO);
}
