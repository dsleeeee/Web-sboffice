/****************************************************************
 *
 * 파일명 : stockExcelUploadErrInfo.js
 * 설  명 : 공통팝업 국민대 재고관리 엑셀업로드 에러내역 JavaScript
 *
 ...
 *  엑셀업로드 에러내역 그리드 생성
 */
app.controller('stockExcelUploadErrInfoCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('stockExcelUploadErrInfoCtrl', $scope, $http, true));
    ...
    // <-- 검색 호출 -->
    $scope.$on("stockExcelUploadErrInfoCtrl", function(event, data) {
        // 그리드 초기화
        ...
        $scope.wjStockExcelUploadErrInfoLayer.show(true);
        $scope.gridVisible();
    ...
    $scope.searchExcelUploadErrInfoList = function () {
        ...
        $scope._inquiryMain("/kookmin/stock/stockAcins/stockCmm/getExcelUploadErrInfoList.sb", params);
    };
