/****************************************************************
 *
 * 파일명 : mobileHqDailyIoStock.js
 * 설  명 : (모바일) 재고현황 > 일자별수불현황 JavaScript
 *
...
app.controller('mobileDailyIoStockCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.searchMobileDailyIoStockList = function () {
        ...
        $scope._inquirySub("/stock/status/dailyIoStock/dailyIoStock/getDailyIoStockList.sb", params,function (){
            gridShowMsgNoData("mobileHqDailyIoStock", $scope.flex, "Y");
        },true);
    ...
    $scope.showOrHide = function (amtFg, qtyFg){
        ...
        for(var c = 0; c < columns.length; c++) {
            ...
            if(columns[c].binding != 'setInQty' && columns[c].binding != 'saleVendrOrderQty' && columns[c].binding != 'saleVendrOrderTot' && columns[c].binding != 'saleVendrRtnQty' && columns[c].binding != 'saleVendrRtnTot') {
                if (substr === "Qty") { // 수량
