 ...
 *
 * 파일명 : chgCostPrice.js
 * 설  명 : 기초관리 > 가격관리 > 원가임의변경 > 원가임의변경 엑셀업로드 탭 JavaScript
 *
 ...
 * 양식다운로드
 */
app.controller('chgCostPriceExcelUploadSampleCtrl', ['$scope', '$http','$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('chgCostPriceExcelUploadSampleCtrl', $scope, $http, false));
    ...
    $scope.$on("chgCostPriceExcelUploadSampleCtrl", function (event, data) {
        ...
        $scope._inquiryMain("/base/price/chgCostPrice/getCostPriceExcelUploadSampleList.sb", params, function () {
            ...
            $timeout(function () {
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.excelFlex,
                    ...
                    messages["chgCostPrice.chgCostPrice"] + '_' + fileNm + '_' + messages["chgCostPrice.excelUpload"] + '_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
    ...
    $scope.excelUpload = function () {
        // 엑셀업로드 팝업
        $("#chgCostPriceExcelUpFile").val('');
        $("#chgCostPriceExcelUpFile").trigger('click');
    };
 ...
 * 원가임의변경 엑셀업로드 그리드
 */
app.controller('chgCostPriceExcelUploadCtrl', ['$scope', '$http','$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('chgCostPriceExcelUploadCtrl', $scope, $http, false));

    // 선택한 변경항목 값을 사용하기 위해
    var vScope = agrid.getScope('chgCostPriceExcelUploadSampleCtrl');
    ...
    $scope.$on("chgCostPriceExcelUploadCtrl", function (event, data) {
        // 원가 업로드 임시테이블 데이터 조회
    ...
    $scope.save = function () {
        ...
        $scope._postJSONSave.withOutPopUp("/base/price/chgCostPrice/deleteCostPriceExcelUploadCheckAll.sb", params, function () {
    ...
    $scope.saveSave = function (jsonData) {
        ...
        $http({
            method: 'POST', //방식
            url: '/base/price/chgCostPrice/saveCostPriceExcelUploadCheckResult.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    ...
                    var msg = messages["chgCostPrice.saveConfirm"];
    ...
    $scope.save2 = function (orgParams) {
        ...
        $http({
            method: 'POST', //방식
            url: '/base/price/chgCostPrice/saveCostPriceExcelUpload.sb', /* 통신할 URL */
            data: params, /* 파라메터로 보낼 데이터 : @requestBody */
            params: sParam,
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
    ...
    $scope.excelDownload = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex,
                ...
                messages["chgCostPrice.chgCostPrice"] + '_' + fileNm + '_' + messages["chgCostPrice.excelUpload"] + '_' + getCurDateTime() + '.xlsx',
                function () {
                    $timeout(function () {
                        $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                    }, 10);
    ...
    $scope.deleteSave = function () {
        ...
        // 저장기능 수행 : 저장URL, 파라미터, 콜백함수
        $scope._postJSONSave.withOutPopUp("/base/price/chgCostPrice/deleteCostPriceExcelUploadCheck.sb", params, function () {
            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
        });
    };
 ...
 * 엑셀업로드
 */
app.controller('chgCostPriceExcelUploadAddCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('chgCostPriceExcelUploadAddCtrl', $scope, $http, false));
    ...
    $scope.$on("chgCostPriceExcelUploadAddCtrl", function (event, data) {
        event.preventDefault();
    });
    ...
    $scope.excelUpload = function () {
        ...
        // 선택한 파일이 있으면
        if ($('#chgCostPriceExcelUpFile')[0].files[0]) {
            var file          = $('#chgCostPriceExcelUpFile')[0].files[0];
            ...
            if (fileExtension.toLowerCase() === '.xlsx' || fileExtension.toLowerCase() === '.xlsm') {
                ...
                // $timeout(function () {
                //     var flex = $scope.flex;
                //     wijmo.grid.xlsx.FlexGridXlsxConverter.loadAsync(flex, $('#chgCostPriceExcelUpFile')[0].files[0], {includeColumnHeaders: true}
                //         , function () {
                //             $timeout(function () {
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            ...
                            renameKey(item, '판매가', 'saleUprc');
                            renameKey(item, '공급가', 'splyUprc');
                            renameKey(item, '현재원가', 'bCostUprc');
                            renameKey(item, '변경원가', 'costUprc');
    ...
    $scope.save = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/price/chgCostPrice/saveCostPriceExcelUploadCheck.sb', /* 통신할 URL */
            data   : params, /* 파라메터로 보낼 데이터 : @requestBody */
            params : sParam,
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    $scope.excelUploadingPopup(false); // 업로딩 팝업 닫기
                    // 저장기능 수행후 재조회
                    $scope._broadcast('chgCostPriceExcelUploadCtrl');
                }
            }
        }, function errorCallback(response) {
            $scope.excelUploadingPopup(false); // 업로딩 팝업 닫기
