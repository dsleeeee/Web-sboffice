...
app.controller('barcdCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("barcdCtrl");
    ...
    s.addEventListener(s.hostElement, 'mousedown', function (e) {
      ...
      if (ht.cellType === wijmo.grid.CellType.Cell) {
        ...
        var params       = {};
        	...
        	params.barcdCd	 = selectedRow.barcdCd;
        if (col.binding === "barcdCd") { // 바코드
            $scope._broadcast('barcdCtrl', params);
        }
      }
    });
  ...
  $scope.searchBarcdList = function () {
	...
	$scope._inquirySub("/sale/status/barcd/barcd/list.sb", params);
	...
	$scope.loadedRows = function(sender, args){
		...
		var params		 = {};
	    params.barcdCd   = rows[0].dataItem.barcdCd;
...
app.controller('barcdDtlCtrl', ['$scope', '$http','$timeout', function ($scope, $http, $timeout) {
	 // 상위 객체 상속 : T/F 는 picker
	  ...
	  // 코너별매출일자별 리스트 조회
	  $scope.searchBarcdDtlList = function () {
	    // 파라미터
	    var params          = {};
