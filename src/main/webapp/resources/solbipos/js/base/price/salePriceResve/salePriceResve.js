/****************************************************************
 *
 * 파일명 : salePriceResve.js
 * 설  명 : 가격예약(판매가관리) JavaScript
 *
 ...
 *  판매가관리(매장용) 그리드 생성
 */
app.controller('salePriceResveCtrl', ['$scope', '$http', function ($scope, $http) {
        ...
        s.addEventListener(s.hostElement, 'mousedown', function (e) {
            ...
            if (ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if (col.binding === "prodCd") {
                    ...
                    $scope.salePriceInfoLayer.show(true);
                    $scope._broadcast('salePriceInfoCtrl', params);
                }
            }
        });
    ...
    $scope.searchSalePriceResveList = function() {
        ...
        $scope._inquirySub('/base/price/salePriceResve/salePriceResve/getSalePriceResveList.sb', params, function() {
    ...
    $scope.delProdPrice = function(){
        $scope._popConfirm(messages["kioskKeyMapResve.delConfirm"], function() {
            ...
            $scope._save('/base/price/salePriceResve/salePriceResve/delSalePriceResve.sb', params, function(){
                $scope.searchSalePriceResveList();
            });
        });
    }
    ...
    $scope.saveProdPrice = function(){
        ...
        $scope._popConfirm(messages["cmm.choo.save"], function() {
                    ...
                    if (($scope.flex.collectionView.items[i].orgStartDate !== $scope.flex.collectionView.items[i].startDate) ||
                        ...
                        if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                            if ($scope.flex.collectionView.items[i].saleUprc < 0) {
                                $scope._popMsg(messages["salePriceResve.saleUprcTypeChk"]);
                                return false;
                            }
                        } else {
                            ...
                            if ($scope.flex.collectionView.items[i].stinSaleUprc !== "" && $scope.flex.collectionView.items[i].stinSaleUprc !== null) {
                                ...
                                if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                    if ($scope.flex.collectionView.items[i].stinSaleUprc < 0) {
                                        $scope._popMsg(messages["salePriceResve.stinSaleUprcTypeChk"]);
                                        return false;
                                    }
                                } else {
                            ...
                            if ($scope.flex.collectionView.items[i].dlvrSaleUprc !== "" && $scope.flex.collectionView.items[i].dlvrSaleUprc !== null) {
                                ...
                                if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                    if ($scope.flex.collectionView.items[i].dlvrSaleUprc < 0) {
                                        $scope._popMsg(messages["salePriceResve.dlvrSaleUprcTypeChk"]);
                                        return false;
                                    }
                                } else {
                            ...
                            if ($scope.flex.collectionView.items[i].packSaleUprc !== "" && $scope.flex.collectionView.items[i].packSaleUprc !== null) {
                                ...
                                if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                    if ($scope.flex.collectionView.items[i].packSaleUprc < 0) {
                                        $scope._popMsg(messages["salePriceResve.packSaleUprcTypeChk"]);
                                        return false;
                                    }
                                } else {
            ...
            // 저장기능 수행 : 저장URL, 파라미터, 콜백함수
            $scope._save('/base/price/salePriceResve/salePriceResve/modSalePriceResve.sb', params, function () {
                $scope.searchSalePriceResveList();
            });
        });
    };
    ...
    $scope.addProdPrice = function () {
        $scope.salePriceResveAddLayer.show(true);
        $scope._pageView('salePriceResveAddCtrl',1);
    };

    // 그리드 선택불가 항목처리
    $scope.setGridReadOnly = function () {
        var grid = wijmo.Control.getControl("#wjGridSalePriceResve");
        var rows = grid.rows;
