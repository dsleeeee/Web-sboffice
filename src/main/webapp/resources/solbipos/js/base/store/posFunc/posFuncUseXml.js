...
// angular 그리드 생성
app.controller('posFuncUseXmlCtrl', ['$scope', '$http', 'saveInfo', function ($scope, $http, saveInfo) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('posFuncUseXmlCtrl', $scope, $http, false));
    ...
    // 포스기능키목록 그리드 조회
    $scope.$on("posFuncUseXmlCtrl", function(event, data) {

        $scope.posFuncUseXmlLayer.show(true);

        $("#xmlTitle").text(data.fnkeyNm);

        // 기능키 분류 정보 Setting
        $scope.setSaveInfo("storeCd", data.storeCd);
        $scope.setSaveInfo("posNo", data.posNo);
        $scope.setSaveInfo("fnkeyFg", data.fnkeyFg);
