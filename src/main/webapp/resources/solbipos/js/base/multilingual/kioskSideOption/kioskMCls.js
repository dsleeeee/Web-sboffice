 ...
 *
 * 파일명 : kioskMCls.js
 * 설  명 : 다국어관리(키오스크/사이드/옵션) - 키오스크중분류(카테고리명) 탭 JavaScript
 *
 ...
 */
app.controller('kioskMClsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('kioskMClsCtrl', $scope, $http, true));

    // 조회조건 콤보박스 데이터 Set
    $scope._setComboData("tuClsType2", kioskTuClsType2List); // 키오스크 키맵그룹 조회(중분류 사용 키맵그룹만 조회)
    ...
    $scope.$on("kioskMClsCtrl", function (event, data) {
    ...
    $scope.getKioskCategory = function () {
        ...
        $scope._postJSONQuery.withOutPopUp('/base/multilingual/kioskSideOption/getKioskCategoryComboList.sb', params, function (response) {
    ...
    $scope.saveRow = function () {
        $scope._popConfirm(messages["cmm.choo.save"], function () {
            ...
            $scope._save("/base/multilingual/kioskSideOption/saveKioskMCls.sb", params, function (result) {
                // 재조회
                $scope.getKioskMClsList();
            });
        });
    ...
    $scope.excelDownload = function(){
        ...
        $timeout(function () {
            ...
            }, messages["kioskSideOption.kioskMCls"] + '_' + getCurDateTime() + '.xlsx', function () {
                $timeout(function () {
   ...
   $scope.sampleDownload = function () {
       ...
       var vScope = agrid.getScope('kioskMClsExcelCtrl');
       vScope.sampleDownload($scope.srchTuClsType2Combo.selectedValue, $scope.srchTuClsCd2Combo.selectedValue);
   };
    ...
    $scope.excelUpload = function () {
        ...
        $scope._popConfirm(msg, function() {
            ...
            $("#kioskMClsExcelUpFile").val('');
            $("#kioskMClsExcelUpFile").trigger('click');
 ...
 */
app.controller('kioskMClsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('kioskMClsExcelCtrl', $scope, $http, true));
    ...
    // 양식 다운로드
    $scope.sampleDownload = function (tuClsType, tuClsCd) {

        var params = {};
        params.tuClsType = tuClsType;
        params.tuClsCd = tuClsCd;

        $scope._inquiryMain("/base/multilingual/kioskSideOption/getKioskMClsList.sb", params, function () {
            ...
            $timeout(function () {
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex,
                    ...
                    messages["kioskSideOption.kioskMCls"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
    ...
    $scope.excelFileChanged = function () {
        if ($('#kioskMClsExcelUpFile')[0].files[0]) {
            // 엑셀업로드 호출
            $scope.excelUpload();
        }
    };
    ...
    $scope.excelUpload = function () {
        ...
        // 선택한 파일이 있으면
        if ($('#kioskMClsExcelUpFile')[0].files[0]) {
            var file = $('#kioskMClsExcelUpFile')[0].files[0];
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/kioskSideOption/saveKioskMCls.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('kioskMClsCtrl').getKioskMClsList();
                }
            }
