/****************************************************************
 *
 * 파일명 : mobileSalePriceManage.js
 * 설  명 : 가격관리(매장) JavaScript
 *
 ...
 *  판매가관리(매장용) 그리드 생성
 */
app.controller('mobileSalePriceManageCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileSalePriceManageCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("mobileSalePriceManageCtrl", function(event, data) {
    ...
    $scope.searchSalePriceManage = function(){
        ...
        $scope._inquirySub('/base/price/salePriceManage/salePriceManage/getSalePriceManageList.sb', params, function() {
            ...
            var grid = wijmo.Control.getControl("#wjGridSalePriceManage");
            var rows = grid.rows;
