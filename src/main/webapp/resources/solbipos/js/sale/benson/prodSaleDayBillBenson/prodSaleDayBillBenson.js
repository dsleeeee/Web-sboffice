/****************************************************************
 *
 * 파일명 : prodSaleDayBillBenson.js
 * 설  명 : (벤슨) 간소화화면 > 상품매출일별(영수) JavaScript
 *
 ...
 *  상품매출일별(영수) 그리드 생성
 */
app.controller('prodSaleDayBillBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodSaleDayBillBensonCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("prodSaleDayBillBensonCtrl", function (event, data) {
        $scope.searchProdSaleDayBillBensonList();
        event.preventDefault();
    });

    $scope.searchProdSaleDayBillBensonList = function () {
        // 기간제한 체크
        ...
        params.storeCds = $("#prodSaleDayBillBensonStoreCd").val();
        params.prodCds = $("#prodSaleDayBillBensonSelectCd").val();
        ...
        $.postJSON("/sale/benson/prodSaleDayBillBenson/prodSaleDayBillBenson/getProdSaleDayBillBensonList.sb", params, function (response) {
            var grid = $scope.flex;
            ...
            if(length > 0){
                ...
                $http({
                    method : 'POST', //방식
                    url    : "/sale/benson/prodSaleDayBillBenson/prodSaleDayBillBenson/getDateDiff.sb", /* 통신할 URL */
                    ...
                    if ($scope._httpStatusCheck(response, true)) {
                        ...
                        grid.columns.push(new wijmo.grid.Column({
                            header: messages["prodSaleDayBillBenson.total"],
                            binding: "totBillCnt1",
                            width: 90,
                            align: "right",
                            isReadOnly: "true"
                        ...
                        // 첫째줄 헤더 생성
                        var dataItem = {};
                        dataItem.prodClassNm = messages["prodSaleDayBillBenson.prodClassNm"];
                        dataItem.prodNm = messages["prodSaleDayBillBenson.prodNm"];
                        dataItem.storeCd = messages["prodSaleDayBillBenson.storeCd"];
                        dataItem.storeNm = messages["prodSaleDayBillBenson.storeNm"];
    ...
    $scope.prodSaleDayBillBensonSelectShow = function () {
        $scope._broadcast('prodSaleDayBillBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function (excelType) {
        ...
        if(params.excelType === '1') {
            ...
            $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
                $scope._broadcast('prodSaleDayBillBensonExcelCtrl', params);
            });
        }else{
            // 분할 엑셀다운로드 사용자 제한 체크
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "상품매출일별(영수)_" + wijmo.Globalize.format(startDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format(endDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('prodSaleDayBillBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodSaleDayBillBensonExcelCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("prodSaleDayBillBensonExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/benson/prodSaleDayBillBenson/prodSaleDayBillBenson/getProdSaleDayBillBensonExcelList.sb", params, function (response) {
                ...
                function delay(x){
                    return new Promise(function(resolve, reject){
                        ...
                        $.postJSON("/sale/benson/prodSaleDayStoreBenson/prodSaleDayStoreBenson/getDivisionExcelDownloadSaveUpdate.sb", params, function(response){
                            ...
                            $.postJSON("/sale/benson/prodSaleDayBillBenson/prodSaleDayBillBenson/getProdSaleDayBillBensonExcelDivisionList.sb", params, function(response){
                                ...
                                if(length > 0){
                                    ...
                                    // 기간선택 두 날짜 사이 모든날짜 구하기
                                    $.postJSON("/sale/benson/prodSaleDayBillBenson/prodSaleDayBillBenson/getDateDiff.sb", params, function(response) {
