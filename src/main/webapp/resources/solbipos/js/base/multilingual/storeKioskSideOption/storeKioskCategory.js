/****************************************************************
 *
 * 파일명 : storeKioskCategory.js
 * 설  명 : 다국어관리(키오스크/사이드/옵션)(매장) - 키오스크(카테고리) 탭 JavaScript
 *
 ...
 * 키오스크(카테고리) 조회 그리드 생성
 */
app.controller('storeKioskCategoryCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeKioskCategoryCtrl', $scope, $http, true));
    ...
    $scope.$on("storeKioskCategoryCtrl", function (event, data) {
    ...
    $scope.setTuClsDropdownList = function () {
        ...
        var params = {};
        params.posNo = $scope.srchPosNoCombo.selectedValue;
        ...
        $http({
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (!$.isEmptyObject(response.data.data.list) && response.data.data.list.length > 0) {
                    ...
                    $scope._setComboData("tuClsType", [{name: "전체", value: ""}].concat(comboArray));
                    //$scope.tuClsTypeDataMap = new wijmo.grid.DataMap(comboArray, 'value', 'name');

                } else {
                    var comboArrayAll = [];
                    comboArrayAll.unshift({name: "전체", value: ""});
                    $scope._setComboData("tuClsType", comboArrayAll);
                }
            }
    ...
    $scope.getKioskCategoryList = function () {
        ...
        // 키오스크(카테고리) 탭 리스트 조회
        $scope._inquirySub("/base/multilingual/storeKioskSideOption/getStoreKioskCategoryList.sb", params);
    };
    ...
    $scope.saveRow = function(){
        $scope._popConfirm(messages["cmm.choo.save"], function () {
            ...
            $scope._save("/base/multilingual/storeKioskSideOption/saveStoreKioskCategory.sb", params, function (result) {
                // 재조회
                $scope.getKioskCategoryList();
            });
        });
    ...
    $scope.sampleDownload = function () {
        ...
        var vScope = agrid.getScope('storeKioskCategoryExcelCtrl');
        vScope.sampleDownload($scope.srchPosNoCombo.selectedValue, $scope.srchTuClsTypeCombo.selectedValue);
    };
 ...
 */
app.controller('storeKioskCategoryExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeKioskCategoryExcelCtrl', $scope, $http, false));
    ...
    // 양식 다운로드
    $scope.sampleDownload = function (posNo, tuClsType) {

        var params = {};
        params.posNo = posNo;
        params.tuClsType = tuClsType;

        $scope._inquiryMain("/base/multilingual/storeKioskSideOption/getStoreKioskCategoryList.sb", params, function (){
            ...
            $timeout(function()	{
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(	$scope.flex,
                    ...
                    messages["storeKioskSideOption.kioskCategory"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            ...
                            renameKey(item, '카테고리명(영문)', 'tuClsEnNm');
                            renameKey(item, '카테고리명(중문)', 'tuClsCnNm');
                            renameKey(item, '카테고리명(일문)', 'tuClsJpNm');
                        });
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/storeKioskSideOption/saveStoreKioskCategory.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('storeKioskCategoryCtrl').getKioskCategoryList();
                }
            }
        }, function errorCallback(response) {
