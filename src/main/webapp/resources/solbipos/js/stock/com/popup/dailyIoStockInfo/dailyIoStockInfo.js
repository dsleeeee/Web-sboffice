...
//			case "11" : {arrBinding=["saleDate", "dlvrPackFg"]; break;} // 매장판매(영업일자, 배달포장구분)
  ...
  $scope.drawGrid = function(grid, arrBinding, arrWidth, arrAlign, arrSum){
	  for(var i=0; i<arrBinding.length; i++){
		  ...
		  if(aggregate == ""){
			  grid.columns.push(new wijmo.grid.Column({header: messages["dailyIostock." + arrBinding[i]], binding: arrBinding[i], width: arrWidth[i] , align: arrAlign[i], isReadOnly: "true"}));
		  }else{
