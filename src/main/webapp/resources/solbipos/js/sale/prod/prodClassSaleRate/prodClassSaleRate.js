...
app.controller('prodClassSaleRateCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("prodClassSaleRateCtrl");
  ...
  $scope.$on("prodClassSaleRateCtrl", function (event, data) {
    $scope.searchprodClassSaleRateList();
    // 기능수행 종료 : 반드시 추가
  ...
  $scope.searchprodClassSaleRateList = function () {
    ...
    $scope._inquiryMain("/sale/prod/prodClassSaleRate/prodClassSaleRate/getProdClassSaleRateList.sb", params);
  };

  // 매장선택 모듈 팝업 사용시 정의
  // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
  // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
  $scope.prodClassSaleRateStoreShow = function () {
    $scope._broadcast('prodClassSaleRateStoreCtrl');
  };
...
app.controller('prodClassSaleRateExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('prodClassSaleRateExcelCtrl', $scope, $http, true));
  ...
  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("prodClassSaleRateExcelCtrl", function (event, data) {
  ...
  $scope.searchExcelList = function (data) {
    ...
    $scope._inquiryMain("/sale/prod/prodClassSaleRate/prodClassSaleRate/getProdClassSaleRateExcelList.sb", params, function() {
      ...
      $timeout(function () {
        ...
        }, messages["prodClassSaleRateMoms.prodClassSaleRateMoms"]+getToday()+'.xlsx', function () {
          $timeout(function () {
