...
public class MonthProdServiceImpl implements MonthProdService {
    ...
    public MonthProdServiceImpl(MonthProdMapper monthProdMapper, PopupMapper popupMapper) {
    ...
    public List<DefaultMap<Object>> getMonthProdList(MonthProdVO monthProdVO, SessionInfoVO sessionInfoVO) {
        ...
        // 매장 array 값 세팅
        if(!StringUtil.getOrBlank(monthProdVO.getStoreCds()).equals("")) {
        ...
        // 상품 array 값 세팅
        if(!StringUtil.getOrBlank(monthProdVO.getProdCds()).equals("")) {
            ProdVO prodVO = new ProdVO();
            prodVO.setArrSplitProdCd(CmmUtil.splitText(monthProdVO.getProdCds(), 3900));
            monthProdVO.setProdCdQuery(popupMapper.getSearchMultiProdRtn(prodVO));
        }
