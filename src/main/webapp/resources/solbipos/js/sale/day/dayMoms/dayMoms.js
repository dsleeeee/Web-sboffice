...
app.controller('dayMomsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.excelDownload = function () {
    ...
    $scope._broadcast('dayMomsExcelCtrl',params);
  }
}]);


app.controller('dayMomsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('dayMomsExcelCtrl', $scope, $http, true));
  ...
  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("dayMomsExcelCtrl", function (event, data) {
  ...
  $scope.searchExcelList = function (data) {
    ...
    // 조회 수행 : 조회URL, 파라미터, 콜백함수
    $scope._inquiryMain("/sale/day/dayMoms/dayMoms/getDayMomsExcelList.sb", params, function() {
      ...
      $timeout(function () {
        ...
        }, messages["dayMomsMoms.dayMomsMoms"]+getToday()+'.xlsx', function () {
          $timeout(function () {
