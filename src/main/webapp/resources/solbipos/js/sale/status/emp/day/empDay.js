...
app.controller('empDayCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.excelDownloadEmpDay = function () {
	    // 파라미터
	    var params     = {};
		$scope._broadcast('empDayExcelCtrl',params);
  };
...
/** 판매자별(일자별) 상세현황 controller */
app.controller('empDayExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
	// 상위 객체 상속 : T/F 는 picker
	angular.extend(this, new RootController('empDayExcelCtrl', $scope, $http, true));
  ...
  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("empDayExcelCtrl", function (event, data) {
  ...
  // 판매자일자별 리스트 조회
  $scope.searchEmpDayExcelList = function () {
    ...
    // 조회 수행 : 조회URL, 파라미터, 콜백함수
    $scope._inquiryMain("/sale/status/emp/day/excelList.sb", params, function() {
  ...
  $scope.getEmpNmList = function () {
	    ...
	    $scope._postJSONQuery.withOutPopUp("/sale/status/emp/day/empList.sb", params, function(response) {
	    	var length = response.data.data.list.length;
	   	 	var grid = wijmo.Control.getControl("#empDayExcelGrid");
