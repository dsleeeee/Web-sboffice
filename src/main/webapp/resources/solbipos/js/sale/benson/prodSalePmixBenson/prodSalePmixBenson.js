/****************************************************************
 *
 * 파일명 : prodSalePmixBenson.js
 * 설  명 : (벤슨) 간소화화면 > 상품매출(P.MIX) JavaScript
 *
 ...
 *  상품매출(P.MIX) 그리드 생성
 */
app.controller('prodSalePmixBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodSalePmixBensonCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("prodSalePmixBensonCtrl", function (event, data) {
        $scope.searchProdSalePmixBensonList();
        event.preventDefault();
    });

    $scope.searchProdSalePmixBensonList = function () {
        // 기간제한 체크
        ...
        params.storeCds = $("#prodSalePmixBensonStoreCd").val();
        params.prodCds = $("#prodSalePmixBensonSelectCd").val();
        ...
        $.postJSON("/sale/benson/prodSalePmixBenson/prodSalePmixBenson/getProdSalePmixBensonList.sb", params, function (response) {
            // <-- 그리드 생성 -->
            ...
            if(length > 0){
                ...
                $http({
                    method : 'POST', //방식
                    url    : "/sale/benson/prodSalePmixBenson/prodSalePmixBenson/getDateDiff.sb", /* 통신할 URL */
                    ...
                    if ($scope._httpStatusCheck(response, true)) {
                        ...
                        grid.columns.push(new wijmo.grid.Column({
                            header: messages["prodSalePmixBenson.total"],
                            binding: "totSaleQty1",
                            width: 90,
                            align: "right",
                            isReadOnly: "true"
                        ...
                        // 첫째줄 헤더 생성
                        var dataItem = {};
                        dataItem.prodClassNm = messages["prodSalePmixBenson.prodClassNm"];
                        dataItem.prodNm = messages["prodSalePmixBenson.prodNm"];
    ...
    $scope.prodSalePmixBensonSelectShow = function () {
        $scope._broadcast('prodSalePmixBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function (excelType) {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('prodSalePmixBensonExcelCtrl', params);
        });
    };
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "상품매출(P.MIX)_" + wijmo.Globalize.format(startDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format(endDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('prodSalePmixBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodSalePmixBensonExcelCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("prodSalePmixBensonExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/benson/prodSalePmixBenson/prodSalePmixBenson/getProdSalePmixBensonExcelList.sb", params, function (response) {
