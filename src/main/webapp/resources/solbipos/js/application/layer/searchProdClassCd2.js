 ...
 * 팝업 그리드 생성
 */
app.controller('prodClassPopUpCtrl2', ['$scope', '$http', function ($scope, $http) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('prodClassPopUpCtrl2', $scope, $http, false));
  // 상품분류 선택 정보
  $scope.selectedClass = "";
  ...
  $scope.initTreeView = function(tree) {
    // 트리데이터 조회
    $scope._broadcast('prodClassPopUpCtrl2');
  };
  // 상품분류 트리 조회
  $scope.$on('prodClassPopUpCtrl2', function (event, data) {
