/** 거래명세표 controller */
app.controller('transReportExcelDownCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('transReportExcelDownCtrl', $scope, $http, true));

  // 다른 컨트롤러의 broadcast 받기
  $scope.$on("transReportExcelDownCtrl", function (event, data) {
    $scope.slipFg     = data.slipFg;
    $scope.stmtAcctFg = data.stmtAcctFg;
    $scope.strSlipNo  = data.strSlipNo;

    //$scope.wjtransReportExcelDownLayer.show(true);

    // transReportExcelDown html 내용 초기화
    $("#transReportExcelDown").html('');
    ...
    $http({
      ...
      if ($scope._httpStatusCheck(response, true)) {
        if (!$.isEmptyObject(response.data.data)) {
          ...
          // 전표 내역 조회
          $scope.transReportExcelDownInfoList();
        }
      }
  ...
  // 전표 내역 조회
  $scope.transReportExcelDownInfoList = function () {
    var params    = {};
    params.slipNo = $scope.strSlipNo;
    params.slipFg = $scope.slipFg;
  ...
  $scope.excelDown = function(){
	  var params    = {};
	  //params.slipInfoList = $scope.slipInfoList;
	  params.slipNo = $scope.strSlipNo;
	  params.slipFg = $scope.slipFg;
	  ...
	  if(document.getElementsByName('sessionId')[0]){
	    ...
	    $http({
	      method : 'POST', //방식
	      url    : '/iostock/transReportExcelDownload/excelDownload/excelDownload/down.sb', /* 통신할 URL */
	      params : params, /* 파라메터로 보낼 데이터 */
	      headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
	    }).then(function successCallback(response) {
	      if ($scope._httpStatusCheck(response, true)) {
  ...
  $scope.reportRender = function () {
    ...
    for (var i = 0; i < slipCnt; i++) { // 전표수만큼 loop
      ...
      transReportExcelDownHtml += (i > 0 ? nextPageHtml : ''); // 프린트 출력시 다음 페이지로 넘기기 위한 html 을 붙여준다.
      ...
      for (var k = 0; k < stmtLoop; k++) { // 거래명세표 인쇄 구분에 따라 공급자 보관용과 공급받는자 보관용을 만들기 위해 loop 돈다.
        // 상품 리스트 HTML 생성
        transReportExcelDownHtml += (k > 0 ? nextPageHtml : ''); // 프린트 출력시 다음 페이지로 넘기기 위한 html 을 붙여준다.
        var prodListHtml       = '';
...
//                + '<td class="tl"></td>'
          ...
          transReportExcelDownHtml += (m > 0 ? nextPageHtml : '') + titleHtml + infoHtml + prodListHeaderHtml + prodListHtml + footerHtml;
          prodListHtml = '';
        } // 데이터 조회 후 한 페이지에 표시할 row 수에 따라 배열에 담아놓은 수만큼 페이지 생성하기 위해 loop END
      } // 거래명세표 인쇄 구분에 따라 공급자 보관용과 공급받는자 보관용을 만들기 위해 loop END
    } // 전표수만큼 loop END

    // console.log(transReportExcelDownHtml);
    $('#transReportExcelDown').append(transReportExcelDownHtml);
  ...
  $scope.print = function () {
    ...
    // add content to it
    var view = document.querySelector('#transReportExcelDown');
    doc.append(view);
