/****************************************************************
 *
 * 파일명 : timeProdSalePmixMoms.js
 * 설  명 : 맘스터치 > 간소화화면 > 시간대별 상품매출(P.MIX) JavaScript
 *
...
/**
 *  시간대별 상품매출(P.MIX) 그리드 생성
 */
app.controller('timeProdSalePmixMomsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeProdSalePmixMomsCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("timeProdSalePmixMomsCtrl", function (event, data) {
        $scope.searchTimeProdSalePmixMomsList();
        event.preventDefault();
    });

    $scope.searchTimeProdSalePmixMomsList = function () {
        // 기간제한 체크
        ...
        params.storeCds = $("#timeProdSalePmixMomsStoreCd").val();
        params.prodCds = $("#timeProdSalePmixMomsSelectCd").val();
        ...
        $scope._postJSONQuery.withPopUp("/sale/moms/timeProdSalePmixMoms/timeProdSalePmixMoms/getTimeProdSalePmixMomsList.sb", params, function (response) {
            // <-- 그리드 생성 -->
            ...
            if(length > 0){
                ...
                $http({
                    method : 'POST', //방식
                    url    : "/sale/moms/timeProdSalePmixMoms/timeProdSalePmixMoms/getDateDiff.sb", /* 통신할 URL */
                    ...
                    if ($scope._httpStatusCheck(response, true)) {
                        ...
                        grid.columns.push(new wijmo.grid.Column({
                            header: messages["timeProdSalePmixMoms.total"],
                            binding: "totSaleQty1",
                            width: 90,
                            align: "right",
                            isReadOnly: "true"
                        ...
                        // 첫째줄 헤더 생성
                        var dataItem = {};
                        dataItem.prodClassNm = messages["timeProdSalePmixMoms.prodClassNm"];
                        dataItem.prodNm = messages["timeProdSalePmixMoms.prodNm"];
                        ...
                        for (var i = 0; i < arrTimeCol.length; i++) {
                            if(i != 23){
                                eval('dataItem.timeSaleQty1' + arrTimeCol[i] + '= "' + arrTimeCol[i] + '시~' + arrTimeCol[i+1] + '시"');
                                eval('dataItem.timeRealSaleAmt1' + arrTimeCol[i] + '= "' + arrTimeCol[i] + '시~' + arrTimeCol[i+1] + '시"');
                                eval('dataItem.timePMixSale1' + arrTimeCol[i] + '= "' + arrTimeCol[i] + '시~' + arrTimeCol[i+1] + '시"');
                            } else {
                                eval('dataItem.timeSaleQty1' + arrTimeCol[i] + '= "' + arrTimeCol[i] + '시~24시"');
                                eval('dataItem.timeRealSaleAmt1' + arrTimeCol[i] + '= "' + arrTimeCol[i] + '시~24시"');
                                eval('dataItem.timePMixSale1' + arrTimeCol[i] + '= "' + arrTimeCol[i] + '시~24시"');
                            }
                        }
    ...
    $scope.timeProdSalePmixMomsSelectShow = function () {
        $scope._broadcast('timeProdSalePmixMomsSelectCtrl');
    };
    ...
    $scope.excelDownload = function (excelType) {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('timeProdSalePmixMomsExcelCtrl', params);
        });
    };
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "시간대별 상품매출(P.MIX)_" + wijmo.Globalize.format(startDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format(endDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('timeProdSalePmixMomsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeProdSalePmixMomsExcelCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("timeProdSalePmixMomsExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._postJSONQuery.withPopUp("/sale/moms/timeProdSalePmixMoms/timeProdSalePmixMoms/getTimeProdSalePmixMomsExcelList.sb", params, function (response) {
                ...
                $http({
                    ...
                    if ($scope._httpStatusCheck(response, true)) {
                        ...
                        $timeout(function () {
                            ...
                            }, "시간대별 상품매출(P.MIX)_" + params.startDate + "_" + params.endDate + "_" + getCurDateTime()+'.xlsx', function () {
                                $timeout(function () {
                                    $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
                                }, 10);
                            });
