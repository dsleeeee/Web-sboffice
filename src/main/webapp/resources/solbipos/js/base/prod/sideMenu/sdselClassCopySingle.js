...
app.controller('sdselClassCopySingleCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.classSingleCopySave = function(){
        ...
        sysLog("[분류복사S] 1. classSingleCopySave 진입(확인전) nonce=" + _reqNonce);
    ...
    $scope.sdselClassSingleCopySave = function(orgParams){
        ...
        var pollCopyStatus = function (cnt) {
            if (cnt >= POLL_MAX_CNT) {
                sysLog("[분류복사S] 9-P. 폴링 상한 초과 nonce=" + _reqNonce);
                $scope.excelUploadingPopup(false);
            ...
            setTimeout(function () {
                $http({
                    ...
                    sysLog("[분류복사S] 9-P. 폴링 " + (cnt + 1) + "회 status=" + st);
                    if (st === 'DONE') {
        ...
        $http({
            ...
            sysLog("[분류복사S] 9. 저장 콜백 진입 status=" + (response.data && response.data.status) + ((response.data && response.data.message) ? " / msg=" + response.data.message : ""));
            // 자동 재전송이 차단된 응답 → 팝업 없이 로딩 유지 + 원본 처리상태 폴링으로 전환
            if (response.data && response.data.data === 'RESEND_BLOCKED') {
                sysLog("[분류복사S] 9-P. 재전송 차단응답 수신 → 상태폴링 전환 nonce=" + _reqNonce);
                pollCopyStatus(0);
                return;
            }
            if ($scope._httpStatusCheck(response, true)) {
            ...
            sysLog("[분류복사S] 9-1. 저장 에러콜백 진입 httpStatus=" + response.status + ((response.data && response.data.message) ? " / msg=" + response.data.message : ""));
            $scope.excelUploadingPopup(false); // 작업내역 로딩 팝업 닫기
            if (response.data.message) {
                $scope._popMsg(response.data.message);
            } else {
