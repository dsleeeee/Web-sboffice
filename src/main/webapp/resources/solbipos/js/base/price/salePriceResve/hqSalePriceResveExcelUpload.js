...
app.controller('hqSalePriceResveExcelUploadCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
        ...
        s.cellEditEnded.addHandler(function (s, e) {
            if (e.panel === s.cells) {
                ...
                if (col.binding === "saleUprc") {
                    var vScope = agrid.getScope("hqSalePriceResveExcelUploadSampleCtrl");
    ...
    $scope.save2 = function(orgParams) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/price/salePriceResve/salePriceResveExcelUpload/getHqSalePriceResveExcelUploadSave.sb', /* 통신할 URL */
            data   : params, /* 파라메터로 보낼 데이터 : @requestBody */
            params : sParam,
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
