/****************************************************************
 *
 * 파일명 : storePayMonthBenson.js
 * 설  명 : (벤슨) 결제수단매출 > 매장-월별결제수단매출 JavaScript
 *
 ...
 *  매장-월별결제수단매출 그리드 생성
 */
app.controller('storePayMonthBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storePayMonthBensonCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("storePayMonthBensonCtrl");
        ...
        dataItem.branchCd    = messages["storePayMonthBenson.branchCd"];
        dataItem.branchNm    = messages["storePayMonthBenson.branchNm"];
    ...
    // <-- 검색 호출 -->
    $scope.$on("storePayMonthBensonCtrl", function(event, data) {
    ...
    $scope.searchMonthTotal = function() {
        ...
        if($scope.option === "store"){
            params.storeCds   = $("#storePayMonthBensonStoreCd").val();
        } else {
            params.storeCds = '';
        }
        ...
        $scope._inquiryMain("/sale/benson/storePayMonthBenson/storePayMonthBenson/getStorePayMonthBensonList.sb", params, function (){
    ...
    $scope.changeOption = function (s){
        if(s.selectedValue === "branch"){
            $(".storePayMonthBenson").hide();
        } else if(s.selectedValue === "store"){
            $(".storePayMonthBenson").show();
        }
    };
...
// 엑셀 다운로드
    $scope.excelDownloadInfo = function () {
        ...
        $scope._broadcast('storePayMonthBensonExcelCtrl',params);
    };
}]);
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('storePayMonthBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storePayMonthBensonExcelCtrl', $scope, $http, true));
    ...
    $scope.$on("storePayMonthBensonExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/benson/storePayMonthBenson/storePayMonthBenson/getStorePayMonthBensonExcelList.sb", params, function(response) {
            ...
            $timeout(function () {
                ...
                }, messages["storePayMonthBenson.storePayMonthBenson"] + '_' + params.startMonth + '_' + params.endMonth + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
