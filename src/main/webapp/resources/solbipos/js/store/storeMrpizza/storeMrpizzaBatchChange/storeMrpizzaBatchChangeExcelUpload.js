/****************************************************************
 *
 * 파일명 : storeMrpizzaBatchChangeExcelUpload.js
 ...
 *  매장목록 샘플양식 조회 그리드 생성
 */
app.controller('storeMrpizzaBatchChangeExcelUploadCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeMrpizzaBatchChangeExcelUploadCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("storeMrpizzaBatchChangeExcelUploadCtrl", function(event, data) {
    ...
    $scope.sampleDownload = function(){
        ...
        $scope._inquiryMain("/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getStoreList.sb", params, function (){
...
app.controller('storeExcelUploadCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.searchStoreExcelUploadProd = function() {
        ...
        $scope._inquiryMain("/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getStoreExcelUploadCheckList.sb", params, function() {}, false);
    };
    // <-- //검색 호출 -->
    ...
    $scope.save = function() {
        ...
        for (var i = 0; i < $scope.flex.collectionView.items.length; i++) {
            if($scope.flex.collectionView.items[i].result !== "검증성공") {
                $scope._popMsg(messages["storeMrpizzaBatchChange.noSaveConfirm"] + (i+1) + messages["storeMrpizzaBatchChange.noSaveConfirm2"]); // 검증성공이 아닌 데이터가 있습니다. <br/> 다시 업로드 해주세요. <br/> 3번째 줄
                return false;
            }
            ...
            if($scope.flex.collectionView.items[i].momsTeam !== $scope.flex.collectionView.items[i].oldMomsTeam
        ...
        if($scope.flex.collectionView.itemsEdited.length > 0) {
            $scope._popMsg(messages["storeMrpizzaBatchChange.noSaveConfirm3"]); // 수정된 내역이 있습니다. <br/> 엑셀다운로드 후, 다시 엑셀업로드 해주세요.
            return false;
        }

        // 검증을 통과한 매장정보를 저장하시겠습니까?
        $scope._popConfirm(messages["storeMrpizzaBatchChange.saveConfirm"], function() {
    ...
    $scope.deleteExl = function (data) {
        ...
        $scope._postJSONSave.withOutPopUp("/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getStoreExcelUploadCheckDeleteAll.sb", params, function(){
            // 변경된 값만 저장
    ...
    $scope.getDiffValSave = function(data) {
        ...
        // 저장기능 수행 : 저장URL, 파라미터, 콜백함수
        $scope._postJSONSave.withOutPopUp("/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getDiffValSave.sb", params, function(){
            // 매장등록 저장
    ...
    $scope.storeExcelUploadSave = function(data) {
        ...
        $scope._postJSONQuery.withOutPopUp("/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getTmpStoreList.sb", params, function(response){
            ...
            $scope._postJSONQuery.withOutPopUp("/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getTmpStoreList.sb", params, function(response){
                ...
                function delay(x){
                    return new Promise(function(resolve, reject){
                        ...
                        $http({
                            method: 'POST', //방식
                            url: '/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getSimpleSave.sb', /* 통신할 URL */
    ...
    $scope.deleteSave = function() {
        ...
        // 저장기능 수행 : 저장URL, 파라미터, 콜백함수
        $scope._postJSONSave.withOutPopUp("/store/storeMrpizza/storeMrpizzaBatchChange/storeMrpizzaBatchChange/getStoreExcelUploadCheckDelete.sb", params, function(){
            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
        });
    };
    // <-- //그리드 행 삭제 -->
