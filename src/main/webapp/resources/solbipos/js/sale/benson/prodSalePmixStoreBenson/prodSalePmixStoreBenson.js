/****************************************************************
 *
 * 파일명 : prodSalePmixStoreBenson.js
 * 설  명 : (벤슨) 간소화화면 > 상품매출(P.MIX 매장) JavaScript
 *
 ...
 */
app.controller('prodSalePmixStoreBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodSalePmixStoreBensonCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("prodSalePmixStoreBensonCtrl", function (event, data) {
        $scope.searchProdSalePmixStoreBensonList();
        event.preventDefault();
    });

    $scope.searchProdSalePmixStoreBensonList = function () {
        // 기간제한 체크
        ...
        params.storeCds = $("#prodSalePmixStoreBensonStoreCd").val();
        params.prodCds = $("#prodSalePmixStoreBensonSelectCd").val();
        ...
        $.postJSON("/sale/benson/prodSalePmixStoreBenson/prodSalePmixStoreBenson/getProdSalePmixStoreBensonList.sb", params, function (response) {
            // <-- 그리드 생성 -->
            ...
            if(length > 0){
                ...
                $http({
                    method : 'POST', //방식
                    url    : "/sale/benson/prodSalePmixStoreBenson/prodSalePmixStoreBenson/getDateDiff.sb", /* 통신할 URL */
                    ...
                    if ($scope._httpStatusCheck(response, true)) {
                        ...
                        grid.columns.push(new wijmo.grid.Column({
                            header: messages["prodSalePmixStoreBenson.total"],
                            binding: "totSaleQty1",
                            width: 90,
                            align: "right",
                            isReadOnly: "true"
                        ...
                        // 첫째줄 헤더 생성
                        var dataItem = {};
                        dataItem.storeCd = messages["prodSalePmixStoreBenson.storeCd"];
                        dataItem.storeNm = messages["prodSalePmixStoreBenson.storeNm"];
                        dataItem.prodClassNm = messages["prodSalePmixStoreBenson.prodClassNm"];
                        dataItem.prodNm = messages["prodSalePmixStoreBenson.prodNm"];
    ...
    $scope.prodSalePmixStoreBensonSelectShow = function () {
        $scope._broadcast('prodSalePmixStoreBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function (excelType) {
        ...
        if(params.excelType === '1') {
            ...
            $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
                $scope._broadcast('prodSalePmixStoreBensonExcelCtrl', params);
            });
        }else{
            // 분할 엑셀다운로드 사용자 제한 체크
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('prodSalePmixStoreBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodSalePmixStoreBensonExcelCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("prodSalePmixStoreBensonExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
            ...
            if (data.dayGubun === "day") {
                data.downloadNo = "4-1"; // 다운로드 화면구분번호
            } else if (data.dayGubun === "month") {
                data.downloadNo = "4-2"; // 다운로드 화면구분번호
            }
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._postJSONQuery.withPopUp("/sale/benson/prodSalePmixStoreBenson/prodSalePmixStoreBenson/getProdSalePmixStoreBensonExcelList.sb", params, function (response) {
                ...
                function delay(x){
                    return new Promise(function(resolve, reject){
                        ...
                        $.postJSON("/sale/benson/prodSaleDayStoreBenson/prodSaleDayStoreBenson/getDivisionExcelDownloadSaveUpdate.sb", params, function(response){
                            ...
                            $.postJSON("/sale/benson/prodSalePmixStoreBenson/prodSalePmixStoreBenson/getProdSalePmixStoreBensonExcelDivisionList.sb", params, function(response){
                                var list = response.data.list;
                                if (list.length === undefined || list.length === 0) {
                                    $scope.data = new wijmo.collections.CollectionView([]);
                                    $scope.excelUploadingPopup(false);
                                ...
                                var grid = wijmo.Control.getControl("#wjGridExcelList");
                                    ...
                                    if(length > 0){
                                        ...
                                        // 기간선택 두 날짜 사이 모든날짜 구하기
                                        $.postJSON("/sale/benson/prodSalePmixStoreBenson/prodSalePmixStoreBenson/getDateDiff.sb", params, function(response) {
