/****************************************************************
 *
 * 파일명 : envConfgBatchChangeStore.js
 * 설  명 : 환경변수일괄변경 > 매장환경탭 JavaScript
 *
 ...
 * 매장환경탭 그리드 생성
 */
app.controller('envConfgBatchChangeStoreCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('envConfgBatchChangeStoreCtrl', $scope, $http, true));
    ...
    // <-- 검색 호출 -->
    $scope.$on("envConfgBatchChangeStoreCtrl", function(event, data) {
        $scope.searchEnvConfgBatchChangeStore();
        event.preventDefault();
    });

    $scope.searchEnvConfgBatchChangeStore = function() {
        // 시스템패스워드 비우기
        $scope.systemPw = "";

        if($scope.envstCd === "" || $scope.envstCd === undefined) {
            $scope._popMsg(messages["envConfgBatchChange.store.envstCdBlank"]); // 환경설정을 선택해주세요.
            return false;
        }
        ...
        if($scope.targtFg === "Z") {
            // 는 단독매장만 검색가능합니다.
            var msg = "'" + $scope.envstNm + "'" +messages["envConfgBatchChange.store.targtFgAlert"];
            $scope._popMsg(msg);
        }
        ...
        $scope._inquiryMain("/store/manage/envConfgBatchChange/envConfgBatchChangeStore/getEnvConfgBatchChangeStoreList.sb", params, function() {}, false);

        // 선택한 시간대에 따른 리스트 항목 visible
        var grid = wijmo.Control.getControl("#wjGridEnvConfgBatchChangeStoreList");
    ...
    // 환경설정 조회 팝업
    $scope.popUpEnvstCdStore = function() {
        // 환경설정 선택취소
        $scope.delEnvstCdStore();
        ...
        popup.show(true, function (s) {
            ...
            popScope.$apply(function () {
                ...
                if (!$.isEmptyObject(popScope.getSelectedEnvConfg())) {
                    ...
                    // 선택한 환결설정 값 보여주기
                    $("#lblEnvstCdStore").text("[ " + $scope.envstCd + " ] ");
                    $("#lblEnvstNmStore").text($scope.envstNm);

                    // 환경변수값 콤보박스
                    $scope.envstCdComboStore();

                    // 시스템패스워드 비우기
                    $scope.systemPw = "";

                    // 그리드 지우기
                    var agencyScope1 = agrid.getScope('envConfgBatchChangeStoreCtrl');
    ...
    // 환경변수값 콤보박스
    $scope.envstCdComboStore = function() {
        if($scope.dirctInYn === "N") {
            // 환경변수값
            $("#divComboEnvstValStore").css('display', '');
            $("#divTextEnvstValStore").css('display', 'none');
            ...
            // 환경변수값
            $("#divComboEnvstValStore").css('display', 'none');
            $("#divTextEnvstValStore").css('display', '');
            $scope._setComboData("envstValCombo", envstValComboData); // 환경변수값
            $scope.envstValCdDataMap = new wijmo.grid.DataMap(envstValComboData, 'value', 'name'); // 환경설정값코드
            $scope.envstVal = ""; // 텍스트용 환경변수값
        }
    ...
    // 환경설정 선택취소
    $scope.delEnvstCdStore = function(){
        $scope.envstCd = "";
        $scope.envstNm = "";

        // 선택한 환결설정 값 보여주기
        $("#lblEnvstCdStore").text("");
        $("#lblEnvstNmStore").text("");
    ...
    // 일괄변경
    $scope.batchChangeStore = function() {
        // 시스템패스워드 비우기
        $scope.systemPw = "";
    ...
    $scope.saveStore = function() {
        ...
        // H:본사전용, C:공통(매장에서변경가능), N:No, S:매장전용, X:공통(매장에서사용하지만_본사기준), Z:본사+단독매장
        if($scope.targtFg === "X") {
            // 는 본사기준(매장변경 불가값)입니다. <br/> 임의로 변경하시겠습니까?
            msg = "'" + $scope.envstNm + "'" +messages["envConfgBatchChange.store.targtFgConfirm"];
        }
        ...
        $scope._popConfirm(msg, function() {
            ...
            $scope._save("/store/manage/envConfgBatchChange/envConfgBatchChangeStore/getEnvConfgBatchChangeStoreSave.sb", params, function(){ $scope.allSearch(); });
        });
    };
    ...
    $scope.excelDownload = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "환경변수일괄변경_" + messages['envConfgBatchChange.store'] + '.xlsx', function () {
                    $timeout(function () {
