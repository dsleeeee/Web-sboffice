/****************************************************************
 *
 * 파일명 : basicSideMenuSelectMenu.js
 * 설  명 : (기준)사이드메뉴 > 선택메뉴(세트) Tab JavaScript
 *
...
/**
 * (기준)사이드메뉴 > 선택메뉴(세트) Tab 생성
 */
app.controller('basicSideMenuSelectGroupCtrl', ['$scope', '$http', function ($scope, $http) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('basicSideMenuSelectGroupCtrl', $scope, $http, false));
            ...
            if (ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if (col.binding === 'sdselGrpCd') {
                    if (selectedRow.sdselGrpCd !== '' && selectedRow.sdselGrpCd !== undefined && selectedRow.sdselGrpCd !== '자동채번') {
                        ...
                        $scope._broadcast('basicSideMenuSelectClassCtrl', selectedRow);
                        var prodGrid = agrid.getScope('basicSideMenuSelectProdCtrl');
    ...
    // <-- 검색 호출 -->
    $scope.$on('basicSideMenuSelectGroupCtrl', function (event, data) {
        // 초기 버튼 셋팅
        // 선택분류 버튼
        $("#btnUpSelClass").hide();
        $("#btnDownSelClass").hide();
    ...
    $scope.srchSelGroup = function () {
        ...
        $scope._inquiryMain('/base/prod/basicSideMenu/menuGrp/list.sb', params, function () {
            // <-- 그리드 visible -->
    ...
    $scope.deleteRow = function () {
        $scope._popConfirm(messages["cmm.choo.delete"], function () {
                    ...
                    if (item.sdselGrpCd === "자동채번" || item.sdselGrpCd === null || item.sdselGrpCd === undefined || item.sdselGrpCd === "") {
                        ...
                        if ((orgnFg == "HQ") || (orgnFg == "STORE" && hqOfficeCd == '00000') || (orgnFg == "STORE" && hqOfficeCd != "00000" && item.sdselGrpCd > 799999)) {
                            if (item.cnt === 0 || item.cnt === null || item.cnt === undefined || item.cnt === "") {
                                ...
                                $scope._popMsg(messages["basicSideMenu.selectMenu.sdselClass.notNull"]);
                                return false;
                            }
                        } else {
                            $scope._popMsg(messages["basicSideMenu.selectMenu.edited"]);
                            return false;
                        }
                    }
                }
            ...
            $scope._save('/base/prod/basicSideMenu/menuGrp/save.sb', params, function () {
                $("#sideSelectGroupTitle").html("");
                var attrScope = agrid.getScope('basicSideMenuSelectClassCtrl');
        ...
        $scope._popConfirm(messages["cmm.choo.save"], function () {
            ...
            for (var u = 0; u < $scope.flex.collectionView.itemsEdited.length; u++) {
                if ((orgnFg == "HQ") || (orgnFg == "STORE" && hqOfficeCd == "00000") || (orgnFg == "STORE" && hqOfficeCd != "00000" && $scope.flex.collectionView.itemsEdited[u].sdselGrpCd > 799999)) {
                    if ($scope.flex.collectionView.itemsEdited[u].sdselGrpNm == "") {
                        $scope._popMsg(messages["basicSideMenu.selectMenu.sdselGrpNm"] + messages["basicSideMenu.selectMenu.inputEnv"]);
                        return false;
                    }
            ...
            for (var i = 0; i < $scope.flex.collectionView.itemsAdded.length; i++) {
                if ($scope.flex.collectionView.itemsAdded[i].sdselGrpNm == "") {
                    $scope._popMsg(messages["basicSideMenu.selectMenu.sdselGrpCd"] + messages["basicSideMenu.selectMenu.inputEnv"]);
                    return false;
                }
...
/**
 * (기준)사이드메뉴 선택분류 그리드 생성
 */
app.controller('basicSideMenuSelectClassCtrl', ['$scope', '$http', 'sdselGrpCd', function ($scope, $http, sdselGrpCd) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('basicSideMenuSelectClassCtrl', $scope, $http, false));
            ...
            if (ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if (col.binding === 'sdselClassCd') {
                    if (selectedRow.sdselClassCd !== '' && selectedRow.sdselClassCd !== undefined && selectedRow.sdselClassCd !== '자동채번') {
                        ...
                        $scope._broadcast('basicSideMenuSelectProdCtrl', params);
                    }
                }
            }
        });
    ...
    // 선택분류 그리드 조회
    $scope.$on('basicSideMenuSelectClassCtrl', function (event, data) {
        // scope 영역에 변수 Set
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수, 팝업결과표시여부
        $scope._inquiryMain('/base/prod/basicSideMenu/menuClass/list.sb', params, function () {
            // <-- 그리드 visible -->
            // 선택한 테이블에 따른 리스트 항목 visible
    ...
    $scope.addRow = function () {
        if ($("#sideSelectGroupTitle").html() == "") {
            $scope._popMsg(messages["basicSideMenu.selectMenu.sdselGrp.null"]);
            return false;
        }
        // 파라미터 설정
        var params = {};
    ...
    $scope.deleteRow = function () {
        $scope._popConfirm(messages["cmm.choo.delete"], function () {
            ...
            $scope._save('/base/prod/basicSideMenu/menuClass/save.sb', params, function () {
    ...
    $scope.saveClass = function () {
        ...
        /*if (orgnFg == 'HQ' && hqOfficeCd == 'DS021') {
            msg = messages["basicSideMenu.selectMenu.sdselClassRegStoreAlert"] + " " + messages["cmm.choo.save"]; // 적용매장구분 변경된 경우 등록된 적용매장은 모두 삭제됩니다. 저장하시겠습니까?
        }*/
        ...
        $scope._popConfirm(msg, function () {
            ...
            for (var u = 0; u < $scope.flex.collectionView.itemsEdited.length; u++) {
                if ($scope.flex.collectionView.itemsEdited[u].sdselClassNm == "") {
                    $scope._popMsg(messages["basicSideMenu.selectMenu.sdselClassNm"] + messages["basicSideMenu.selectMenu.inputEnv"]);
                    return false;
                }
                ...
                if (nvl($scope.flex.collectionView.itemsEdited[u].sdselClassNm, '') !== '' &&
                    ...
                    var msg = messages["basicSideMenu.selectMenu.sdselClassNm"] + messages["cmm.overLength"] + " 50 " +
                ...
                if (nvl($scope.flex.collectionView.itemsEdited[u].fixProdCnt, 0) > nvl($scope.flex.collectionView.itemsEdited[u].sdselQty, 0)) {
                    $scope._popMsg("'" + $scope.flex.collectionView.itemsEdited[u].sdselClassNm + "[" + $scope.flex.collectionView.itemsEdited[u].sdselClassCd + "]'"
                        + messages["basicSideMenu.selectMenu.sdselQtyChk.msg"]
                ...
                if ($scope.flex.collectionView.itemsEdited[u].requireYn === "Y") {
                    if (parseInt(nvl($scope.flex.collectionView.itemsEdited[u].sdselQty, 0)) >= 1) {
                        ...
                        $scope._popMsg(messages["basicSideMenu.selectMenu.requireYnQtyChk.msg"]); // 필수선택시 수량은 1 이상 입력해주세요.
                        return false;
                    }
                }
            ...
            /*if (orgnFg == 'HQ') {
                // 적용매장 전체 삭제
                $scope._postJSONSave.withOutPopUp("/base/prod/basicSideMenu/menuClass/getSdselClassRegStoreDeleteAll.sb", params, function () {
                    // 저장
                    $scope.save(params);
                });
            } else {
    ...
    $scope.sdselClassCopy = function () {
        ...
        $scope.wjBasicSdselClassCopyLayer.show(true);
        event.preventDefault();
    };
...
/**
 * (기준)사이드메뉴 선택상품 그리드 생성
 */
app.controller('basicSideMenuSelectProdCtrl', ['$scope', '$http', 'sdselClassCd', function ($scope, $http, sdselClassCd) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('basicSideMenuSelectProdCtrl', $scope, $http, false));
    ...
    // 선택상품 그리드 조회
    $scope.$on('basicSideMenuSelectProdCtrl', function (event, data) {
    ...
    $scope.addRow = function () {
        if ($("#sideClassTitle").html() == "") {
            $scope._popMsg(messages["basicSideMenu.selectMenu.sdselClass.null"]);
            return false;
        }
        ...
        $scope._broadcast('basicSideMenuProdCtrl', params);
    ...
    $scope.deleteRow = function () {
        $scope._popConfirm(messages["cmm.choo.delete"], function () {
            ...
            // 삭제기능 수행 : 저장URL, 파라미터, 콜백함수
            $scope._save('/base/prod/basicSideMenu/menuProd/save.sb', params, function () {
    ...
    $scope.saveProd = function () {
        ...
        $scope._popConfirm(msg, function () {
            ...
            if (chkFixProdCnt > parseInt($scope.sdselQty)) {
                $scope._popMsg(messages["basicSideMenu.selectMenu.fixProdCntChk.msg"] + "<br/> (고정상품수량합계 : " + chkFixProdCnt + " / 선택분류수량 : " + $scope.sdselQty + ")"); // 구분이 '고정'인 상품의 수량합이 선택분류의 수량보다 클 수 없습니다.
                return false;
            }
            ...
            /*if (orgnFg == 'HQ') {
                // 적용매장 전체 삭제
                $scope._postJSONSave.withOutPopUp("/base/prod/basicSideMenu/menuProd/getSdselProdRegStoreDeleteAll.sb", params, function () {
                    // 저장
                    $scope.save(params);
                });
            } else {
    ...
    $scope.selectProdView = function (type) {
        var popUp = $scope.basicSideMenuProdLayer;
        setTimeout(function () {
            popUp.show(true, function (s) {
                // 수정 버튼 눌렀을때만
                if (s.dialogResult === "wj-hide-apply") {
                    var scope = agrid.getScope('basicSideMenuProdCtrl', $scope.getSdselClassCd());
