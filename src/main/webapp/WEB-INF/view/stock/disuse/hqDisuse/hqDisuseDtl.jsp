...
<c:set var="baseUrl" value="/stock/disuse/hqDisuse/hqDisuseDtl/"/>

<wj-popup id="wjHqDisuseDtlLayer" control="wjHqDisuseDtlLayer" show-trigger="Click" hide-trigger="Click" style="display:none;width:900px;">
  <div id="hqDisuseDtlLayer" class="wj-dialog wj-dialog-columns" ng-controller="hqDisuseDtlCtrl">
    ...
    <div class="wj-dialog-body sc2" style="height: 600px;">
      ...
      <table class="tblType01" style="position: relative;">
        ...
        <tbody>
        <tr>
          <th><s:message code="hqDisuse.dtl.disuseTitle"/><em class="imp">*</em></th>
          <td colspan="3">
            <input type="text" id="dtlAdjTitle" name="dtlAdjTitle" ng-model="disuseTitle" class="sb-input w100" maxlength="33"/>
          </td>
        </tr>
        </tbody>
      </table>
      ...
      <ul class="txtSty3 mt10">
        <li class="red"><s:message code="hqDisuse.dtl.txt1"/></li>
      </ul>

      <div class="mt20 tr">
        <%-- 상품추가 --%>
        <button type="button" class="btn_skyblue ml5" id="btnDtlAddProd" ng-click="addProd()" ng-if="btnDtlAddProd">
          <s:message code="hqDisuse.dtl.addProd"/></button>
        <%-- 저장 --%>
        <button type="button" class="btn_skyblue ml5" id="btnDtlSave" ng-click="saveHqDisuseDtl('')" ng-if="btnDtlSave">
          <s:message code="cmm.save"/></button>
        <%-- 확정 --%>
        <button type="button" class="btn_skyblue ml5" id="btnDtlConfirm" ng-click="confirm()" ng-if="btnDtlConfirm">
          <s:message code="hqDisuse.dtl.confirm"/></button>
      </div>
      ...
      <div class="w100 mt10 mb20">
        ...
        <div class="wj-gridWrap" style="height: 400px;">
          <wj-flex-grid
            ...
            <wj-flex-grid-column header="<s:message code="hqDisuse.reg.disuseQty"/>" binding="disuseQty" width="70" align="right" is-read-only="false" data-type="Number" format="n0" aggregate="Sum"></wj-flex-grid-column>
      ...
      <div class="pageNum mt20">
        <%-- id --%>
        <ul id="hqDisuseDtlCtrlPager" data-size="10">
        </ul>
      </div>
      <%--//페이지 리스트--%>
    </div>
...
<script type="text/javascript">
  ...
  app.controller('hqDisuseDtlCtrl', ['$scope', '$http', function ($scope, $http) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('hqDisuseDtlCtrl', $scope, $http, true));
    ...
    $scope.$on("hqDisuseDtlCtrl", function (event, data) {
      // 그리드 초기화
    ...
    $scope.procFgCheck = function () {
      ...
      $http({
        ...
        // "complete" code here
        $scope.wjHqDisuseDtlLayer.show(true);
    ...
    // 폐기상품 리스트 조회
    $scope.searchHqDisuseDtlList = function () {
      // 파라미터
      var params        = {};
      params.disuseDate = $scope.disuseDate;
      params.seqNo      = $scope.seqNo;
      ...
      $scope._inquiryMain("/stock/disuse/hqDisuse/hqDisuseDtl/list.sb", params);
    };


    // 폐기 상품 저장
    $scope.saveHqDisuseDtl = function (confirmFg) {
      if (nvl($scope.disuseTitle, '') === '') {
        var msg = messages["hqDisuse.reg.disuseTitle"] + messages["cmm.require.text"];
        $scope._popMsg(msg);
        return false;
      }
      ...
      $scope._save("/stock/disuse/hqDisuse/hqDisuseDtl/save.sb", params, function () {
    ...
    $scope.saveRegistCallback = function (confirmFg) {
      ...
      if (confirmFg === 'Y') {
        $scope.wjHqDisuseDtlLayer.hide(true);
      }
    };
    ...
    $scope.confirm = function () {
      var msg = messages["hqDisuse.dtl.confirmMsg"]; // 확정하시겠습니까?
      s_alert.popConf(msg, function () {
        $scope.saveHqDisuseDtl('Y');
        return false;
      });
    };
