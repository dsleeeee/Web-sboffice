    ...
    $scope.searchMobileMonthSaleShop = function(data){
        ...
        $scope._postJSONQuery.withOutPopUp( "/mobile/sale/status/monthSale/monthSale/getMobileMonthSaleShopList.sb", params, function(response){
            ...
            if(response.data.data.result != null) {
                ...
                $("#lblShopTblAvr").text($scope.mobileMonthSaleShop.tblAvr.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
                $("#lblShopTblCnt").text($scope.mobileMonthSaleShop.tblCnt.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
                $("#lblShopTblUprc").text($scope.mobileMonthSaleShop.tblUprc.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
            }
        });
    };
    // <-- //검색 호출 -->
...
app.controller('mobileMonthSaleDtlCtrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.initGrid = function (s, e) {
        ...
        dataItem.totGuestCnt = messages["mobile.monthSale.shop"];
        dataItem.guestUprc = messages["mobile.monthSale.shop"];
