/****************************************************************
 *
 * 파일명 : timeSaleStoreDtMoms.js
 * 설  명 : 맘스터치 > 간소화화면 > 시간대매출(매장) DT포장 JavaScript
 *
...
/**
 *  시간대매출(매장) DT포장 그리드 생성
 */
app.controller('timeSaleStoreDtMomsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeSaleStoreDtMomsCtrl', $scope, $http, false));
    ...
    $scope.initGrid = function (s, e) {
        ...
        // 첫째줄 헤더 생성
        var dataItem = {};
        dataItem.saleDate = messages["timeSaleStoreDtMoms.saleDate"];
        dataItem.storeCd = messages["timeSaleStoreDtMoms.storeCd"];
        dataItem.storeNm = messages["timeSaleStoreDtMoms.storeNm"];
        // 시간대별 컬럼 생성
        for (var i = 0; i < 10; i++) {
    ...
    // <-- 검색 호출 -->
    $scope.$on("timeSaleStoreDtMomsCtrl", function (event, data) {
        $scope.searchtimeSaleStoreDtMomsList();
        event.preventDefault();
    });

    $scope.searchtimeSaleStoreDtMomsList = function () {
        ...
        var srchOption = "";
        var dlvrInOut = '';
        if($("#chkOptStin").is(":checked")) {srchOption = "1"; dlvrInOut = '0';}
        if($("#chkOptDt").is(":checked")) {srchOption = "15"; dlvrInOut = '0';}
        if($("#chkOptChannel").is(":checked")) {srchOption = "1,15"; dlvrInOut = '1';}
        if($("#chkOptStin").is(":checked") && $("#chkOptDt").is(":checked")) {srchOption = "1,15"; dlvrInOut = '0';}
        if($("#chkOptStin").is(":checked") && $("#chkOptChannel").is(":checked")) {srchOption = "15"; dlvrInOut = '1';}
        if($("#chkOptDt").is(":checked") && $("#chkOptChannel").is(":checked")) {srchOption = "1"; dlvrInOut = '1';}
        if($("#chkOptStin").is(":checked") && $("#chkOptDt").is(":checked") && $("#chkOptChannel").is(":checked")) {srchOption = ""; dlvrInOut = '';}
        params.dlvrInOut = dlvrInOut;
        params.dlvrInFg = srchOption;
        params.storeHqBrandCd = $scope.storeHqBrandCd;
        params.storeCds = $("#timeSaleStoreDtMomsStoreCd").val();
        ...
        $scope._inquiryMain("/sale/moms/timeSaleStoreDtMoms/timeSaleStoreDtMoms/getTimeSaleStoreDtMomsList.sb", params, function (){});
    };
    // <-- //검색 호출 -->
    ...
    $scope.excelDownload = function (excelType) {
        ...
        if(params.excelType === '1') {
            ...
            $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
                $scope._broadcast('timeSaleStoreDtMomsExcelCtrl', params);
            });
        }else{
            // 분할 엑셀다운로드 사용자 제한 체크
            $scope._postJSONQuery.withOutPopUp('/sale/moms/prodSaleDayStoreMoms/prodSaleDayStoreMoms/getDivisionExcelDownloadUserIdChk.sb', params, function (response) {
                if (response.data.data.list === 0) {
                    $scope._popMsg(messages["timeSaleStoreDtMoms.userIdChkAlert"]); // 사용권한이 없습니다.
                    return;
                } else {
                    // 데이터양에 따라 2-3초에서 수분이 걸릴 수도 있습니다.
                    $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "시간대매출(매장) DT포장_" + wijmo.Globalize.format(startDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format(endDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('timeSaleStoreDtMomsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeSaleStoreDtMomsExcelCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("timeSaleStoreDtMomsExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
            ...
            data.downloadNo = "13"; // 다운로드 화면구분번호
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        // $scope._inquiryMain("/sale/moms/timeSaleStoreDtMoms/timeSaleStoreDtMoms/gettimeSaleStoreDtMomsExcelList.sb", params, function() {
        ...
        //                 return column.visible;
        //             }
        //         }, "시간대매출(매장) DT포장_" + params.startDate + "_" + params.endDate + "_" + getCurDateTime()+'.xlsx', function () {
        //             $timeout(function () {
        //                 $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
        //             }, 10);
        //         });
    ...
    $scope.searchExcelDivisionList = function (params) {
        ...
        $scope._postJSONQuery.withOutPopUp("/sale/moms/timeSaleStoreDtMoms/timeSaleStoreDtMoms/getTimeSaleStoreDtMomsList.sb", params, function(response){
                ...
                function delay(x){
                    return new Promise(function(resolve, reject){
                        ...
                        $scope._postJSONQuery.withOutPopUp("/sale/moms/prodSaleDayStoreMoms/prodSaleDayStoreMoms/getDivisionExcelDownloadSaveUpdate.sb", params, function(response){
                            ...
                            $http({
                                method: 'POST', //방식
                                // url: '/sale/moms/timeSaleStoreDtMoms/timeSaleStoreDtMoms/gettimeSaleStoreDtMomsList.sb', /* 통신할 URL */
                                url: '/sale/moms/timeSaleStoreDtMoms/timeSaleStoreDtMoms/getTimeSaleStoreDtMomsExcelDivisionList.sb', /* 통신할 URL */
                                ...
                                setTimeout(function() {
                                    ...
                                    }, "시간대매출(매장) DT포장_" + params.startDate + "_" + params.endDate + "_" + getCurDateTime() + '_' + (x + 1) + '.xlsx', function () {
                                        $timeout(function () {
