...
app.controller('standMoveProdCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('standMoveProdCtrl', $scope, $http, true));
...
//	        item.standTotQty   		= arrTotQty;   //총입고수량     
  ...
  $scope.$on("standMoveProdCtrl", function (event, data) {
    // 그리드 초기화
    ...
    $scope.wjStandMoveProdLayer.show(true);
  ...
  $scope.searchStoreMoveRegistList = function () {
    ...
    $scope._inquirySub("/iostock/move/standMove/standMoveAddProd/list.sb", params, function () {
...
//                grid.setCellData(i, 'arrTot_'		+ j,	arrTot	[i][j]);
  ...
  $scope.save = function () {
    ...
    console.log(params);
    $scope._save("/iostock/move/standMove/standMoveAddProd/save.sb", params, function () {
      $scope.saveStoreMoveProdCallback()
    });
  };

  $scope.saveStoreMoveProdCallback = function () {
    $scope.wjStandMoveProdLayer.hide(true);

    var standMoveDtlScope = agrid.getScope('standMoveDtlCtrl');
    standMoveDtlScope.searchStoreMoveDtlList();
