/****************************************************************
 *
 * 파일명 : operStoreList.js
 * 설  명 : 운영현황 운영현황 JavaScript
 *
 ...
 *  운영현황 그리드 생성
 */
app.controller('operStoreListCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('operStoreListCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope.posFgDataMap = new wijmo.grid.DataMap(posFgData, 'value', 'name'); //상태구분

    };

    // <-- 검색 호출 -->
    $scope.$on("operStoreListCtrl", function(event, data) {
        $scope.operStoreList();
        event.preventDefault();
    });

    // 매출매장현황 그리드 조회
    $scope.operStoreList = function() {
        if (!$scope.valueCheck()) {
            return false;
        }
        var params = {};
        ...
        params.manageVanCd = $("#manageVanCdOper").val();
...
// 체크
     ...
     * 관리밴사 조회
     * *******************************************************/
    $scope.searchManageVanOper = function(){
        ...
        popup.show(true, function (s) {
            ...
            vanScope.$apply(function () {
                ...
                if (!$.isEmptyObject(vanScope.getVan())) {
                    $("#manageVanCdOper").val(vanScope.getVan().vanCd);
                    $("#manageVanNmOper").val(vanScope.getVan().vanNm);
                }
            });
        });
    };
    ...
    // 관리밴사 선택취소
    $scope.delManageVanCdOper = function(){
        $("#manageVanCdOper").val("");
        $("#manageVanNmOper").val(messages["cmm.all"]);
    }

    /*********************************************************
     * 대리점(관리업체) 조회
     * *******************************************************/
    $scope.searchAgencyOper = function(){
        if(orgnFg === "MASTER") {
            ...
            popup.show(true, function (s) {
                ...
                agencyScope.$apply(function () {
                    ...
                    if (!$.isEmptyObject(agencyScope.getAgency())) {
                        $("#agencyCdOper").val(agencyScope.getAgency().agencyCd);
                        $("#agencyNmOper").val(agencyScope.getAgency().agencyNm);
                    }
                });
            });
        }
    ...
    // 대리점(관리업체) 선택취소
    $scope.delAgencyCdOper = function(){
        $("#agencyCdOper").val("");
        $("#agencyNmOper").val(messages["cmm.all"]);
    }
    ...
    $scope.excelDownloadInfo = function () {
        ...
        $timeout(function () {
            ...
            }, '운영현황_운영현황(' + wijmo.Globalize.format($scope.startDate, 'yyyy-MM-dd') +'_' + wijmo.Globalize.format($scope.endDate, 'yyyy-MM-dd') + ')'+'_'+ getCurDateTime() +'.xlsx', function () {
                $timeout(function () {
