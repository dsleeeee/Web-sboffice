/****************************************************************
 *
 * 파일명 : storeSoldOutHistory.js
 * 설  명 : 맘스터치 > 매장분석 > 매장품절현황 JavaScript
 *
 ...
 *  매장품절현황 그리드 생성
 */
app.controller('storeSoldOutHistoryCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeSoldOutHistoryCtrl', $scope, $http, false));
    ...
    $scope.$on("storeSoldOutHistoryCtrl", function (event, data) {
    ...
    $scope.getSearchSoldOutHistory = function () {
        ...
        params.storeCds = $("#storeSoldOutHistoryStoreCd").val();
    ...
    $scope.excelDownload = function (excelType) {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('storeSoldOutHistoryExcelCtrl', params);
        });
    };
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                "매장품절현항" + '_' + wijmo.Globalize.format($scope.srchStartDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format($scope.srchEndDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('storeSoldOutHistoryExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeSoldOutHistoryExcelCtrl', $scope, $http, false));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("storeSoldOutHistoryExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/store/storeSoldOutHistory/storeSoldOutHistory/getStoreSoldOutHistoryExcelList.sb", params, function() {
