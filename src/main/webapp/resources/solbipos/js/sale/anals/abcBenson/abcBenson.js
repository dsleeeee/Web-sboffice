 ...
 *
 * 파일명 : abcBenson.js
 * 설  명 : 벤슨 > 매출분석 > 상품 ABC분석 JavaScript
 *
...
app.controller('abcBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('abcBensonCtrl', $scope, $http, $timeout, true));
    ...
    $scope.srchAbcBensonStartDate = wcombo.genDateVal("#srchAbcBensonStartDate", getToday());
    $scope.srchAbcBensonEndDate   = wcombo.genDateVal("#srchAbcBensonEndDate", getToday());
    ...
    $scope.initGrid = function (s, e) {
        ...
        // 콤보박스 데이터 Set
        // $scope._setComboData('abcBensonlistScaleBox', gvListScaleBoxData);
        $scope._setComboData("srchAbcBensonDisplay", vSortFg);

        // picker 사용시 호출 : 미사용시 호출안함
        $scope._makePickColumns("abcBensonCtrl");
    ...
    $scope.$on("abcBensonCtrl", function (event, data) {
        $scope.searchAbcBensonList(true);
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    //다른 컨트롤러의 broadcast 받기
    $scope.$on("abcBensonCtrlSrch", function (event, data) {
        $scope.searchAbcBensonList(false);
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });
    ...
    // 매장형태별 매출 리스트 조회
    $scope.searchAbcBensonList = function (isPageChk) {

        var startDt = new Date(wijmo.Globalize.format($scope.srchAbcBensonStartDate.value, 'yyyy-MM-dd'));
        var endDt = new Date(wijmo.Globalize.format($scope.srchAbcBensonEndDate.value, 'yyyy-MM-dd'));
        ...
        params.gradeA = $("#abcBensonGradeA").val();
        params.gradeB = $("#abcBensonGradeB").val();
        params.gradeC = $("#abcBensonGradeC").val();
        params.sortFg   = $scope.sortFg;
        if($scope.sortFg === null) {
            params.sortFg   = "1";
        }
        params.storeCd = $("#abcBensonSelectStoreCd").val();
        params.isPageChk = isPageChk;

        // 등록일자 '전체기간' 선택에 따른 params
        if(!$scope.isChecked){
            params.startDate = wijmo.Globalize.format($scope.srchAbcBensonStartDate.value, 'yyyyMMdd');
            params.endDate = wijmo.Globalize.format($scope.srchAbcBensonEndDate.value, 'yyyyMMdd');
        }
        ...
        if(!$scope.gradeValidation()){
            $scope._popMsg($scope.highGrade + messages["abcBenson.rateMessage1"] + ' ' + $scope.lowGrade + messages["abcBenson.rateMessage2"]);
            return false;
        }

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/anals/abcBenson/abcBenson/abcBensonList.sb", params);
    ...
    $scope.gradeValidation = function(){
        ...
        if(Number($("#abcBensonGradeA").val()) >= Number($("#abcBensonGradeB").val())){
            $scope.highGrade = 'A';
            $scope.lowGrade = 'B';
            valCheck = false;
        } else if(Number($("#abcBensonGradeA").val()) >= Number($("#abcBensonGradeC").val())){
            $scope.highGrade = 'A';
            $scope.lowGrade = 'C';
            valCheck = false;
        } else if(Number($("#abcBensonGradeB").val()) >= Number($("#abcBensonGradeC").val())){
            $scope.highGrade = 'B';
            $scope.lowGrade = 'C';
            valCheck = false;
        }
    ...
    $scope.isChkDt = function() {
        $scope.srchAbcBensonStartDate.isReadOnly = $scope.isChecked;
        $scope.srchAbcBensonEndDate.isReadOnly = $scope.isChecked;
    };
    ...
    $scope.abcBensonSelectStoreShow = function () {
        $scope._broadcast('abcBensonSelectStoreCtrl');
    };

    //엑셀 다운로드
    $scope.excelDownloadAbcBenson = function () {
