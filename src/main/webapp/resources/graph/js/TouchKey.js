    ...
    for(var selected = 0; selected < grid.selectedItems.length; selected++ ) {
      ...
      try {
        ...
        var prodTag = graph.insertVertex(btn, null,
          item.prodNm,
          5, 5,
          graph.keySize.width - 10, graph.keySize.height / 2 - 5,
          'align=left;verticalAlign=top;strokeColor=none;resizable=0;autoSize=1;');

        // 버튼에 금액 추가
        var priceTag = graph.insertVertex(btn, null,
          item.saleUprc,
          5, graph.keySize.height / 2 + 5,
          graph.keySize.width - 10, graph.keySize.height / 2 - 10,
          'align=right;strokeColor=none;resizable=0;autoSize=1;');
...
Graph.prototype.initProdArea = function(group, sidebar) {
  ...
  graph.isGroup = false;
  // vertax에 child 삽입시 접는기능 제거 : 20180906 노현수
  graph.foldingEnabled = false;

  graph.removeCellsFromParent = false;
