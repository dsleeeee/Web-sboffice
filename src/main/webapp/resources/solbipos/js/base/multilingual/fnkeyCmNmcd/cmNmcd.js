 ...
 *
 * 파일명 : cmNmcd.js
 * 설  명 : 다국어관리(기능키/공통코드) - 공통코드 탭 JavaScript
 *
 ...
 * 공통코드 조회 그리드 생성
 */
app.controller('cmNmcdCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('cmNmcdCtrl', $scope, $http, true));

    // 콤보박스 데이터 Set
    $scope._setComboData("nmcodeGrpCd", nmcodeGrpCdList); // 기능구분
    ...
    $scope.initGrid = function (s, e) {
        ...
        s.cellEditEnded.addHandler(function (s, e) {
            if (e.panel === s.cells) {
                ...
                // 값 변경시 체크박스 체크
                if (col.binding === "nmcodeEnNm" || col.binding === "nmcodeCnNm" || col.binding === "nmcodeJpNm") {
                    $scope.checked(item);
                }
            }
    ...
    $scope.$on("cmNmcdCtrl", function (event, data) {
        // 조회
        $scope.getCmNmcdList();
    ...
    // 조회
    $scope.getCmNmcdList = function () {

        // 파라미터
        var params = {};
        params.nmcodeGrpCd = $scope.srchNmcodeGrpCdCombo.selectedValue;
        params.nmcodeCd = $("#srchNmcodeCd").val();
        params.nmcodeNm = $("#srchNmcodeNm").val();

        // 공통코드 탭 리스트 조회
        $scope._inquiryMain("/base/multilingual/fnkeyCmNmcd/getCmNmcdList.sb", params);
    };
    ...
    $scope.excelDownload = function(){
        ...
        $scope.$broadcast('loadingPopupActive', messages["cmm.progress"]); // 데이터 처리중 메시지 팝업 열기
            $timeout(function () {
                ...
                }, messages["fnkeyCmNmcd.cmNmcd"]  + '_' +  getCurDateTime() + '.xlsx', function () {
                    $timeout(function () {
    ...
    $scope.excelUpload = function () {
        ...
        $scope._popConfirm(msg, function() {
            ...
            $("#cmNmcdExcelUpFile").trigger('click');
...
app.controller('cmNmcdExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.sampleDownload = function (data) {
        ...
        var params = {};
        params.nmcodeGrpCd = data.nmcodeGrpCd;
        params.nmcodeCd = data.nmcodeCd;
        params.nmcodeNm = data.nmcodeNm;

        $scope._inquiryMain("/base/multilingual/fnkeyCmNmcd/getCmNmcdList.sb", params, function (){
            ...
            $timeout(function()	{
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(	$scope.flex,
                    ...
                    messages["fnkeyCmNmcd.cmNmcd"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
    ...
    $scope.excelUpload = function () {
        ...
        if ($('#cmNmcdExcelUpFile')[0].files[0]) {
            var file = $('#cmNmcdExcelUpFile')[0].files[0];
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/fnkeyCmNmcd/saveCmNmcd.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('cmNmcdCtrl').getCmNmcdList();
                }
            }
