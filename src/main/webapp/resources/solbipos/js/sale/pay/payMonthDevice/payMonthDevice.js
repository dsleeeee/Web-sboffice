/****************************************************************
 *
 * 파일명 : payMonthDevice.js
 * 설  명 : 맘스터치 > 결제수단별 매출 > 월별 결제수단 매출(기기별) JavaScript
 *
 ...
 *  월별종합 조회 그리드 생성
 */
app.controller('payMonthDeviceCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('payMonthDeviceCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("payMonthDeviceCtrl");
        ...
        s.addEventListener(s.hostElement, 'mousedown', function(e) {
            ...
            if( ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                params.storeCd = $("#payMonthDeviceStoreCd").val();
                if(orgnFg == "STORE") {
    ...
    // <-- 검색 호출 -->
    $scope.$on("payMonthDeviceCtrl", function(event, data) {
    ...
    $scope.searchMonthTotal = function() {
        ...
        params.storeCds = $("#payMonthDeviceStoreCd").val();
        params.payCol    = payCol;

        $scope._inquiryMain("/sale/pay/payMonthDevice/payMonthDevice/getPayMonthDeviceList.sb", params, function() {}, false);
    };
    // <-- //검색 호출 -->
    ...
    $scope.payMonthDeviceStoreShow = function () {
        $scope._broadcast('payMonthDeviceStoreCtrl');
    };
    ...
    $scope.excelDownloadInfo = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                messages["payMonthDeviceMoms.payMonthDeviceMoms"] + getCurDateTime() +'.xlsx', function () {
                    $timeout(function () {
