/****************************************************************
 *
 * 파일명 : mobileStoreDailyIoStock.js
 * 설  명 : (모바일) 재고현황 > 일자별수불현황(매장)JavaScript
 *
...
app.controller('mobileDailyIoStockCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileDailyIoStockCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("mobileDailyIoStockCtrl");

        // 그리드 링크 효과
        s.formatItem.addHandler(function (s, e) {
            if (e.panel === s.cells) {
                var col = s.columns[e.col];
                if (col.binding.slice(-3) === "Qty" && s.cells.getCellData(e.row,e.col,false) != null && col.binding !== "setInQty" && col.binding !== "storeSaleQty") { // 수량 링크
        ...
        s.addEventListener(s.hostElement, 'mousedown', function (e) {
            ...
            if (ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if (col.binding.slice(-3) === "Qty" && selectedRow[col.binding] != null && col.binding !== "setInQty" && col.binding !== "storeSaleQty") { // 수량 클릭
                    var params    = {};
                    ...
                    $scope._broadcast('mobileDailyIoStockInfoCtrl', params);
                }
            }
        }, true);
        ...
        dataItem.storeInQty	    = messages["dailyIostock.col13"];
        dataItem.storeInTot	    = messages["dailyIostock.col13"];
        dataItem.storeOutQty 	= messages["dailyIostock.col14"];
        dataItem.storeOutTot 	= messages["dailyIostock.col14"];
        dataItem.purchsInQty 	= messages["dailyIostock.col15"];
        dataItem.purchsInTot 	= messages["dailyIostock.col15"];
        dataItem.purchsOutQty	= messages["dailyIostock.col16"];
        dataItem.purchsOutTot	= messages["dailyIostock.col16"];
        dataItem.storeSaleQty	= messages["dailyIostock.col17"];
        dataItem.storeSaleTot	= messages["dailyIostock.col17"];
        dataItem.moveInQty	    = messages["dailyIostock.col18"];
        dataItem.moveInTot	    = messages["dailyIostock.col18"];
        dataItem.moveOutQty	    = messages["dailyIostock.col19"];
        dataItem.moveOutTot	    = messages["dailyIostock.col19"];
    ...
    $scope.$on("mobileDailyIoStockCtrl", function (event, data) {

        $scope.searchMobileDailyIoStockList();
    ...
    // 일자별수불현황 리스트 조회
    $scope.searchMobileDailyIoStockList = function () {
        // 파라미터
        var params     = {};
        ...
        $scope._inquirySub("/mobile/stock/status/dailyIoStock/mobileDailyIoStock/getDailyIoStockList.sb", params, function (){
            gridShowMsgNoData("mobileStoreDailyIoStock", $scope.flex, "Y");
        },true);
    ...
    $scope.excelDownload = function () {
        ...
        // 파라미터
        var params     = {};
        $scope._broadcast('mobileDailyIoStockExcelCtrl',params);
    };
...
/** 일자별수불현황 그리드 controller */
app.controller('mobileDailyIoStockExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileDailyIoStockExcelCtrl', $scope, $http, true));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("mobileDailyIoStockExcelCtrl", function (event, data) {
        ...
        if(data != undefined && $scope.excelFg) {
            ...
            $scope.searchMobileDailyIoStockExcelList();
    ...
    // 일자별수불현황 리스트 조회
    $scope.searchMobileDailyIoStockExcelList = function () {
        // 파라미터
        var params     = {};
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/mobile/stock/status/dailyIoStock/mobileDailyIoStock/getDailyIoStockExcelList.sb", params, function(){
