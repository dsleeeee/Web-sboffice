...
/** 반품본사입고현황 그리드 controller */
app.controller('rtnInstockConfmStoreCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('rtnInstockConfmStoreCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("rtnInstockConfmStoreCtrl");
        ...
        s.addEventListener(s.hostElement, 'mousedown', function (e) {
            ...
            if (ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if (col.binding === "slipNo") { // 전표번호 클릭
                    ...
                    params.vendrCd = $("#rtnInstockConfmStoreSelectVendrCd").val();
                    $scope._broadcast('rtnInstockConfmStoreDtlCtrl', params);
                }
            }
        });
    ...
    // <-- 검색 호출 -->
    $scope.$on("rtnInstockConfmStoreCtrl", function(event, data) {
    ...
    // 입고확정 리스트 조회
    $scope.searchRtnInstockConfmStoreList = function () {
        // 파라미터
        var params = {};
        params.slipFg = $scope.slipFg;
        params.procFg = $scope.srchProcFgCombo.selectedValue;
        params.slipKind = $scope.srchSlipKindCombo.selectedValue;
        params.vendrCd = $("#rtnInstockConfmStoreSelectVendrCd").val();
        params.startDate = wijmo.Globalize.format(srchStartDate.value, 'yyyyMMdd');
        params.endDate = wijmo.Globalize.format(srchEndDate.value, 'yyyyMMdd');

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/iostock/orderReturn/rtnInstockConfmStore/rtnInstockConfmStore/list.sb", params);
    };
    ...
    $scope.rtnInstockConfmStoreSelectVendrShow = function () {
        $scope._broadcast('rtnInstockConfmStoreSelectVendrCtrl');
    };
    ...
    $scope.excelDownload = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(
                ...
                //'반품본사입고현황_' + getToday() + '.xlsx',
                '반품본사입고현황_' + getCurDate('-') + '.xlsx',
                function () {
                    $timeout(function () {
