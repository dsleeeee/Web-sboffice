/****************************************************************
 *
 * 파일명 : kioskDisplayResve.js
 * 설  명 : 상품 비노출관리(예약) JavaScript
 *
...
// 비노출여부
var kioskDisplayResveYnData = [
  {"name": "노출", "value": "Y"},
  {"name": "비노출", "value": "N"}
];

// 비노출여부
var kioskDisplayResveYnAllData = [
  {"name": "전체", "value": ""},
  {"name": "노출", "value": "Y"},
  {"name": "비노출", "value": "N"}
];
 ...
 * 상품정보관리 그리드 생성
 */
app.controller('kioskDisplayResveCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('kioskDisplayResveCtrl', $scope, $http, false));
  ...
  // 비노출여부 콤보박스
  $scope._getComboDataQuery('094', 'kioskDisplayResveYnComboData');
  // 세트상품구분 콤보박스
  ...
  // 비노출여부 콤보박스
  $scope._setComboData("kioskDisplayResveYnCombo", kioskDisplayResveYnAllData);
  $scope._setComboData("kioskDisplayResveYnComboChg", kioskDisplayResveYnData);
  $scope._setComboData("kioskDisplayResveYnComboChg", kioskDisplayResveYnData);
  $scope._setComboData("kioskDisplayYnCombo2", kioskDisplayResveYnData);
  // 매장브랜드 콤보박스
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope.kioskDisplayResveYnDataMap = new wijmo.grid.DataMap(kioskDisplayResveYnData, 'value', 'name'); // 비노출여부
    ...
    s.cellEditEnded.addHandler(function (s, e) {
      if (e.panel === s.cells) {
        ...
        if (col.binding === "startDate" || col.binding === "startTime" || col.binding === "kioskDisplayYn") {
          item.gChk = true;
        }
      }
  ...
  $scope.change = function() {
    for(var i = $scope.flex.collectionView.items.length-1; i >= 0; i-- ){
      ...
      if(item.gChk){
        item.kioskDisplayYn = $scope.kioskDisplayYnChange;
      }
    }
  ...
  // 상품정보관리 그리드 조회
  $scope.$on("kioskDisplayResveCtrl", function(event, data) {
    $scope.searchKioskDisplayResveList();
    // 기능수행 종료 : 반드시 추가
  ...
  $scope.kioskDisplayResveStoreShow = function () {
    $scope._broadcast('kioskDisplayResveStoreCtrl');
  };

  // 상품선택 모듈 팝업 사용시 정의
  // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
  // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
  $scope.kioskDisplayResveProdShow = function () {
    $scope._broadcast('kioskDisplayResveProdCtrl');
  };

  // 예약 내역 조회
  $scope.searchKioskDisplayResveList = function(){

    if(orgnFg == "HQ"){
      if(($("#kioskDisplayResveStoreCd").val() === "" || $("#kioskDisplayResveStoreCd").val() === undefined) && ($("#kioskDisplayResveProdCd").val() === "" || $("#kioskDisplayResveProdCd").val() === undefined)){
    ...
    params.storeCds = $("#kioskDisplayResveStoreCd").val();
    params.prodCds = $("#kioskDisplayResveProdCd").val();
    ...
    $scope._inquiryMain("/base/prod/kioskDisplayResve/kioskDisplayResve/getKioskDisplayResve.sb", params, function(){
  ...
  $scope.batchChange = function(chgGubun) {
    ...
    for (var i = 0; i < $scope.flex.collectionView.items.length; i++) {
      if($scope.flex.collectionView.items[i].gChk) {
        $scope.flex.collectionView.items[i].kioskDisplayResveYn = $scope.kioskDisplayResveYnChg;
      }
    }
    $scope.flex.refresh();
  };
  ...
  // 삭제
  $scope.del = function (){
    $scope._popConfirm(messages["kioskDisplayResve.delConfirm"], function() {
      // 파라미터 설정
      var params = new Array();
      ...
      $scope._save('/base/prod/kioskDisplayResve/kioskDisplayResve/deleteKioskDisplayResve.sb', params, function () {
        $scope.searchKioskDisplayResveList();
      });
    });
  }
  ...
  $scope.save = function(){
    ...
    $scope._popConfirm(messages["cmm.choo.save"], function() {
      ...
      // 저장기능 수행 : 저장URL, 파라미터, 콜백함수
      $scope._save("/base/prod/kioskDisplayResve/kioskDisplayResve/modKioskDisplayResve.sb", params, function(){
        $scope.searchKioskDisplayResveList();
      });
    });
  ...
  // 추가 팝업
  $scope.add = function () {
    $scope.kioskDisplayResveAddLayer.show(true);
  };

  // 그리드 선택불가 항목처리
  $scope.setGridReadOnly = function () {
    var grid = wijmo.Control.getControl("#wjGridKioskDisplayResve");
    var rows = grid.rows;
