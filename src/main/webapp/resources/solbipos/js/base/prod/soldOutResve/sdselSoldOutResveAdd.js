 ...
 *  예약추가 팝업
 */
app.controller('sdselSoldOutResveAddCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('sdselSoldOutResveAddCtrl', $scope, $http, true));
    ...
    // 조회
    $scope.$on("sdselSoldOutResveAddCtrl", function(event, data) {
        event.preventDefault();
    });

    // 리스트 조회
    $scope.searchSdselSoldOutResveAddList = function () {

        if(orgnFg == "HQ"){
            if(($("#sdselSoldOutResveAddStoreCd").val() === "" || $("#sdselSoldOutResveAddStoreCd").val() === undefined) && ($("#sdselSoldOutResveAddProdCd").val() === "" || $("#sdselSoldOutResveAddProdCd").val() === undefined)){
        ...
        var params = {};
        params.storeCds = $("#sdselSoldOutResveAddStoreCd").val();
        params.prodCds = $("#sdselSoldOutResveAddProdCd").val();
        ...
        $scope._inquirySub('/base/prod/soldOutResve/soldOutResve/getSdselProdList.sb', params, function() {}, false);
    };

    // 체크박스 체크
    $scope.checked = function (item){
        if(item.soldOutYn != undefined && item.soldOutYn != null && item.soldOutYn != ""){
            item.gChk = true;
        }
    }
    ...
    $scope.saveProdPrice2 = function(){
        ...
        $scope._save('/base/prod/soldOutResve/soldOutResve/saveSdselSoldOutResve.sb', params, function(){
            ...
            // 팝업 닫기
            $scope.sdselSoldOutResveAddLayer.hide(true);
    ...
    $scope.searchAddShowChangeEnv = function(){
        if( $("#tblSearchAddShowAdd2").css("display") === 'none') {
            $("#tblSearchAddShowAdd2").show();
        } else {
            $("#tblSearchAddShowAdd2").hide();
        }
    };

    // 일괄변경 테이블 숨김/보임
    $scope.changeAddShow = function(){
        if( $("#tblAddChange2").css("display") === 'none'){
            $("#tblAddChange2").show();
        } else {
            $("#tblAddChange2").hide();
        }
    ...
    $scope.sdselSoldOutResveAddStoreShow = function () {
        $scope._broadcast('sdselSoldOutResveAddStoreCtrl');
    };

    // 상품선택 모듈 팝업 사용시 정의
    // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
    // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
    $scope.sdselSoldOutResveAddProdShow = function () {
        $scope._broadcast('sdselSoldOutResveAddProdCtrl');
    };
