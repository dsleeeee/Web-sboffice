...
app.controller('saleRegistCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.initGrid = function (s, e) {
        $scope.saleGubunDataMap = new wijmo.grid.DataMap(saleGubunData, 'value', 'name'); // 결제구분
    ...
    $scope.newRegist = function (){
        if(srchGubun){
            ...
            $scope._popMsg(messages["saleRegistChargeKmu.srchGubun.chk"]);
            return false;
        }
    };
    ...
    $scope.searchSaleRegistList = function () {
        ...
        $scope._inquirySub("/excclc/excclc/saleRegistChargeKmu/saleRegistChargeKmu/getSaleRegistChargeKmuList.sb", params, function (){
            srchGubun = true;
        });
    };
