 ...
 * 상품별 판매가관리 그리드 생성
 */
app.controller('hqSalePriceCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('hqSalePriceCtrl', $scope, $http, false));
    ...
    $scope.searchSalePriceList = function(){
        ...
        $scope._inquirySub('/base/price/salePrice/hqSalePrice/getHqSalePriceList.sb', params, function() {
            ...
            if(coercionFg === "0") {
                var grid = wijmo.Control.getControl("#wjGridHqSalePriceArea");
                var rows = grid.rows;
                ...
                if (($scope.flex.collectionView.items[i].storeSaleUprc !== $scope.flex.collectionView.items[i].saleUprc) ||
                    ...
                    if(subPriceFg === "1") {
                        ...
                        if ($scope.flex.collectionView.items[i].stinSaleUprc !== "" && $scope.flex.collectionView.items[i].stinSaleUprc !== null) {
                            ...
                            // 변경내점-판매가 - 저울상품이면 음수 불가(0 허용), 일반상품이면 9자리 제한
                            if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                if ($scope.flex.collectionView.items[i].stinSaleUprc < 0) {
                                    $scope._popMsg(messages["salePrice.stinSaleUprcTypeChk"]); // 양수만 입력 가능
                                    return false;
                                }
                            } else {
                        ...
                        if ($scope.flex.collectionView.items[i].dlvrSaleUprc !== "" && $scope.flex.collectionView.items[i].dlvrSaleUprc !== null) {
                            ...
                            // 변경배달-판매가 - 저울상품이면 음수 불가(0 허용), 일반상품이면 9자리 제한
                            if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                if ($scope.flex.collectionView.items[i].dlvrSaleUprc < 0) {
                                    $scope._popMsg(messages["salePrice.dlvrSaleUprcTypeChk"]); // 양수만 입력 가능
                                    return false;
                                }
                            } else {
                        ...
                        if ($scope.flex.collectionView.items[i].packSaleUprc !== "" && $scope.flex.collectionView.items[i].packSaleUprc !== null) {
                            ...
                            // 변경포장-판매가 - 저울상품이면 음수 불가(0 허용), 일반상품이면 9자리 제한
                            if ($scope.flex.collectionView.items[i].prodTypeFg === "3") {
                                if ($scope.flex.collectionView.items[i].packSaleUprc < 0) {
                                    $scope._popMsg(messages["salePrice.packSaleUprcTypeChk"]); // 양수만 입력 가능
                                    return false;
                                }
                            } else {
    ...
    $scope.excelDownload = function () {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('hqSalePriceExcelCtrl', params);
        });
    };
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('hqSalePriceExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('hqSalePriceExcelCtrl', $scope, $http, false));
    ...
    $scope.$on("hqSalePriceExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/base/price/salePrice/hqSalePrice/getHqSaleExcelList.sb", params, function() {
            ...
            $timeout(function () {
                ...
                }, "본사판매가관리" + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
