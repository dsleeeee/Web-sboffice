...
<wj-popup id="wj<c:out value="${param.targetId}"/>LayerS" control="wj<c:out value="${param.targetId}"/>LayerS" show-trigger="Click" hide-trigger="Click" style="display:none;width:500px;">
  <div class="wj-dialog wj-dialog-columns" ng-controller="<c:out value="${param.targetId}"/>Ctrl">
    <div class="wj-dialog-header wj-dialog-header-font">
      <s:message code="cmm.storage.select"/>
      ...
      <div class="w100">
        <%--위즈모 테이블--%>
          ...
          <wj-flex-grid
            ...
            <!-- define columns -->
            <wj-flex-grid-column header="<s:message code="hqMove.storageCd"/>" binding="storageCd" width="70" align="center" is-read-only="true"></wj-flex-grid-column>
            <wj-flex-grid-column header="<s:message code="hqMove.storageNm"/>" binding="storageNm" width="*" align="left" is-read-only="true"></wj-flex-grid-column>
...
<script type="text/javascript">
  ...
  app.controller('${param.targetId}Ctrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $scope.initGrid = function (s, e) {
      ...
      s.addEventListener(s.hostElement, 'mousedown', function (e) {
        ...
        if (ht.cellType === wijmo.grid.CellType.Cell) {
          ...
          if (col.binding === "storageCd") {
            $("#" + $scope.targetId + "Cd").val(selectedRow.storageCd);
            $("#" + $scope.targetId + "Nm").val("[" + selectedRow.storageCd + "] " + selectedRow.storageNm);
    ...
    $scope.searchStore = function () {
      // 파라미터
      var params = {};
      $scope._inquirySub("/iostock/cmm/iostockCmm/selectStorageList.sb", params, function () {
        $scope.searchFg = "Y";
      });
    };
  }]);
