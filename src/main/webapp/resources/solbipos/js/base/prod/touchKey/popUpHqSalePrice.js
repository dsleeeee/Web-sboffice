...
app.controller('popUpHqSalePriceCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.$on("popUpHqSalePriceCtrl", function(event, key) {
        $scope._setComboData("hqTukeyGrp", tukeyGrpData);
    ...
    $scope.searchSalePriceList = function(data){
        ...
        if(data !== undefined){
            ...
            params.tukeyGrpCd = $scope.hqTukeyGrp;
        }
        console.log(params);

        $scope._inquirySub("/base/prod/touchKey/touchKey/getHqSalePrice.sb", params, function () {
            if(data !== undefined) {
                $scope.hqTukeyGrpCombo.selectedValue = data;
            }
        });
    };
