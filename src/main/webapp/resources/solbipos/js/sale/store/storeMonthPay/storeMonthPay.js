...
app.controller('storeMonthPayCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeMonthPayCtrl', $scope, $http, true));

    // 검색조건에 조회기간
    var startMonth = new wijmo.input.InputDate('#startMonthTotal', {
        format       : "yyyy-MM",
        selectionMode: "2" // 달력 선택 모드(1:day 2:storeMonthPay)
    });
    var endMonth = new wijmo.input.InputDate('#endMonthTotal', {
        format       : "yyyy-MM",
        selectionMode: "2" // 달력 선택 모드(1:day 2:storeMonthPay)
    });
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("storeMonthPayCtrl");
    ...
    // <-- 검색 호출 -->
    $scope.$on("storeMonthPayCtrl", function(event, data) {
    ...
    $scope.searchMonthTotal = function() {
        ...
        params.storeCds = $("#storeMonthPayStoreCd").val();
        params.payCol    = payCol;

        $scope._inquiryMain("/sale/store/storeMonthPay/storeMonthPay/getStoreMonthPayList.sb", params, function() {}, false);
    };
    // <-- //검색 호출 -->
    ...
    $scope.storeMonthPayStoreShow = function () {
        $scope._broadcast('storeMonthPayStoreCtrl');
    };
    ...
    $scope.excelDownloadInfo = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                messages["storeMonthPayMoms.storeMonthPayMoms"] + '_' + getCurDateTime() +'.xlsx', function () {
                    $timeout(function () {
