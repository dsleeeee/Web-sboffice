...
app.controller('dayCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('dayCtrl', $scope, $http, true));
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("dayCtrl");
  ...
  $scope.$on("dayCtrl", function (event, data) {
    $scope.searchDayList();
    // 기능수행 종료 : 반드시 추가
  ...
  $scope.searchDayList = function () {
    ...
    $scope.searchedStoreCd = $("#daySelectStoreCd").val();
    // 파라미터
    var params       = {};
    ...
    $scope._inquirySub("/sale/anals/store/saleAnals/getDayList.sb", params);
  };
  ...
  $scope.daySelectStoreShow = function () {
    $scope._broadcast('daySelectStoreCtrl');
  };
    ...
    $timeout(function () {
      wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
        ...
        includeColumns: function (column) {
          ...
          messages["saleAnals.saleAnals"] + '(' + messages["saleAnals.day"] + ')_' + getCurDateTime() +'.xlsx', function () {
        $timeout(function () {
