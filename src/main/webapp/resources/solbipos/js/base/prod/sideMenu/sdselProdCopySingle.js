 ...
 *  선택분류복사 팝업 조회 그리드 생성
 */
app.controller('sdselProdCopySingleCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    // 복사 실행 식별용 nonce : 진입 시마다 새로 발급(재전송이면 같은 값이 다시 들어옴)
    var _reqNonce = "";
    ...
    $scope.prodCopySingleSave = function(){
        ...
        sysLog("[상품복사S] 1. prodCopySingleSave 진입(확인전) nonce=" + _reqNonce);
        $scope._popConfirm(msg, function() {
            sysLog("[상품복사S] 2. 확인(OK) 콜백 진입");
            ...
            sysLog("[상품복사S] 3. params 구성완료 건수=" + params.length + " / 오버레이active=" + (document.getElementById('loadingOverlay') ? document.getElementById('loadingOverlay').classList.contains('active') : "null"));

            if(params.length <= 0) {
                sysLog("[상품복사S] 3-1. 선택없음 return");
            ...
            var pollCopyStatus = function (cnt) {
                if (cnt >= POLL_MAX_CNT) {
                    sysLog("[상품복사S] 5-P. 폴링 상한 초과 nonce=" + _reqNonce);
                ...
                setTimeout(function () {
                    $http({
                        ...
                        sysLog("[상품복사S] 5-P. 폴링 " + (cnt + 1) + "회 status=" + st);
                        if (st === 'DONE') {
            ...
            // 전체 데이터를 한 번에 전송
            sysLog("[상품복사S] 4. getSdselProdCopySave 호출 직전");
            $http({
                method: 'POST',
                url: '/base/prod/sideMenu/menuClass/getSdselProdCopySave.sb',
                data: params,  // 전체 전송
                params: sParam,
                ...
                if (response.data && response.data.data === 'RESEND_BLOCKED') {
                    sysLog("[상품복사S] 5-P. 재전송 차단응답 수신 → 상태폴링 전환 nonce=" + _reqNonce);
                    pollCopyStatus(0);
                    return;
                }
                ...
                sysLog("[상품복사S] 6. 저장 complete(then) 진입");
            });
        });
    };
    ...
    $scope.excelUploadingPopup = function (showFg) {
        ...
        sysLog("[상품복사S] 오버레이 " + (showFg ? "등록(ON)" : "해제(OFF)") + " 요청 / 현재active=" + (overlay ? overlay.classList.contains('active') : "null"));
