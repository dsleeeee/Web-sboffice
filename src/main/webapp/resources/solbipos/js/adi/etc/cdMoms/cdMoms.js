 ...
 * 대표명칭 그리드 생성
 */
app.controller('cdMomsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('cdMomsCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("cdMomsCtrl");
        ...
        s.hostElement.addEventListener('mousedown', function(e) {
            ...
            if( ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if( col.binding === "nmcodeCd" && selectedRow.status !== "I") {
                    //$scope._broadcast('cdMomsDetailCtrl', selectedRow.nmcodeCd);
                    $scope._broadcast('cdMomsDetailCtrl', selectedRow);
                }
            }
        });
    };
    ...
    // 대표명칭 그리드 조회
    $scope.$on("cdMomsCtrl", function(event, data) {
        // 파라미터
        var params = {};
    ...
    $scope.save = function() {
        ...
        for (var i = 0; i < $scope.flex.collectionView.itemsAdded.length; i++) {
            ...
            if(item.nmcodeCd === undefined || item.nmcodeCd.length === 0){
                $scope._popMsg(messages["cdMoms.represent.require.nmcodeCd"]); // 대표명칭의 코드를 입력해주세요
                return false;
            }

            if(item.nmcodeNm === undefined || item.nmcodeNm.length === 0){
                $scope._popMsg(messages["cdMoms.represent.require.nmcodeNm"]); // 대표명칭의 코드명을 입력해주세요
                return false;
            }
        ...
        $scope._save("/adi/etc/cdMoms/cdMoms/getNmcodeCdMomsSave.sb", params);
    };
 ...
 * 세부명칭 그리드 생성
 */
app.controller('cdMomsDetailCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('cdMomsDetailCtrl', $scope, $http, false));

    // grid 초기화 : 생성되기전 초기화되면서 생성된다
    $scope.initGrid = function (s, e) {
        // picker 사용시 호출 : 미사용시 호출안함
        $scope._makePickColumns("cdMomsDetailCtrl");
    ...
    // 세부명칭 그리드 조회
    $scope.$on("cdMomsDetailCtrl", function(event, data) {
        // 파라미터
        var params = {};
    ...
    // 세부명칭 그리드 행 추가
    $scope.addRow = function() {
        var gridRepresent = agrid.getScope('cdMomsCtrl');
    ...
    $scope.save = function() {
        ...
        for (var i = 0; i < $scope.flex.collectionView.itemsAdded.length; i++) {
            var item = $scope.flex.collectionView.itemsAdded[i];
            if(item.nmcodeCd === undefined || item.nmcodeCd.length === 0){
                $scope._popMsg(messages["cdMoms.detail.require.nmcodeCd"]); // 세부명칭의 코드를 입력해주세요
                return false;
            }

            if(item.nmcodeNm === undefined || item.nmcodeNm.length === 0){
                $scope._popMsg(messages["cdMoms.detail.require.nmcodeNm"]); // 세부명칭의 코드명을 입력해주세요
                return false;
            }

            if(item.nmcodeCd.length != $("#s_nmcodeItem2").val()){
                $scope._popMsg(messages["cdMoms.detail.require.nmcodeCdLengthChk"]+' ('+$("#s_nmcodeItem2").val()+')'); // 세부명칭의 코드자릿수를 확인하여 주십시오.
                return false;
            }
            ...
            if(check_nmcodeCd_cnt > 1){
                $scope._popMsg(messages["cdMoms.detail.require.nmcodeCdChk"]+' ('+item.nmcodeCd+')'); // 코드중복 확인
                return false;
            }
        ...
        // 저장기능 수행 : 저장URL, 파라미터, 콜백함수
        $scope._save("/adi/etc/cdMoms/cdMoms/getNmcodeCdMomsSave.sb", params, function(){ $scope.allSearch() });
    ...
    // 세부명칭 그리드 행 삭제
    $scope.deleteRow = function() {
        $scope._popConfirm(messages["cdMoms.detail.require.delConfirm"], function() {
            for(var i = $scope.flex.collectionView.items.length-1; i >= 0; i-- ){
                var item = $scope.flex.collectionView.items[i];
                if(item.gChk) {
                    if (item.nmcodeGrpCd == "093" && (item.nmcodeCd == "1" || item.nmcodeCd == "2"))
                    {
                        $scope._popMsg(messages["cdMoms.detail.require.chk.093.1"]); // 093 발주단위 기본값 0 낱개 1 박스 는 삭제할 수 없습니다.
                        return false;
                    }
                    $scope.flex.collectionView.removeAt(i);
                }
