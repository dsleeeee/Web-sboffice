/****************************************************************
 *
 * 파일명 : dayProdBenson.js
 * 설  명 : (벤슨) 일별상품매출현황 JavaScript
 *
...
app.controller('dayProdBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dayProdBensonCtrl', $scope, $http, true));
    ...
    $scope.$on("dayProdBensonCtrl", function (event, data) {

        // 일별상품매출현황 리스트 조회
        $scope.searchDayProdBensonList();
    ...
    // 일별상품매출현황 리스트 조회
    $scope.searchDayProdBensonList = function () {
        ...
        params.storeCds = $("#dayProdBensonStoreCd").val();
        params.prodCds = $("#dayProdBensonSelectCd").val();
        ...
        $scope._inquiryMain("/sale/prod/dayProdBenson/dayProdBenson/getDayProdBensonList.sb", params, function (){
            // <-- 그리드 visible -->
    ...
    $scope.dayProdBensonSelectShow = function () {
        $scope._broadcast('dayProdBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function () {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('dayProdBensonExcelCtrl', params);
        });
    };
 ...
 */
app.controller('dayProdBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dayProdBensonExcelCtrl', $scope, $http, false));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("dayProdBensonExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/prod/dayProdBenson/dayProdBenson/getDayProdBensonExcelList.sb", params, function() {
            if ($scope.flex.rows.length <= 0) {
                $scope._popMsg(messages["excelUpload.not.downloadData"]); // 다운로드 할 데이터가 없습니다.
                return false;
            }
