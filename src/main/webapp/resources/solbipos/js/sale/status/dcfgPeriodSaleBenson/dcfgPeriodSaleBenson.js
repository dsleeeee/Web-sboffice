/****************************************************************
 *
 * 파일명 : dcfgPeriodSaleBenson.js
 * 설  명 : (벤슨) 할인구분기간상세 화면 JavaScript
 *
...
/** 할인구분기간상세 controller */
app.controller('dcfgPeriodSaleBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dcfgPeriodSaleBensonCtrl', $scope, $http, $timeout, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        s.formatItem.addHandler(function (s, e) {
            if (e.panel === s.cells) {
                ...
                // 소계, 합계 row에 색상 넣기
                if (col.binding === "seq" || col.binding === "saleDate"|| col.binding === "storeCd" || col.binding === "storeNm" || col.binding === "dcNm" || col.binding === "dcdtlDcNm" ||
    ...
    $scope.$on("dcfgPeriodSaleBensonCtrl", function (event, data) {

        // 조회
        $scope.srchDcfgPeriodSaleBensonList();
    ...
    // 조회
    $scope.srchDcfgPeriodSaleBensonList = function () {
        ...
        params.storeCd = $("#dcfgPeriodSaleBensonStoreCd").val();

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/status/dcfgPeriodSaleBenson/list.sb", params, function () {

            var grid = wijmo.Control.getControl("#wjGridDcfgPeriodSaleBenson");
            var rows = grid.rows;
            ...
            for (var i = 0; i < $scope.flex.collectionView.items.length; i++) {
                ...
                if (item.seq === "1") {
                    ...
                    item.storeCd = rowCountAllMsg;
                    item.storeNm = rowCountAllMsg;
                    item.dcNm = rowCountAllMsg;
                    item.dcdtlDcNm = rowCountAllMsg;
                    item.lv1Cd = rowCountAllMsg;
                    item.lv2Cd = rowCountAllMsg;
                    item.lv3Cd = rowCountAllMsg;
    ...
    $scope.dcfgPeriodSaleBensonDcfgShow = function () {
        $scope._broadcast('dcfgPeriodSaleBensonDcfgCtrl');
    };
    ...
    $scope.isChkProdClassDisplay = function () {
        ...
        for (var i = 0; i < columns.length; i++) {
            if (columns[i].binding === 'lv1Cd' || columns[i].binding === 'lv2Cd' || columns[i].binding === 'lv3Cd' ||
                columns[i].binding === 'lv1Nm' || columns[i].binding === 'lv2Nm' || columns[i].binding === 'lv3Nm') {
    ...
    $scope.excelDownload = function () {
        ...
        $timeout(function () {
            ...
            }, messages["dcfgPeriodSaleBenson.dcfgPeriodSaleBenson"] + '_' + getToday() + '.xlsx', function () {
                $timeout(function () {
    ...
    $scope.resetDcfg = function () {
        if ($("#dcfgPeriodSaleBensonStoreCd").val() !== $("#org_dcfgPeriodSaleBensonStoreCd").val()) {
            $("#dcfgPeriodSaleBensonDcfgNm").val(messages["cmm.all"]);
            $("#dcfgPeriodSaleBensonDcfgCd").val("");
        }
    };
