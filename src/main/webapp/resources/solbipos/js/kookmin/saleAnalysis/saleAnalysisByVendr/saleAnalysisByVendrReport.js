/****************************************************************
 *
 * 파일명 : saleAnalysisByVendrReport.js
 * 설  명 : 국민대 > 매출분석 > 매입처별 매출분석 출력 팝업 JavaScript
 *
...
/** 매입처별 매출분석 출력 팝업 그리드 controller */
app.controller('saleAnalysisByVendrReportCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('saleAnalysisByVendrReportCtrl', $scope, $http, true));

    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("saleAnalysisByVendrReportCtrl", function (event, data) {
        $scope.data = data;

        $scope.wjSaleAnalysisByVendrReportLayer.show(true);

        // saleAnalysisByVendrReport html 내용 초기화
        $("#saleAnalysisByVendrReport").html('');
    ...
    $scope.getSaleAnalysisByVendrList = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/saleAnalysis/saleAnalysisByVendr/saleAnalysisByVendr/getSaleAnalysisByVendrList.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (!$.isEmptyObject(response.data.data)) {
                    ...
                    if(dataList.length > 0) {
                        var acquireSet = new Set();

                        for (var i = 0; i < dataList.length; i++) {
                            acquireSet.add(dataList[i].vendrNm);
                        }
                        var acquireArr = Array.from(acquireSet);
                        if(($scope.data.acquireNm !== null && $scope.data.acquireNm !== '' && $scope.data.acquireNm !== undefined)
                            || ($scope.data.acquireCd !== null && $scope.data.acquireCd !== '' && $scope.data.acquireCd !== undefined)){
                            if(acquireArr.length > 1) {
                                acquireCnt = acquireArr[0] + "외 " + (acquireArr.length - 1) + "개";
                            }else{
                                acquireCnt = acquireArr[0];
                            }
                        }else{
                            acquireCnt = '전체';
                        }
                    ...
                    var titleHtml          = ''; // 매입처별 매출분석 최상단 html
                    var arrSaleAnalysisListHtml       = ''; // 매입처별 매출분석리스트 html
                    var nextPageHtml       = '<p class="nextPage"></p>'; // 프린트 출력시 다음 페이지로 넘기기 위한 html
                    var arrSaleAnalysisListHeaderHtml = '<table class="w100 mt5 small-text-table">' // 매입처별 매출분석리스트 header html
                        + '<colgroup>'
                        + '<col style="width:4%;">'
                        + '<col style="width:3%;">'
                        + '<col style="width:8%;">'
                    ...
                    for (var i = 0; i < arrSaleAnalysisList.length; i++) {
                        ...
                        for (var j = 0; j < payHistList.length; j++) {
                            ...
                            arrSaleAnalysisListHtml += '<tr class="h25">'
                                ...
                                + '<td class="tc">' + '' + '</td>' // 구분 값 추가예정
                                + '<td class="tc">' + addComma(item.saleUprc) + '</td>'
                                + '<td class="tc">' + addComma(item.splyUprc) + '</td>'
                                + '<td class="tc">' + addComma(item.totSaleQty) + '</td>'
                                + '<td class="tc">' + addComma(item.totSaleAmt) + '</td>'
                                + '<td class="tc">' + addComma(item.totDcAmt) + '</td>'
                                + '<td class="tc">' + addComma(item.realSaleAmt) + '</td>'
                                + '<td class="tc">' + addComma(item.cogs) + '</td>'
                                + '<td class="tc">' + addComma(item.salesProfit) + '</td>'
                                + '</tr>';
                        }
                        ...
                        titleHtml    = '<table class="w100">'
                            ...
                            + '</colgroup>'
                            + '<tr>'
                            + '<td class="tl br0"><p class="bk s20">매입처별 매출분석 </p></td>'
                            + '</tr>'
                            + '<tr>'
                            + '<td class="tl br0" valign="bottom">기간 : ' + srchStartDate + ' ~ ' + srchEndDate + ' 매입처 : ' + acquireCnt + ' 지점 : 전체</td>'
                    ...
                    $('#saleAnalysisByVendrReport').append(arrSaleAnalysisReportList);
    ...
    $scope.print = function () {
        ...
        // add content to it
        var view = document.querySelector('#saleAnalysisByVendrReport');
        doc.append(view);
