/****************************************************************
 *
 * 파일명 : mobileBoardDetail.js
 * 설  명 : (모바일)일반게시판 상세팝업 JavaScript
 *
...
function viewDelAnswer(idx){
    var scope = agrid.getScope('mobileBoardDetailCtrl');
    var params = {};
 ...
 *  팝업 그리드 생성
 */
app.controller('mobileBoardDetailCtrl', ['$scope', '$http', function ($scope, $http) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileBoardDetailCtrl', $scope, $http, false));

    // <-- 검색 호출 -->
    $scope.$on("mobileBoardDetailCtrl", function(event, data) {
        // 파일과 댓글 DIV 값 초기화
        $("#fileContent").html("");
        $("#divComment").html("");
    ...
    $scope.searchBoardDetailAnswer = function() {
        ...
        $scope._postJSONQuery.withOutPopUp("/adi/board/board/board/getBoardDetailAnswerList.sb", params, function(response) {
            ...
            if(list.length > 0){
                ...
                innerHtml += "<colgroup><col width=\"70%\" /><col width=\"30%\" /></colgroup>";
                innerHtml += "<tbody>";
    ...
    $scope.searchBoardDetailAtch = function() {
        ...
        $scope._postJSONQuery.withOutPopUp( "/adi/board/board/board/getBoardDetailAtchList.sb", params, function(response){
            ...
            if(list.length > 0) {
                ...
                innerHtml += "<table class=\"tblType01 mt10\">";
                innerHtml += "<colgroup><col width=\"20%\" /><col width=\"80%\" /></colgroup>";
                innerHtml += "<tbody>";
    ...
    $scope.saveAnswer = function(){
        if($scope.content === "") {
            $scope._popMsg(messages["mobile.boardDetail.contentBlank"]); // 댓글을 입력해주세요.
            return false;
        }
    ...
    $scope.delAnswer = function(data){
        // 해당 댓글을 삭제하시겠습니까?
        $scope._popConfirm(messages["mobile.boardDetail.delConfirmAnswer"], function() {
    ...
    $scope.close = function(){
        ...
        $scope.wjMobileBoardDetailLayer.hide();
    };
