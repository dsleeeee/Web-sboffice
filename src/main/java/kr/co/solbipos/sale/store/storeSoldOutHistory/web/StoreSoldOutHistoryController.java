package kr.co.solbipos.sale.store.storeSoldOutHistory.web;
...
/**
 * @Class Name  : StoreSoldOutHistoryController.java
 ...
 */
@Controller
@RequestMapping("/sale/store/storeSoldOutHistory")
public class StoreSoldOutHistoryController {

    private final SessionService sessionService;
    private final CmmCodeUtil cmmCodeUtil;
    private final DayProdService dayProdService;
    private final StoreSoldOutHistoryService storeSoldOutHistoryService;

    /**
     *  Constructor Injection
     */
    @Autowired
    public StoreSoldOutHistoryController(SessionService sessionService, CmmCodeUtil cmmCodeUtil, DayProdService dayProdService, StoreSoldOutHistoryService storeSoldOutHistoryService) {
        this.sessionService = sessionService;
        this.cmmCodeUtil = cmmCodeUtil;
        this.dayProdService = dayProdService;
        this.storeSoldOutHistoryService = storeSoldOutHistoryService;
    }
     ...
     * @param model
     */
    @RequestMapping(value = "/storeSoldOutHistory/view.sb", method = RequestMethod.GET)
    public String dayProdStoreView(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        return "sale/store/storeSoldOutHistory/storeSoldOutHistory";
    }
     ...
     * @param   response
     * @param   model
     * @param   storeSoldOutHistoryVO
     * @return  String
     * @author  김유승
     * @since   2026.03.03
     */
    @RequestMapping(value = "/storeSoldOutHistory/getSearchSoldOutHistory.sb", method = RequestMethod.POST)
    @ResponseBody
    public Result getSearchSoldOutHistory(HttpServletRequest request, HttpServletResponse response,
                                          Model model, StoreSoldOutHistoryVO storeSoldOutHistoryVO) {

        SessionInfoVO sessionInfoVO = sessionService.getSessionInfo(request);

        List<DefaultMap<Object>> list = storeSoldOutHistoryService.getSearchSoldOutHistory(storeSoldOutHistoryVO, sessionInfoVO);

        return ReturnUtil.returnListJson(Status.OK, list, storeSoldOutHistoryVO);
    }
    /**
     * 매장품절현황 - 엑셀조회
     * @param   request
     ...
     * @since   2026.03.03
     */
    @RequestMapping(value = "/storeSoldOutHistory/getStoreSoldOutHistoryExcelList.sb", method = RequestMethod.POST)
    @ResponseBody
    public Result getStoreSoldOutHistoryExcelList(HttpServletRequest request, HttpServletResponse response,
                                           Model model, StoreSoldOutHistoryVO storeSoldOutHistoryVO) {

        SessionInfoVO sessionInfoVO = sessionService.getSessionInfo(request);

        List<DefaultMap<Object>> list = storeSoldOutHistoryService.getStoreSoldOutHistoryExcelList(storeSoldOutHistoryVO, sessionInfoVO);
