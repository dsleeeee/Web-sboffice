...
app.controller('mobileHqCurrCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileHqCurrCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        $scope._makePickColumns("mobileHqCurrCtrl");
    ...
    $scope.$on("mobileHqCurrCtrl", function (event, data) {
        $scope.searchMobileHqCurrList(true);
        // 기능수행 종료 : 반드시 추가
    ...
    // 현재고현황 리스트 조회
    $scope.searchMobileHqCurrList = function (isPageChk) {
        // 파라미터
        var params     = {};
        ...
        params.vendrCd = $("#mobileHqCurrSelectVendrCd").val();
        ...
        $scope._inquirySub("/mobile/stock/curr/hqCurr/mobileHqCurr/list.sb", params, function (){
            gridShowMsgNoData("mobileHqCurr", $scope.flex, "Y");
        }, true);
    };
    ...
    $scope.mobileHqCurrSelectVendrShow = function () {
        $scope._broadcast('mobileHqCurrSelectVendrCtrl');
    };
    ...
    $scope.hqCurrSelectStorageShow = function () {
        $scope._broadcast('mobileHqCurrSelectStorageCtrl');
    };
    ...
    $scope.excelDownload = function () {
        ...
        $scope._broadcast('mobileHqCurrExcelCtrl',params);
    };

}]);

app.controller('mobileHqCurrExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileHqCurrExcelCtrl', $scope, $http, $timeout, true));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("mobileHqCurrExcelCtrl", function (event, data) {
    ...
    $scope.searchMobilePeriodIoStockExcelList = function (isPageChk) {// 파라미터
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/mobile/stock/curr/hqCurr/mobileHqCurr/excelList.sb", params, function(){
