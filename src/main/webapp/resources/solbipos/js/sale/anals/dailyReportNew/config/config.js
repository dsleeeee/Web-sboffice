	    ...
	    $scope._postJSONQuery.withOutPopUp	(	"/sale/anals/dailyReportNew/config/list.sb",	//영업일보 구성 조회
					        					...
					        					function(response)	{
															            ...
															            for(var i=0; i<configCtrl_2.flex.collectionView.items.length; i++){
															            	...
															            	//console.log('item.cfgCd: ' + item.cfgCd + ' & item.cfgDispSeq: ' + item.cfgDispSeq + ' & item.cfgSelYn: ' + item.cfgSelYn);
...
*/
    ...
    $scope.save = function() {
      ...
      //$scope._save("/sale/anals/dailyReportNew/report/save.sb", params);	//저장(URL, parameter, callback function)
        $scope._save("/sale/anals/dailyReportNew/report/save.sb", params, function(){$scope.getConfigList()});	//저장(URL, parameter, callback function)
    }
    /*
...
//        $scope._save("/sale/anals/dailyReportNew/config/save.sb", params, agrid.getScope('reportCtrl').reportCtrlList()	);	//저장(URL, parameter, callback function) - [영업일보 구성]저장 후, [영업일보]내역 재조회 되게 변경
    ...
    //Grid 순서  START   --------------------------------------------------------------------------------------------------------------------------
    $scope.rowUp = function(){	//위로 옮기기
        var movedRows = 0;
        for(var i=0; i<$scope.flex.collectionView.itemCount; i++){
            var item = $scope.flex.collectionView.items[i];
            //console.log('i:' + i + ' & ' + item.cfgSaleDailyReportNm);
            
            if(i > 0   &&   item.gChkDispSeq){
                if(!$scope.flex.collectionView.items[i - 1].gChkDispSeq){
                    movedRows	= i - 1;
    ...
    $scope.rowDown = function(){	//아래로 옮기기
        var movedRows = 0;
        for(var i=($scope.flex.collectionView.itemCount - 1); i >= 0; i--){
            var item = $scope.flex.collectionView.items[i];
            //console.log('i:' + i + ' & ' + item.cfgSaleDailyReportNm);

            if((i < $scope.flex.collectionView.itemCount - 1)   &&   item.gChkDispSeq){
                if(!$scope.flex.collectionView.items[i + 1].gChkDispSeq){
                    movedRows	= i + 1;
                    var tmpItem = $scope.flex.collectionView.items[movedRows];
    ...
    $scope.rowDown_OLD = function(){	//아래로 옮기기
        var movedRows = 0;
        for(var i=($scope.flex.itemsSource.itemCount - 1); i >= 0; i--){
            var item = $scope.flex.collectionView.items[i];
            console.log('i:' + i + ' & ' + item.cfgSaleDailyReportNm);

            if((i < $scope.flex.itemsSource.itemCount - 1)   &&   item.gChkDispSeq){
                if(!$scope.flex.collectionView.items[i + 1].gChkDispSeq){
                    movedRows	= i + 1;
                    var tmpItem = $scope.flex.collectionView.items[movedRows];
    ...
    $scope.itemFormatter = function (panel, r, c, cell) {
        if (panel.cellType === wijmo.grid.CellType.ColumnHeader) {
            ...
            if ((panel.grid.columnHeaders.rows.length - 1) === r) {
                ...
                if(column.binding === 'cfgSaleDailyReportNm')
                	column.allowSorting = false;	//정렬 방지
                	
                //check that this is a boolean column
                if (column.binding === 'gChk' || column.format === 'checkBox' || column.format === 'checkBoxText') {
	                column.allowSorting = false;	//prevent sorting on click
	                ...
	                //console.log('column.binding:' + column.binding + '   &   column.format:' + column.format);
	                if (column.format === 'checkBoxText') {
	                    cell.innerHTML = '<input id=\"' + column.binding + '\" type=\"checkbox\" class=\"wj-cell-check\" />' + '<label for=\"' + column.binding + '\" class=\"wj-header-label\">' + cell.innerHTML + '</label>';
	                } else {
