...
<%--readonly/>--%>
<button class="btn_skyblue" ng-click="selectUploadProdShow()"><s:message code="selectProdMMoms.upload" /></button>
...
<wj-popup id="wj<c:out value="${param.targetId}"/>Layer" control="wj<c:out value="${param.targetId}"/>Layer" show-trigger="Click" hide-trigger="Click" style="display:none;width:450px;">
  <div class="wj-dialog wj-dialog-columns">
    <div class="wj-dialog-header wj-dialog-header-font">
      > <s:message code="selectProdMMoms.uploadProdSelect"/>
    ...
    <div class="wj-dialog-body" ng-controller="<c:out value="${param.targetId}"/>Ctrl">
      <div class="w100">
        ...
        <div class="mt10 tr">
          ...
          <%-- 조회 --%>
          <button class="btn_skyblue" ng-click="searchProdUpload()"><s:message code="cmm.search" /></button>
          <%-- 삭제 --%>
        ...
        <div class="theGrid mt10" style="height: 400px;">
          <wj-flex-grid
                  ...
                  selection-mode="Row"
                  items-source="data"
                  control="selectUploadProdFlex"
                  initialized="initGrid(s,e)"
                  is-read-only="false"
                  item-formatter="_itemFormatter">

            <!-- define columns -->
            <wj-flex-grid-column header="<s:message code="selectProdMMoms.prodCd"/>" binding="prodCd" width="120" align="center" is-read-only="true"></wj-flex-grid-column>
            <wj-flex-grid-column header="<s:message code="selectProdMMoms.prodNm"/>" binding="prodNm" width="*" align="left" is-read-only="true"></wj-flex-grid-column>
          </wj-flex-grid>
        </div>
        <%--//위즈모 테이블--%>
      </div>
...
<c:import url="/WEB-INF/view/common/popup/selectUploadProdExcelUpload.jsp">
...
<script type="text/javascript">
  ...
  app.controller('${param.targetId}Ctrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    // 조회
    $scope.searchProdUpload = function () {
      // 파라미터
      var params = {};

      $scope._inquirySub("/iostock/cmm/iostockCmm/getSelectUploadProdList.sb", params, function () {
        // $scope.searchFg = "Y";

        // 저장기능 수행후 재조회
        var preTargetId = targetId.substring(0, targetId.indexOf('SelectUploadProd'));
    ...
    $scope.sampleDownload = function () {
      ...
      $timeout(function () {
        wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.selectUploadProdFlex, {
          includeColumnHeaders: true,
          includeCellStyles   : false,
          includeColumns      : function (column) {
            return column.visible;
          }
        }, '업로드상품_'+getToday()+'.xlsx', function () {
          $timeout(function () {
    ...
    $scope.excelUpload = function () {
      ...
      $("#selectUploadProdExcelUploadFile").trigger('click');
      $("#selectUploadProdExcelUploadTargetId").text(targetId);
    };
    // <-- //엑셀업로드 -->
    ...
    $scope.uploadDeleteSave = function () {
      ...
      $scope._postJSONSave.withPopUp("/iostock/cmm/iostockCmm/getSelectUploadProdExcelUploadDeleteAll.sb", params, function(){
        // 조회
        $scope.searchProdUpload();
      });
    };
