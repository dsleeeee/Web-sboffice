/****************************************************************
 *
 * 파일명 : timeProdBenson.js
 * 설  명 : (벤슨) 상품매출분석 > 상품별시간대매출 JavaScript
 *
 ...
 *  상품별 시간대 매출 그리드 생성
 */
app.controller('timeProdBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeProdBensonCtrl', $scope, $http, false));
    ...
    $scope.initGrid = function (s, e) {
        ...
        // 첫째줄 헤더 생성
        var dataItem = {};
        dataItem.saleDate = messages["timeProdBenson.saleDate"];
        dataItem.yoil = messages["timeProdBenson.yoil"];
        dataItem.saleYm = messages["timeProdBenson.saleYm"];
        dataItem.dayFrom = messages["timeProdBenson.dayFrom"];
        dataItem.dayTo = messages["timeProdBenson.dayTo"];
        dataItem.branchNm = messages["timeProdBenson.branchNm"];
        dataItem.storeCd = messages["timeProdBenson.storeCd"];
        dataItem.storeNm = messages["timeProdBenson.storeNm"];
        dataItem.lClassCd = messages["timeProdBenson.lClassCd"];
        dataItem.lClassNm = messages["timeProdBenson.lClassNm"];
        dataItem.mClassCd = messages["timeProdBenson.mClassCd"];
        dataItem.mClassNm = messages["timeProdBenson.mClassNm"];
        dataItem.sClassCd = messages["timeProdBenson.sClassCd"];
        dataItem.sClassNm = messages["timeProdBenson.sClassNm"];
        dataItem.prodCd = messages["timeProdBenson.prodCd"];
        dataItem.sideProdCd = messages["timeProdBenson.sideProdCd"];
        dataItem.selTypeFg = messages["timeProdBenson.selTypeFg"];
        dataItem.sideProdNm = messages["timeProdBenson.sideProdNm"];
        dataItem.totSaleQty = messages["timeProdBenson.tot"];
        dataItem.totRealSaleAmt = messages["timeProdBenson.tot"];
    ...
    $scope.$on("timeProdBensonCtrl", function (event, data) {
        $scope.searchTimeProdBensonList();
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    // 리스트 조회
    $scope.searchTimeProdBensonList = function () {
        ...
        if($scope.startTime*1 > $scope.endTime*1){ // *1하는이유 : Time들이 String이라 int로 바꿀라고
            $scope._popMsg(messages["timeProdBenson.startEnd"]); // 검색 시작 시간대가 검색 종료 시간대보다 큽니다.
            return false;
        }
        ...
        params.storeCds = $("#timeProdBensonStoreCd").val();
        params.prodCds = $("#timeProdBensonSelectCd").val();
        ...
        $.postJSON("/sale/benson/timeProdBenson/timeProdBenson/getTimeProdBensonList.sb", params, function (response){
            var grid = $scope.flex;
    ...
    $scope.timeProdBensonSelectShow = function () {
        $scope._broadcast('timeProdBensonSelectCtrl');
    };
    ...
    $scope.excelDownload = function (excelType) {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('timeProdBensonExcelCtrl', params);
        });
    };
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('timeProdBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeProdBensonExcelCtrl', $scope, $http, false));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("timeProdBensonExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/benson/timeProdBenson/timeProdBenson/getTimeProdBensonExcelList.sb", params, function(response) {
