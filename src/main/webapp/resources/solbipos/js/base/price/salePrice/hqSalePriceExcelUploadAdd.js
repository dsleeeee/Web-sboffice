/****************************************************************
 *
 * 파일명 : hqSalePriceExcelUploadAdd.js
 * 설  명 : 본사판매가관리 엑셀업로드 팝업 JavaScript
 *
...
/**
 *  본사판매가관리 엑셀업로드 팝업 조회 그리드 생성
 */
app.controller('hqSalePriceExcelUploadAddCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('hqSalePriceExcelUploadAddCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("hqSalePriceExcelUploadAddCtrl", function(event, data) {
    ...
    $scope.excelUpload = function () {
        ...
        // 선택한 파일이 있으면
        if ($('#hqSalePriceExcelUpFile')[0].files[0]) {
            var file          = $('#hqSalePriceExcelUpFile')[0].files[0];
            ...
            if (fileExtension.toLowerCase() === '.xlsx' || fileExtension.toLowerCase() === '.xlsm') {
                ...
                // $timeout(function () {
                //     var flex = $scope.flex;
                //     wijmo.grid.xlsx.FlexGridXlsxConverter.loadAsync(flex, $('#hqSalePriceExcelUpFile')[0].files[0], {includeColumnHeaders: true}
                //         , function () {
                //             $timeout(function () {
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            ...
                            renameKey(item, '현재판매가', 'hqSaleUprc');
                            renameKey(item, '변경판매가', 'saleUprc');
                            renameKey(item, '현재내점판매가', 'hqStinSaleUprc');
                            renameKey(item, '변경내점판매가', 'stinSaleUprc');
                            renameKey(item, '현재배달판매가', 'hqDlvrSaleUprc');
                            renameKey(item, '변경배달판매가', 'dlvrSaleUprc');
                            renameKey(item, '현재포장판매가', 'hqPackSaleUprc');
                            renameKey(item, '변경포장판매가', 'packSaleUprc');
    ...
    $scope.save = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/price/salePrice/salePriceExcelUpload/getSalePriceExcelUploadCheckSave.sb', /* 통신할 URL */
