/****************************************************************
 *
 * 파일명 : mobileHqPeriodIoStock.js
 * 설  명 : (모바일) 재고현황 > 기간수불현황 JavaScript
 *
...
app.controller('mobileHqPeriodIoStockCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileHqPeriodIoStockCtrl', $scope, $http, true));
    ...
    $scope.mobileHqPeriodIoStockSelectVendrShow = function () {
        $scope._broadcast('mobileHqPeriodIoStockSelectVendrCtrl');
    };

    // grid 초기화 : 생성되기전 초기화되면서 생성된다
    $scope.initGrid = function (s, e) {
        // picker 사용시 호출 : 미사용시 호출안함
        $scope._makePickColumns("mobileHqPeriodIoStockCtrl");
    ...
    $scope.$on("mobileHqPeriodIoStockCtrl", function (event, data) {

        $scope.searchMobileHqPeriodIoStockList(true);
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    //다른 컨트롤러의 broadcast 받기(페이징 초기화)
    $scope.$on("mobileHqPeriodIoStockCtrlSrch", function (event, data) {
        $scope.comboArray =  $scope.comboArrayForSrc; // ?????????????
        $scope.searchMobileHqPeriodIoStockList(false);
    ...
    // 매장기간수불 리스트 조회
    $scope.searchMobileHqPeriodIoStockList = function (isPageChk) {
        // 파라미터
        var params     = {};
        ...
        $scope._inquirySub("/mobile/stock/status/periodIoStock/mobilePeriodIoStock/periodIoStockList.sb", params,function (){
            gridShowMsgNoData("mobileHqPeriodIoStock", $scope.flex, "Y");
        },true);
    };
    ...
    $scope.excelDownload = function () {
        ...
        $scope._broadcast('mobileHqPeriodIoStockExcelCtrl',params);
    };
}]);

app.controller('mobileHqPeriodIoStockExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('mobileHqPeriodIoStockExcelCtrl', $scope, $http, $timeout, true));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("mobileHqPeriodIoStockExcelCtrl", function (event, data) {
        if(data != undefined && $scope.isSearch) {
            $scope.searchMobileHqPeriodIoStockExcelList(true);
            // 기능수행 종료 : 반드시 추가
            event.preventDefault();
        } else{
    ...
    // 전체 엑셀 리스트 조회
    $scope.searchMobileHqPeriodIoStockExcelList = function (isPageChk) {// 파라미터
