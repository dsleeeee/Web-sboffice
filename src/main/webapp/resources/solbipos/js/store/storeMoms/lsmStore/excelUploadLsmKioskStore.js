/****************************************************************
 *
 * 파일명 : excelUploadLsmKioskStore.js
 * 설  명 : LSM사용매장조회 > 키오스크탭 엑셀업로드 JavaScript
 *
 ...
 *  엑셀업로드 그리드 생성
 */
app.controller('excelUploadLsmKioskStoreCtrl', ['$scope', '$http','$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('excelUploadLsmKioskStoreCtrl', $scope, $http, true));
    ...
    $scope.excelFileChanged = function () {
        ...
        if ($('#lsmKioskStoreExcelUpFile')[0].files[0]) {
            // 엑셀업로드 호출
            $scope.excelUpload();
        }
    };
    ...
    $scope.excelUpload = function () {
        ...
        // 선택한 파일이 있으면
        if ($('#lsmKioskStoreExcelUpFile')[0].files[0]) {
            var file = $('#lsmKioskStoreExcelUpFile')[0].files[0];
            ...
            if (fileExtension.toLowerCase() === '.xlsx' || fileExtension.toLowerCase() === '.xlsm') {
                ...
                // $timeout(function () {
                //     var flex = $scope.flex;
                //     wijmo.grid.xlsx.FlexGridXlsxConverter.loadAsync(flex, $('#lsmKioskStoreExcelUpFile')[0].files[0], {includeColumnHeaders: true}
                //         , function (workbook) {
                //             $timeout(function () {
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            ...
                            renameKey(item, '키오스크그룹코드', 'tuClsType');
                            renameKey(item, '키오스크카테고리코드', 'tuClsCd');
    ...
    $scope.excelUploadToJsonConvert = function () {
        ...
        $timeout(function () {
            // 데이터 임시 저장
            $scope.kioskTempInsert(jsonData);
        }, 10);
    };

    // 데이터 임시 저장
    $scope.kioskTempInsert = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/store/storeMoms/lsmStore/lsmStore/getKioskKeyTempInsert.sb', /* 통신할 URL */
    ...
    // 엑셀 업로드 데이터 중 미사용/단종 상품 유무 확인
    $scope.getKioskChkUseYn = function(){
        // 파라미터
        var params = {};

        // 엑셀 업로드 데이터 중 미사용/단종 상품 유무 확인
        $scope._postJSONQuery.withOutPopUp("/store/storeMoms/lsmStore/lsmStore/getKioskChkUseYn.sb", params, function(response){
            ...
            // 상품 수 확인
            $scope.getKioskChkProdCnt();
        });
    };

    // 엑셀 업로드 데이터 중 카테고리별 상품수 확인
    $scope.getKioskChkProdCnt = function(){
        // 파라미터
        var params = {};

        // 엑셀 업로드 데이터 중 카테고리별 상품수 확인
        $scope._postJSONQuery.withOutPopUp("/store/storeMoms/lsmStore/lsmStore/getKioskChkProdCnt.sb", params, function(response){
            if (response.data.data.list === 0 || response.data.data.list === null) {
            } else {
                var popMsg = response.data.data.list; //
                $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
            ...
            // 상품 수 확인
            $scope.deleteKioskKey();
        });
    };

    // 키오스크 카테고리코드 전체 DELETE
    $scope.deleteKioskKey = function(){
        // 파라미터
        var params = {};

        // 키오스크 카테고리코드 전체 DELETE
        $scope._postJSONSave.withOutPopUp("/store/storeMoms/lsmStore/lsmStore/getDeleteKioskKey.sb", params, function(){
            // 엑셀 업로드 데이터 중 LSM '사용'인 데이터 수 조회
            $scope.getKioskKeyCnt();
        });
    };

    // 엑셀 업로드 데이터 중 LSM '사용'인 데이터 수 조회
    // $scope.getKioskKeyCnt = function(){
    //     // 파라미터
    //     var params = {};
    //
    //     // 엑셀 업로드 데이터 중 LSM '사용'인 데이터 수 조회
    //     $scope._postJSONSave.withOutPopUp("/store/storeMoms/lsmStore/lsmStore/getKioskKeyCnt.sb", params, function(response){
    //         var result = response.data.data;
    //
    //         // 데이터 저장
    //         $scope.getInsertKioskKey(result);
    //     });
    // };

    // 데이터 저장
    $scope.getKioskKeyCnt = function (result) {

        $scope.totalRows = result;
        ...
        $http({
            method : 'POST', //방식
            url    : '/store/storeMoms/lsmStore/lsmStore/getInsertKioskKey.sb', /* 통신할 URL */
            data   : params, /* 파라메터로 보낼 데이터 : @requestBody */
            params : sParam,
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
