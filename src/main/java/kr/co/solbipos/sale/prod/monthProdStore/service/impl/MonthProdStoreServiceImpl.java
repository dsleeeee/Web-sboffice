...
public class MonthProdStoreServiceImpl implements MonthProdStoreService {
    ...
    public List<DefaultMap<Object>> getMonthProdStoreList(MonthProdStoreVO monthProdStoreVO, SessionInfoVO sessionInfoVO) {
        ...
        // 분류 array 값 세팅
        if (monthProdStoreVO.getProdClassCd() != null && !"".equals(monthProdStoreVO.getProdClassCd())) {
            String[] prodCdList = monthProdStoreVO.getProdClassCd().split(",");
            monthProdStoreVO.setArrProdClassCd(prodCdList);
        }

        // 상품 array 값 세팅
        if(!StringUtil.getOrBlank(monthProdStoreVO.getProdCds()).equals("")) {
            ProdVO prodVO = new ProdVO();
            prodVO.setArrSplitProdCd(CmmUtil.splitText(monthProdStoreVO.getProdCds(), 3900));
            monthProdStoreVO.setProdCdQuery(popupMapper.getSearchMultiProdRtn(prodVO));
        }
