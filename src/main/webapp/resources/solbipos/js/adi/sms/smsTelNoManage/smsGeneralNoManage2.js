...
app.controller('smsGeneralNoManage2Ctrl', ['$scope', '$http', function ($scope, $http) {
    ...
    $("#funcSaveSmsGeneralNoManage2").click(function(e){
        ...
        $scope._popMsg(messages["smsGeneralNoManage2.popMsg"], function() {
                ...
                if (nvl($scope.flex.collectionView.items[i].addProcFg, "") !== nvl($scope.flex.collectionView.items[i].backAddProcFg, "")) {
                    ...
                    if ($scope.flex.collectionView.items[i].addProcFg === "2" || $scope.flex.collectionView.items[i].addProcFg === "3") {
                        if (nvl($scope.flex.collectionView.items[i].telDt, "") === "") {
                            ...
                            $scope._popMsg(rowContent + messages["smsGeneralNoManage2.returnTelDt"]);
                            return false;
                        }
                    }
            ...
            $scope._popConfirm(messages["cmm.choo.save"], function() {
                ...
                for (var groupKey in addProcFgCntMap) {
                    ...
                    var groupLimit = keyParts[2] === "0" ? 2 : 5;
                ...
                if (addProcFgCntOverMsg !== '') {
                    $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                    // 처리구분 완료 가능 건수를 초과했습니다. (휴대폰번호 2건, 유선번호 5건까지 가능)
                ...
                // 나머지(기존 시스템과의 충돌 등)는 저장 후 서버에서 최종 검증(위반 시 롤백)
