/****************************************************************
 *
 * 파일명 : timeSaleStoreBenson.js
 * 설  명 : (벤슨) 간소화화면 > 시간대매출(매장) JavaScript
 *
 ...
 *  시간대매출(매장) 그리드 생성
 */
app.controller('timeSaleStoreBensonCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeSaleStoreBensonCtrl', $scope, $http, false));
    ...
    $scope.initGrid = function (s, e) {
        ...
        // 첫째줄 헤더 생성
        var dataItem = {};
        dataItem.saleDate = messages["timeSaleStoreBenson.saleDate"];
        dataItem.storeCd = messages["timeSaleStoreBenson.storeCd"];
        dataItem.storeNm = messages["timeSaleStoreBenson.storeNm"];
        // 시간대별 컬럼 생성
        for (var i = 0; i < 10; i++) {
    ...
    // <-- 검색 호출 -->
    $scope.$on("timeSaleStoreBensonCtrl", function (event, data) {
        $scope.searchTimeSaleStoreBensonList();
        event.preventDefault();
    });

    $scope.searchTimeSaleStoreBensonList = function () {
        ...
        params.storeCds = $("#timeSaleStoreBensonStoreCd").val();
        // '전체' 일때
        ...
        $.postJSON("/sale/benson/timeSaleStoreBenson/timeSaleStoreBenson/getTimeSaleStoreBensonList.sb", params, function (response){
            var grid = $scope.flex;
    ...
    $scope.excelDownload = function (excelType) {
        ...
        if(params.excelType === '1') {
            ...
            $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
                $scope._broadcast('timeSaleStoreBensonExcelCtrl', params);
            });
        }else{
            // 분할 엑셀다운로드 사용자 제한 체크
 ...
 *  엑셀다운로드 그리드 생성
 */
app.controller('timeSaleStoreBensonExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('timeSaleStoreBensonExcelCtrl', $scope, $http, false));
    ...
    // <-- 검색 호출 -->
    $scope.$on("timeSaleStoreBensonExcelCtrl", function (event, data) {
        if(data.excelType === '1') {
    ...
    $scope.searchExcelList = function (params) {
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/benson/timeSaleStoreBenson/timeSaleStoreBenson/getTimeSaleStoreBensonExcelList.sb", params, function(response) {
                ...
                function delay(x){
                    return new Promise(function(resolve, reject){
                        ...
                        $.postJSON("/sale/benson/prodSaleDayStoreBenson/prodSaleDayStoreBenson/getDivisionExcelDownloadSaveUpdate.sb", params, function(response){
                            ...
                            // ajax 통신 설정
                            $.postJSON('/sale/benson/timeSaleStoreBenson/timeSaleStoreBenson/getTimeSaleStoreBensonExcelDivisionList.sb', params, function(response) {
