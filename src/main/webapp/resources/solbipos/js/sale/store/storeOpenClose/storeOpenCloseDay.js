...
app.controller('storeOpenCloseDayCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.excelDownloadDay = function () {
        ...
        $timeout(function () {
            ...
            }, messages["storeOpenCloseMoms.storeOpenCloseMoms"] + messages["storeOpenClose.day"] + '_' + $scope.flex.rows[0].dataItem.saleDate + '_' + getCurDateTime() + '.xlsx', function () {
                $timeout(function () {
...
app.controller('storeOpenCloseDayDtlCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.excelDownloadDay = function () {
        ...
        $timeout(function () {
            ...
            }, messages["storeOpenCloseMoms.storeOpenCloseMoms"] + messages["storeOpenClose.day"] + '_' + $scope.flex.rows[0].dataItem.saleDate + '_(상세)' + getCurDateTime() + '.xlsx', function () {
                $timeout(function () {
                    $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
                }, 10);
            });
