/****************************************************************
 *
 * 파일명 : excelUploadInOutStockProd.js
 * 설  명 : 매입전표등록 > 상품추가/변경 엑셀업로드 JavaScript
 *
 ...
 *  엑셀업로드 그리드 생성
 */
app.controller('excelUploadInOutStockProdCtrl', ['$scope', '$http','$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('excelUploadInOutStockProdCtrl', $scope, $http, true));
    ...
    // <-- 검색 호출 -->
    // $scope.$on("excelUploadInOutStockProdCtrl", function(event, data) {
    //
    ...
    $scope.deleteExl = function (upFg) {
        ...
        $http({
            method : 'POST', //방식
            url    : "/kookmin/acquire/acquireSlipRegist/excelUploadInOutStockProd/1.sb", /* 통신할 URL */
    ...
    $scope.save = function (jsonData) {
        ...
        for (var i = $scope.progressCnt; i < loopCnt; i++) {
            ...
            if (nvl(item.remark, '') !== '' && nvl(item.remark + '', '').getByteLengthForOracle() > 1000) {
                msg = messages["excelUploadMPS.remark"] + messages["excelUploadMPS.overLength"] + " 1000 " + messages["excelUploadMPS.bateLengthInfo"]; // 비고의 데이터 중 문자열의 길이가 너무 긴 데이터가 있습니다. 최대 : 1000
                $scope.valueCheckErrPopup(msg);
                return false;
            }
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/acquire/acquireSlipRegist/excelUploadInOutStockProd/saveExcelUpload.sb', /* 통신할 URL */
    ...
    $scope.saveUpdateProdCd = function () {
        ...
        $http({
            method : 'POST', //방식
            url    : '/kookmin/acquire/acquireSlipRegist/excelUploadInOutStockProd/saveUpdateProdCd.sb', /* 통신할 URL */
            params : params, /* 파라메터로 보낼 데이터 */
            headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
        }).then(function successCallback(response) {
            if ($scope._httpStatusCheck(response, true)) {
