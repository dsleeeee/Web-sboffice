...
<c:set var="baseUrl" value="/stock/adj/hqAdj/hqAdjDtl/"/>

<wj-popup id="wjHqAdjDtlLayer" control="wjHqAdjDtlLayer" show-trigger="Click" hide-trigger="Click" style="display:none;width:900px;">
  <div id="hqAdjDtlLayer" class="wj-dialog wj-dialog-columns" ng-controller="hqAdjDtlCtrl">
    ...
    <div class="wj-dialog-body sc2" style="height: 600px;">
      ...
      <div class="mt20 tr">
        ...
        <button type="button" class="btn_skyblue ml5" id="btnDtlAddProd" ng-click="addProd()" ng-if="btnDtlAddProd">
          <s:message code="hqAdj.dtl.addProd"/></button>
        <%-- 저장 --%>
        <button type="button" class="btn_skyblue ml5" id="btnDtlSave" ng-click="saveHqAdjDtl('')" ng-if="btnDtlSave">
          <s:message code="cmm.save"/></button>
        <%-- 확정 --%>
        <button type="button" class="btn_skyblue ml5" id="btnDtlConfirm" ng-click="confirm()" ng-if="btnDtlConfirm">
          <s:message code="hqAdj.dtl.confirm"/></button>
      </div>
      ...
      <div class="w100 mt10 mb20">
        ...
        <div class="wj-gridWrap" style="height: 400px;">
          <wj-flex-grid
            ...
            <wj-flex-grid-column header="<s:message code="hqAdj.reg.adjQty"/>" binding="adjQty" width="70" align="right" is-read-only="false" data-type="Number" format="n0" aggregate="Sum"></wj-flex-grid-column>
      ...
      <div class="pageNum mt20">
        <%-- id --%>
        <ul id="hqAdjDtlCtrlPager" data-size="10">
        </ul>
      </div>
      <%--//페이지 리스트--%>
    </div>
...
<script type="text/javascript">
  ...
  app.controller('hqAdjDtlCtrl', ['$scope', '$http', function ($scope, $http) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('hqAdjDtlCtrl', $scope, $http, true));
    ...
    $scope.$on("hqAdjDtlCtrl", function (event, data) {
      // 그리드 초기화
    ...
    $scope.procFgCheck = function () {
      ...
      $http({
        ...
        // "complete" code here
        $scope.wjHqAdjDtlLayer.show(true);
    ...
    // 조정상품 리스트 조회
    $scope.searchHqAdjDtlList = function () {
      // 파라미터
      var params       = {};
      params.adjDate   = $scope.adjDate;
      params.seqNo     = $scope.seqNo;
      ...
      $scope._inquiryMain("/stock/adj/hqAdj/hqAdjDtl/list.sb", params);
    };


    // 조정 상품 저장
    $scope.saveHqAdjDtl = function (confirmFg) {
      if (nvl($scope.adjTitle, '') === '') {
        var msg = messages["hqAdj.reg.adjTitle"] + messages["cmm.require.text"];
        $scope._popMsg(msg);
        return false;
      }
      ...
      $scope._save("/stock/adj/hqAdj/hqAdjDtl/save.sb", params, function () {
    ...
    $scope.saveRegistCallback = function (confirmFg) {
      ...
      if (confirmFg === 'Y') {
        $scope.wjHqAdjDtlLayer.hide(true);
      }
    };
    ...
    $scope.confirm = function () {
      var msg = messages["hqAdj.dtl.confirmMsg"]; // 확정하시겠습니까?
      s_alert.popConf(msg, function () {
        $scope.saveHqAdjDtl('Y');
        return false;
      });
    };
