/****************************************************************
 *
 * 파일명 : stockDisuseDtl.js
 * 설  명 : 국민대 > 재고관리 > 재고폐기 > 폐기상세 팝업 JavaScript
 *
...
app.controller('stockDisuseDtlCtrl', ['$scope', '$http', function ($scope, $http) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('stockDisuseDtlCtrl', $scope, $http, true));
    ...
    $scope.$on("stockDisuseDtlCtrl", function (event, data) {
        // 그리드 초기화
        ...
        $http({
            ...
            if ($scope._httpStatusCheck(response, true)) {
                ...
                if (!$.isEmptyObject(response.data.data)) {
                    ...
                    $("#stockDisuseDtlVendrCd").val(response.data.data.disuseVendrCd);
                    $("#stockDisuseDtlVendrNm").val('[' + response.data.data.disuseVendrCd + ']' + response.data.data.vendrNm);
                    $("#stockDisuseDtlStoreCd").val(response.data.data.disuseStoreCd);
                    $("#stockDisuseDtlStoreNm").val('[' + response.data.data.disuseStoreCd + ']' + response.data.data.storeNm);
                    $("#stockDisuseDtlStorebtnCancelStoreCd").attr("disabled", true);
                    $("#stockDisuseDtlStoreNm").attr("disabled", true);
                    $scope.stockDisuseDtlVendrNmDisabled  = true; // 반출 거래처선택 모듈 input
                    $scope.stockDisuseDtlVendrBtnDisabled = true; // 반출 선택취소
                }
            }
            ...
            // "complete" code here
            $scope.wjStockDisuseDtlLayer.show(true);
    ...
    // 폐기상품 리스트 조회
    $scope.getSearchDisuseDtlList = function () {
        // 파라미터
        var params        = {};
        params.disuseDate = $scope.disuseDate;
        params.seqNo      = $scope.seqNo;
        ...
        $scope._inquiryMain("/kookmin/stock/stockDisuse/stockDisuseDtl/getSearchDisuseDtlList.sb", params);
    };
    ...
    $scope.saveDisuseDtl = function (confirmFg) {
        ...
        $scope._save("/kookmin/stock/stockDisuse/stockDisuseDtl/saveDisuseDtl.sb", params, function () {
    ...
    $scope.saveRegistCallback = function (confirmFg) {
        ...
        if (confirmFg === 'Y') {
            $scope.wjStockDisuseDtlLayer.hide(true);
        }
    };
    ...
    $scope.addProd = function () {
        ...
        $scope._broadcast('stockDisuseRegistCtrl', params);
    };

    // 거래처선택 모듈 팝업 사용시 정의
    // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
    // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
    $scope.stockDisuseDtlVendrShow = function () {
        $scope._broadcast('stockDisuseDtlVendrCtrl');
    };
