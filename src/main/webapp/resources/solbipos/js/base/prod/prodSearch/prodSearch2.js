 ...
 * 상품정보관리 그리드 생성
 */
app.controller('prodctrl2', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodctrl2', $scope, $http, false));
    ...
    // 확장조회 콤보박스
    $scope._setComboData('srchComboData', srchComboData);
    ...
    // 상품정보관리 그리드 조회
    $scope.$on("prodctrl2", function(event, data) {
    ...
    $scope.searchProdList = function(){
        ...
        params.tuKey = $scope.tuKey.selectedValue;
        params.kiosk = $scope.kiosk.selectedValue;
        params.sdsel = $scope.sdsel.selectedValue;
        params.bsImg = $scope.bsImg.selectedValue;
        params.kioskImg = $scope.kioskImg.selectedValue;
        params.didImg = $scope.didImg.selectedValue;
        params.proImg = $scope.proImg.selectedValue;
        params.toImg = $scope.toImg.selectedValue;
        params.dlvrNm = $scope.dlvrNm.selectedValue;
        params.prtCnt = $scope.prtCnt.selectedValue;
        params.orgNm = $scope.orgNm.selectedValue;
        params.algiNm = $scope.algiNm.selectedValue;
        params.sdyNm = $scope.sdyNm.selectedValue;
        params.disNm = $scope.disNm.selectedValue;
        params.proNm = $scope.proNm.selectedValue;

        // 조회 수행 : 조회URL, 파라미터, 콜백함수, 팝업결과표시여부
        $scope._inquiryMain("/base/prod/prodSearch/prodSearch2/list2.sb", params, function(){
    ...
    $scope.registProdStore = function(prodInfo) {
        ...
        $scope.prodStoreRegistLayer.show(true, function (s) {
            ...
            $scope._pageView('prodctrl2', $scope.currPageNo);
        });
    };
    ...
    $scope.excelDownload = function () {
        ...
        $timeout(function () {
            wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
                ...
                messages["prodSearch2.prodSearch2"] + '_' + getCurDateTime() +'.xlsx', function () {
                    $timeout(function () {
...
app.controller('totalExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('totalExcelCtrl', $scope, $http, false));
    ...
    $scope.searchProdExcelList = function (data) {
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/base/prod/prodSearch/prodSearch2/getProdExcelList2.sb", params, function() {
            ...
            $timeout(function () {
                ...
                }, messages["prodSearch2.prodSearch2"] + excelGubun + getCurDateTime() +'.xlsx', function () {
                    $timeout(function () {
                        $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
                    }, 10);
                });
