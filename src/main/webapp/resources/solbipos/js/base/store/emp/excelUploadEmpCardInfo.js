/****************************************************************
 *
 * 파일명 : excelUploadEmpCardInfo.js
 * 설  명 : 사원카드정보관리 엑셀업로드 JavaScript
 *
...
app.controller('excelUploadEmpCardInfoCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('excelUploadEmpCardInfoCtrl', $scope, $http, true));
    ...
    $scope.addRow = function () {
        ...
        // 샘플양식에 값 넣기
        var params = {};
        params.employeeCardNo = '0000000000001';
        params.employeeNo = '090001';
        params.employeeNm = '홍길동';
        params.divNm = '경영관리본부';
        params.deptNm = '총무부';
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            ...
                            renameKey(item, '소속명', 'divNm');
                            renameKey(item, '부서명', 'deptNm');
                            renameKey(item, '직위명', 'positionNm');
                            renameKey(item, '카드사용구분', 'useFg');
    ...
    $scope.save = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/store/emp/cardInfo/saveEmpCardInfo.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    var parentScope = agrid.getScope('empCardInfoCtrl');
