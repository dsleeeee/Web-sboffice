/****************************************************************
 *
 * 파일명 : storeOpenCloseBensonMonth.js
 * 설  명 : (벤슨) 매장분석 > 매장 오픈/마감 현황 - 월별 JavaScript
 *
...
app.controller('storeOpenCloseBensonMonthTimeCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeOpenCloseBensonMonthTimeCtrl', $scope, $http, $timeout, true));
    ...
    $scope.$on("storeOpenCloseBensonMonthTimeCtrl", function (event, data) {
        ...
        params.storeCds = $("#storeOpenCloseBensonMonthStoreCd").val();
        ...
        var scope = agrid.getScope("storeOpenCloseBensonMonthCtrl");
        // 월별 조회
        scope.searchStoreOpenCloseBensonMonth(params);
...
/** 매장 오픈/마감 현황 - 월별 controller */
app.controller('storeOpenCloseBensonMonthCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeOpenCloseBensonMonthCtrl', $scope, $http, $timeout, true));
        ...
        s.addEventListener(s.hostElement, 'mousedown', function (e) {
            ...
            if (ht.cellType === wijmo.grid.CellType.Cell) {
                ...
                if (col.binding === "yyyymmdd") {
                    ...
                    $scope._broadcast('storeOpenCloseBensonMonthDtlCtrl', params);
                }
            }
        });
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("storeOpenCloseBensonMonthCtrl", function (event, data) {

        event.preventDefault();
    });

    // 월별 조회
    $scope.searchStoreOpenCloseBensonMonth = function(data){
        ...
        $.postJSON("/sale/benson/storeOpenCloseBenson/storeOpenCloseBenson/getStoreOpenCloseBensonMonthList.sb", params, function(response) {
            var grid = $scope.flex;
    ...
    $scope.excelDownloadMonth = function () {
        ...
        $timeout(function () {
            ...
            }, messages["storeOpenCloseBenson.storeOpenCloseBenson"] + messages["storeOpenCloseBenson.month"] + '_' + $scope.flex.rows[0].dataItem.startMonth + '_' +  $scope.flex.rows[0].dataItem.endMonth + '_' + getCurDateTime() + '.xlsx', function () {
                $timeout(function () {
...
/** 매장 오픈/마감 현황 - 월별 상세 controller */
app.controller('storeOpenCloseBensonMonthDtlCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeOpenCloseBensonMonthDtlCtrl', $scope, $http, $timeout, true));
    ...
    $scope.initGrid = function (s, e) {
        ...
        // 첫째줄 헤더 생성
        var dataItem			= {};
        dataItem.saleDate		= messages["storeOpenCloseBenson.date"];
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("storeOpenCloseBensonMonthDtlCtrl", function (event, data) {

        // 월별 조회
        $scope.searchStoreOpenCloseBensonMonthDtl(data);
        event.preventDefault();
    });

    // 월별 조회
    $scope.searchStoreOpenCloseBensonMonthDtl = function(data){
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $.postJSON("/sale/benson/storeOpenCloseBenson/storeOpenCloseBenson/getStoreOpenCloseBensonMonthDtlList.sb", params, function(response) {
            var grid = $scope.flex;
            grid.itemsSource = response.data.list;
            grid.itemsSource.trackChanges = true;
        }, function(response) {
    ...
    $scope.excelDownloadMonth = function () {
        ...
        $timeout(function () {
            ...
            }, messages["storeOpenCloseBenson.storeOpenCloseBenson"] + messages["storeOpenCloseBenson.month"] + '_' + $scope.flex.rows[0].dataItem.startMonth + '_' +  $scope.flex.rows[0].dataItem.endMonth  + '_(상세)' + getCurDateTime() + '.xlsx', function () {
                $timeout(function () {
                    $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
                }, 10);
            });
