/****************************************************************
 *
 * 파일명 : storeOptionVal.js
 * 설  명 : 다국어관리(키오스크/사이드/옵션)(매장) - 옵션(옵션명) 탭 JavaScript
 *
 ...
 * 옵션(옵션명) 조회 그리드 생성
 */
app.controller('storeOptionValCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeOptionValCtrl', $scope, $http, true));
    ...
    $scope.$on("storeOptionValCtrl", function (event, data) {
    ...
    $scope.getOptionValList = function () {
        ...
        // 옵션명(옵션명) 탭 리스트 조회
        $scope._inquiryMain("/base/multilingual/storeKioskSideOption/getStoreOptionValList.sb", params);
    };
    ...
    $scope.saveRow = function(){
        $scope._popConfirm(messages["cmm.choo.save"], function() {
            ...
            $scope._save("/base/multilingual/storeKioskSideOption/saveStoreOptionVal.sb", params, function(result) {
                // 재조회
                $scope.getOptionValList();
            });
        });
    ...
    $scope.excelDownload = function(){
        ...
        $scope.$broadcast('loadingPopupActive', messages["cmm.progress"]); // 데이터 처리중 메시지 팝업 열기
            $timeout(function () {
                ...
                }, messages["storeKioskSideOption.optionVal"]  + '_' +  getCurDateTime() + '.xlsx', function () {
                    $timeout(function () {
    ...
    $scope.sampleDownload = function () {
        ...
        var vScope = agrid.getScope('storeOptionValExcelCtrl');
 ...
 */
app.controller('storeOptionValExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeOptionValExcelCtrl', $scope, $http, false));
    ...
    $scope.sampleDownload = function (data) {
        ...
        $scope._inquiryMain("/base/multilingual/storeKioskSideOption/getStoreOptionValList.sb", params, function (){
            ...
            $timeout(function()	{
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(	$scope.flex,
                    ...
                    messages["storeKioskSideOption.optionVal"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/storeKioskSideOption/saveStoreOptionVal.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('storeOptionValCtrl').getOptionValList();
                }
            }
