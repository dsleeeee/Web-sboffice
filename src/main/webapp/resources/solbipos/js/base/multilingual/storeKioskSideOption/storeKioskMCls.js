/****************************************************************
 *
 * 파일명 : storeKioskMCls.js
  * 설  명 : 다국어관리(키오스크/사이드/옵션)(매장) - 키오스크중분류(카테고리명) 탭 JavaScript
 *
...
/**
 * 키오스크중분류(카테고리명) 조회 그리드 생성
 */
app.controller('storeKioskMClsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeKioskMClsCtrl', $scope, $http, true));

    // 조회조건 콤보박스 데이터 Set
    $scope._setComboData("posNo2", kioskPos2List); // 키오스크용 포스 목록(중분류 사용 포스만 조회)
    ...
    $scope.initGrid = function (s, e) {
        ...
        s.cellEditEnded.addHandler(function (s, e) {
            if (e.panel === s.cells) {
                ...
                // 값 변경시 체크박스 체크
                if (col.binding === "tuMClsEnNm" || col.binding === "tuMClsCnNm" || col.binding === "tuMClsJpNm") {
                    $scope.checked(item);
                }
            }
    ...
    $scope.$on("storeKioskMClsCtrl", function (event, data) {
        $scope.getKioskMClsList();
    ...
    $scope.setTuClsDropdownList = function () {
        ...
        $http({
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (!$.isEmptyObject(response.data.data.list) && response.data.data.list.length > 0) {
                    ...
                    $scope._setComboData("tuClsType2", [{name: "전체", value: ""}].concat(comboArray));
                    //$scope.tuClsTypeDataMap = new wijmo.grid.DataMap(comboArray, 'value', 'name');

                } else {
                    var comboArrayAll = [];
                    comboArrayAll.unshift({name: "전체", value: ""});
                    $scope._setComboData("tuClsType2", comboArrayAll);
                }
            }
    ...
    // 조회
    $scope.getKioskMClsList = function () {
        ...
        $scope._inquirySub("/base/multilingual/storeKioskSideOption/getStoreKioskMClsList.sb", params);
    };
    ...
    $scope.saveRow = function () {
        $scope._popConfirm(messages["cmm.choo.save"], function () {
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                if (nvl(item.tuMClsEnNm + '', '').getByteLengthForOracle() > 50) {
                ...
                if (nvl(item.tuMClsCnNm + '', '').getByteLengthForOracle() > 50) {
                ...
                if (nvl(item.tuMClsJpNm + '', '').getByteLengthForOracle() > 50) {
            ...
            $scope._save("/base/multilingual/storeKioskSideOption/saveStoreKioskMCls.sb", params, function (result) {
                // 재조회
                $scope.getKioskMClsList();
            });
        });
   ...
   $scope.sampleDownload = function () {
       ...
       var vScope = agrid.getScope('storeKioskMClsExcelCtrl');
       vScope.sampleDownload($scope.srchPosNo2Combo.selectedValue, $scope.srchTuClsType2Combo.selectedValue, $scope.srchTuClsCd2Combo.selectedValue);
   };
    ...
    $scope.excelUpload = function () {
        ...
        $scope._popConfirm(msg, function() {
            ...
            $("#storeKioskMClsExcelUpFile").val('');
            $("#storeKioskMClsExcelUpFile").trigger('click');
...
/**
 * 키오스크중분류(카테고리명) 엑셀다운로드 그리드 생성
 */
app.controller('storeKioskMClsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeKioskMClsExcelCtrl', $scope, $http, true));
    ...
    // 양식 다운로드
    $scope.sampleDownload = function (posNo, tuClsType, tuClsCd) {
        ...
        $scope._inquiryMain("/base/multilingual/storeKioskSideOption/getStoreKioskMClsList.sb", params, function () {
            ...
            $timeout(function () {
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex,
                    ...
                    messages["storeKioskSideOption.kioskMCls"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
    ...
    $scope.excelFileChanged = function () {
        if ($('#storeKioskMClsExcelUpFile')[0].files[0]) {
            // 엑셀업로드 호출
            $scope.excelUpload();
        }
    };
    ...
    $scope.excelUpload = function () {
        ...
        // 선택한 파일이 있으면
        if ($('#storeKioskMClsExcelUpFile')[0].files[0]) {
            var file = $('#storeKioskMClsExcelUpFile')[0].files[0];
            ...
            if (fileExtension.toLowerCase() === '.xlsx' || fileExtension.toLowerCase() === '.xlsm') {
                ...
                /*$timeout(function () {
                    var flex = $scope.flex;
                    wijmo.grid.xlsx.FlexGridXlsxConverter.loadAsync(flex, $('#kioskMClsExcelUpFile')[0].files[0], {includeColumnHeaders: true}
                        , function () {
                            $timeout(function () {
                ...
                reader.onload = function(){
                    ...
                    wb.SheetNames.forEach(function(sheetName) {
                        ...
                        arr.forEach(function(item){
                            renameKey(item, '포스번호', 'posNo');
                            renameKey(item, '키맵그룹코드', 'tuClsType');
                            renameKey(item, '키맵그룹명', 'tuClsTypeNm');
                            renameKey(item, '카테고리코드', 'tuClsCd');
                            renameKey(item, '카테고리명', 'tuClsNm');
                            renameKey(item, '중분류코드', 'tuMClsCd');
                            renameKey(item, '중분류명', 'tuMClsNm');
                            renameKey(item, '중분류명(영문)', 'tuMClsEnNm');
                            renameKey(item, '중분류명(중문)', 'tuMClsCnNm');
                            renameKey(item, '중분류명(일문)', 'tuMClsJpNm');
                        });

                        // 엔터값 제거
                        arr.forEach(function(item){
                            if (item.tuMClsEnNm !== null && item.tuMClsEnNm !== undefined && item.tuMClsEnNm !== "") {
                                item.tuMClsEnNm = item.tuMClsEnNm.toString().replace(/\r\n|\r|\n/g, ' ');
                            }

                            if (item.tuMClsCnNm !== null && item.tuMClsCnNm !== undefined && item.tuMClsCnNm !== "") {
                                item.tuMClsCnNm = item.tuMClsCnNm.toString().replace(/\r\n|\r|\n/g, ' ');
                            }

                            if (item.tuMClsJpNm !== null && item.tuMClsJpNm !== undefined && item.tuMClsJpNm !== "") {
                                item.tuMClsJpNm = item.tuMClsJpNm.toString().replace(/\r\n|\r|\n/g, ' ');
                            }
                        });
                        console.log(arr);
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/storeKioskSideOption/saveStoreKioskMCls.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('storeKioskMClsCtrl').getKioskMClsList();
                }
            }
        }, function errorCallback(response) {
