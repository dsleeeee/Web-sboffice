...
app.controller('storeDayCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
	// 상위 객체 상속 : T/F 는 picker
	angular.extend(this, new RootController('storeDayCtrl', $scope, $http, true));
	...
	$scope.isChecked = false;
	  ...
	  $scope.storeDaySelectVendrShow = function () {
	    $scope._broadcast('storeDaySelectVendrCtrl');
	  };
...
/** 매장일수불 그리드 controller */
app.controller('storeDayMainCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('storeDayMainCtrl', $scope, $http, true));
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("storeDayMainCtrl");
  ...
  $scope.$on("storeDayMainCtrl", function (event, data) {
    $scope.searchStoreDayList(true);
    // 기능수행 종료 : 반드시 추가
    event.preventDefault();
  });

  //다른 컨트롤러의 broadcast 받기
  $scope.$on("storeDayMainCtrlSrch", function (event, data) {
    $scope.searchStoreDayList(false);
    // 기능수행 종료 : 반드시 추가
    event.preventDefault();
  });

  // 매장일수불 리스트 조회
  $scope.searchStoreDayList = function (isPageChk) {
    ...
    // 조회 수행 : 조회URL, 파라미터, 콜백함수
    $scope._inquiryMain("/stock/status/storeDay/storeDay/viewList.sb", params, function () {
