/****************************************************************
 *
 * 파일명 : dayProdMrpizza.js
 * 설  명 : 미스터피자 > 상품매출분석 > 일별상품매출현황 JavaScript
 *
 ...
 *  일별상품매출현황 그리드 생성
 */
app.controller('dayProdMrpizzaCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dayProdMrpizzaCtrl', $scope, $http, false));
    ...
    $scope.$on("dayProdMrpizzaCtrl", function (event, data) {
    ...
    $scope.searchDayProdList = function () {
        ...
        params.prodClassCd = $("#dayProdMrpizzaSelectClassCd").val();
        params.prodCd = $scope.prodCd;
        params.prodNm = $scope.prodNm;
        //params.storeHqBrandCd = $scope.storeHqBrandCd;
        params.storeCds = $("#dayProdMrpizzaStoreCd").val();
        params.prodCds = $("#dayProdMrpizzaSelectCd").val();
       ...
       /* params.momsStoreFg01 = $scope.momsStoreFg01;
        ...
        $scope._inquiryMain("/sale/prod/dayProdMrpizza/dayProdMrpizza/getDayProdMrpizzaList.sb", params, function (){
            // <-- 그리드 visible -->
    ...
    $scope.dayProdMrpizzaSelectShow = function () {
        $scope._broadcast('dayProdMrpizzaSelectCtrl');
    };
    ...
    $scope.excelDownload = function () {
        ...
        $scope._popConfirm(messages["cmm.excel.totalExceDownload"], function() {
            $scope._broadcast('dayProdMrpizzaExcelCtrl', params);
        });
    };
        ...
        $timeout(function () {
         wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
           ...
           includeColumns: function (column) {
             ...
             "일별상품매출현황_" + wijmo.Globalize.format($scope.srchStartDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format($scope.srchEndDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
           $timeout(function () {
 ...
 */
app.controller('dayProdMrpizzaExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('dayProdMrpizzaExcelCtrl', $scope, $http, false));
    ...
    // 다른 컨트롤러의 broadcast 받기
    $scope.$on("dayProdMrpizzaExcelCtrl", function (event, data) {
    ...
    $scope.searchExcelList = function (params) {
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/prod/dayProdMrpizza/dayProdMrpizza/getDayProdMrpizzaExcelList.sb", params, function() {
            if ($scope.flex.rows.length <= 0) {
                $scope._popMsg(messages["excelUpload.not.downloadData"]); // 다운로드 할 데이터가 없습니다.
                return false;
            }
            ...
            $timeout(function () {
                ...
                }, "일별상품매출현황" + '_' + params.startDate + '_' + params.endDate + '_' + getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
                        $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
                    }, 10);
                });
