...
//		$scope.getRePosNmList(storeCd);
    ...
    $scope.searchPosDayList = function (isPageChk) {
        ...
        var startDt = new Date(wijmo.Globalize.format($scope.srchPosDayStartDate.value, 'yyyy-MM-dd'));
        var endDt = new Date(wijmo.Globalize.format($scope.srchPosDayEndDate.value, 'yyyy-MM-dd'));
        ...
        $scope._inquiryMain("/sale/status/pos/day/getPosDayList.sb", params, function() {
...
app.controller('posDayExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.searchPosDayExcelList = function (isPageChk) {
        ...
        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/status/pos/day/getPosDayExcelList.sb", params, function() {
            ...
            $timeout(function () {
                ...
                }, messages["month.sale"]+'_'+messages["empsale.pos"]+'_'+messages["pos.day"]+'_'+getCurDateTime()+'.xlsx', function () {
                    $timeout(function () {
