/****************************************************************
 *
 * 파일명 : saleProdRankMoms.js
 * 설  명 : 상품별매출순위 JavaScript
 *
...
/** controller */
app.controller('saleProdRankMomsCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('saleProdRankMomsCtrl', $scope, $http, true));
    ...
    $scope.$on("saleProdRankMomsCtrl", function (event, data) {
        // 조회
        $scope.searchList();
        // 기능수행 종료 : 반드시 추가
    ...
    $scope.searchList = function(){
       ...
       $scope._inquiryMain("/sale/prod/saleProdRankMoms/saleProdRankMoms/getSaleProdRankList.sb", params, function (){
    ...
    $scope.excelDownload2 = function () {
        ...
        $timeout(function () {
         wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.flex, {
           ...
           messages["prodRankMoms.prodRankMoms"] + '_' +  wijmo.Globalize.format($scope.srchStartDate.value, 'yyyyMMdd') + '_' + wijmo.Globalize.format($scope.srchEndDate.value, 'yyyyMMdd') + '_' + getCurDateTime()+'.xlsx', function () {
           $timeout(function () {
    ...
    $scope.saleProdRankMomsStoreShow = function () {
        $scope._broadcast('saleProdRankMomsStoreCtrl');
    };

    // 상품선택 모듈 팝업 사용시 정의
    // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
    // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
    $scope.saleProdRankMomsSelectShow = function () {
        $scope._broadcast('saleProdRankMomsSelectCtrl');
    };
...
app.controller('saleProdRankMomsExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.searchExcelList = function (params) {
        ...
        $scope._inquiryMain("/sale/prod/saleProdRankMoms/saleProdRankMoms/getSaleProdRankExcelList.sb", params, function() {
            ...
            $timeout(function () {
                ...
                }, messages["prodRankMoms.prodRankMoms"] + '_' + params.startDate + '_' + params.endDate + '_' + getCurDateTime() +'.xlsx', function () {
                    $timeout(function () {
                        $scope.$broadcast('loadingPopupInactive'); // 데이터 처리중 메시지 팝업 닫기
                    }, 10);
                });
    ...
    $scope.searchExcelDivisionList = function (params) {
        ...
        params.limit = 1;
        params.offset = 1;
        $scope._postJSONQuery.withOutPopUp( "/sale/prod/saleProdRankMoms/saleProdRankMoms/getSaleProdRankList.sb", params, function(response){
                    ...
                    return new Promise(function(resolve, reject){
                        //setTimeout(function() {
                            ...
                            $scope._postJSONQuery.withOutPopUp("/sale/moms/prodSaleDayStoreMoms/prodSaleDayStoreMoms/getDivisionExcelDownloadSaveUpdate.sb", params, function(response){
                                ...
                                $http({
                                    method: 'POST', //방식
                                    url: '/sale/prod/saleProdRankMoms/saleProdRankMoms/getSaleProdRankList.sb', /* 통신할 URL */
                                    ...
                                    setTimeout(function() {
                                        ...
                                        }, messages["prodRankMoms.prodRankMoms"] + '_' + params.startDate + '_' + params.endDate + '_' + getCurDateTime() + '_' + (x + 1) +'.xlsx', function () {
                                            $timeout(function () {
