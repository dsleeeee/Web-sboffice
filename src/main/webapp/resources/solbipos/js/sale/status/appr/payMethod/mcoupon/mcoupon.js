...
app.controller('apprMcouponCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  //조회조건 승인구분 데이터 Set
  $scope._setComboData("srchMcouponSaleFgDisplay", [
  ...
  //조회조건 승인처리 데이터 Set
  $scope._setComboData("srchMcouponApprProcFgDisplay", [
    {"name": messages["cmm.all"], "value": ""},
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("apprMcouponCtrl");
...
//다른 컨트롤러의 broadcast 받기
  ...
  $scope.searchApprMcouponList = function (isPageChk) {
	  ...
	  var startDt = new Date(wijmo.Globalize.format($scope.srchApprMcouponStartDate.value, 'yyyy-MM-dd'));
	  var endDt = new Date(wijmo.Globalize.format($scope.srchApprMcouponEndDate.value, 'yyyy-MM-dd'));
    ...
    $scope.srchPosNo  	  = $("#apprMcouponSelectPosCd").val();
    $scope.srchCornrCd    = $("#apprMcouponSelectCornerCd").val();
    $scope.srchSaleFg	  = $scope.saleFgModel;
    $scope.srchCashBillApprProcFg = $scope.cashBillApprProcFgModel;
  ...
  $scope.isChkDt = function() {
    $scope.srchApprMcouponStartDate.isReadOnly = $scope.isChecked;
    $scope.srchApprMcouponEndDate.isReadOnly = $scope.isChecked;
  };

  //매장선택 모듈 팝업 사용시 정의
  // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
  // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
  $scope.apprMcouponSelectStoreShow = function () {
    $scope._broadcast('apprMcouponSelectStoreCtrl');
  };

  	//포스선택 모듈 팝업 사용시 정의
	// 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
	// _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
	$scope.apprMcouponSelectPosShow = function () {
		$scope._broadcast('apprMcouponSelectPosCtrl');
	};

	//포스선택 모듈 팝업 사용시 정의
	//함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
	//_broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
	$scope.apprMcouponSelectCornerShow = function () {
		$scope._broadcast('apprMcouponSelectCornerCtrl');
	};
...
//엑셀 다운로드
  $scope.excelDownloadMcoupon = function () {
	...
	// 파라미터
    var params       = {};
    $scope._broadcast('apprMcouponExcelCtrl',params);
  };
 ...
 //매장의 코너(corner) 리스트 조회
	$scope.getCornerNmList = function () {
		...
		var comboParams     = {};
		comboParams.storeCd = $("#apprMcouponSelectStoreCd").val();
	};


	// 선택한 승인구분에 따른 리스트 항목 visible
	$scope.editDataGrid = function () {
        var grid = wijmo.Control.getControl("#apprMcouponGrid");
...
app.controller('apprMcouponExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

	// 상위 객체 상속 : T/F 는 picker
	angular.extend(this, new RootController('apprMcouponExcelCtrl', $scope, $http, $timeout, true));
	...
	// 다른 컨트롤러의 broadcast 받기
	$scope.$on("apprMcouponExcelCtrl", function (event, data) {

		if(data != undefined && $scope.isSearch) {
			$scope.searchApprMcouponExcelList();
			// 기능수행 종료 : 반드시 추가
			event.preventDefault();
		} else{
	...
	// 신용카드 승인현황 리스트 조회
	  $scope.searchApprMcouponExcelList = function () {
		...
		// 조회 수행 : 조회URL, 파라미터, 콜백함수
		$scope._inquiryMain("/sale/status/appr/mcoupon/excelList.sb", params, function() {
			...
			$timeout(function () {
				...
				}, messages["dailyReport.appr"]+'_'+messages["dailyReport.appr"]+'_'+messages["dailyReport.apprMcoupn"]+'_'+getToday()+'.xlsx', function () {
					$timeout(function () {
	...
	// 선택한 승인구분에 따른 리스트 항목 visible
		$scope.editDataGrid = function () {
	        var grid = wijmo.Control.getControl("#apprMcouponExcelGrid");
	        var columns = grid.columns;
