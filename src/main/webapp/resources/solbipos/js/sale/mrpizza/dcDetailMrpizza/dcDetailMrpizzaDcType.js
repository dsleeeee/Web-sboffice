...
var searchConditionData = [
    {"name":"전체","value":""},
    {"name":"할인코드","value":"dcCode"},
    {"name":"할인구분","value":"dcType"}
];
...
app.controller('dcDetailMrpizzaDcTypeCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    // 조회일자
    $scope.srchStartDate = wcombo.genDateVal("#startDateDcDetailMrpizzaDcType", gvStartDate);
    $scope.srchEndDate = wcombo.genDateVal("#endDateDcDetailMrpizzaDcType", gvEndDate);

    $scope._setComboData("searchCondition", searchConditionData); // 검색조건 콤보박스
    ...
    $scope.$on("dcDetailMrpizzaDcTypeCtrl", function (event, data) {

        // 할인구분 탭 리스트 조회
        $scope.searchDcDetailMrpizzaDcTypeList();
    ...
    // 할인구분 탭 리스트 조회
    $scope.searchDcDetailMrpizzaDcTypeList = function () {
        ...
        // 매장(을)를 선택해주세요.
        if($("#dcDetailMrpizzaDcTypeStoreCd").val() === "") {
        ...
        // 파라미터
        var params = {};
        params.searchCondition = $scope.searchConditionCombo.selectedValue;
        params.searchTerm = $("#searchTerm").val();
        params.startDate = wijmo.Globalize.format($scope.srchStartDate.value, 'yyyyMMdd');
        params.endDate = wijmo.Globalize.format($scope.srchEndDate.value, 'yyyyMMdd');
        params.storeCds = $("#dcDetailMrpizzaDcTypeStoreCd").val();

        // 조회 수행 : 조회URL, 파라미터, 콜백함수
        $scope._inquiryMain("/sale/mrpizza/dcDetailMrpizza/getDcDetailMrpizzaDcTypeList.sb", params, function () {

            // 그리도 header 조회 날짜 셋팅
            var grid = wijmo.Control.getControl("#wjGridList3");
            ...
            dataItem.storeCd        = salePeriod;
            dataItem.storeNm        = salePeriod;
            dataItem.saleDate       = salePeriod;
            dataItem.billDt         = salePeriod;
            dataItem.posNo          = salePeriod;
            dataItem.billNo         = salePeriod;
            dataItem.totSaleAmt     = salePeriod;
            dataItem.dcAmt          = salePeriod;
            dataItem.realSaleAmt    = salePeriod;
            dataItem.dlvrOrderFg    = salePeriod;
