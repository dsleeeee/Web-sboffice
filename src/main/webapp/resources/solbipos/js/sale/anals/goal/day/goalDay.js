...
//    s.bottomLeftCells.setCellData(0, 0, '합계');
  ...
  $scope.excelDownloadGoalDay = function () {
	  var params = {};
	  $scope._broadcast('goalDayExcelCtrl',params);
  };
...
/** 일자별(코너별 매출) controller */
app.controller('goalDayExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('goalDayExcelCtrl', $scope, $http, $timeout, true));
...
//    s.bottomLeftCells.setCellData(0, 0, '합계');
  ...
  //다른 컨트롤러의 broadcast 받기(페이징 초기화)
  $scope.$on("goalDayExcelCtrl", function (event, data) {
	  if(data != undefined && $scope.isSearch) {
		  $scope.searchGoalDayExcelList(false);
			// 기능수행 종료 : 반드시 추가
			event.preventDefault();
		} else{
  ...
  // 코너별매출일자별 리스트 조회
  $scope.searchGoalDayExcelList = function (isPageChk) {
    // 파라미터
    var params       = {};
	...
	$scope._postJSONQuery.withOutPopUp("/sale/anals/goal/day/dayList.sb", params, function(response) {
	 ...
	 if (length != "" || length != null) {
	  ...
	  event.preventDefault();
		...
		// 조회 수행 : 조회URL, 파라미터, 콜백함수
		$scope._inquirySub("/sale/anals/goal/day/excelList.sb", params, function() {
