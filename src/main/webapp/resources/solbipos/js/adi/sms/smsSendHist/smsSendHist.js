...
app.controller('smsSendHistExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
    ...
    $scope.searchExcelList = function (params) {
        ...
        $scope._inquiryMain("/adi/sms/smsSendHist/smsSendHist/getSmsSendHistExcelList.sb", params, function() {
            if ($scope.smsSendHistExcelFlex.rows.length <= 0) {
                $scope._popMsg(messages["excelUpload.not.downloadData"]); // 다운로드 할 데이터가 없습니다.
                return false;
            }
            ...
            $timeout(function () {
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync($scope.smsSendHistExcelFlex, {
                    includeColumnHeaders: true,
                    includeCellStyles   : false,
                    includeColumns      : function (column) {
                        return column.visible;
                    }
                }, "SMS전송이력_" + getCurDateTime() +'.xlsx', function () {
                    $timeout(function () {
