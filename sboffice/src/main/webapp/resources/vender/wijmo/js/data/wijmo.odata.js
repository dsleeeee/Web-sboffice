        ...
        var ODataCollectionView = /** @class */ (function (_super) {
            ...
            ODataCollectionView.prototype._convertItem = function (dataTypes, item) {
                for (var k in dataTypes) {
                    ...
                    if (val != null) {
                        val = type == wijmo.DataType.Date && wijmo.isString(val) && val.indexOf('/Date(') == 0 // verbosejson
                            ? new Date(parseInt(val.substr(6)))
                            : wijmo.changeType(val, type, null);
                        if (wijmo.isDate(val) && this._showDatesAsGmt) {
                            val = new Date(val.getTime() + val.getTimezoneOffset() * 60000);
                        }
                        item[k] = val;
