...
app.controller('tableDayOfWeekCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  angular.extend(this, new RootController('tableDayOfWeekCtrl', $scope, $http, true));
 ...
 // 그리드 링크 효과
    ...
    s.addEventListener(s.hostElement, 'mousedown', function (e) {
      ...
      if (ht.cellType === wijmo.grid.CellType.Cell) {
        ...
        if (col.binding === "saleYn") { // 전표번호
          ...
          $scope._broadcast('tableDayOfWeekDetailCtrl', params);
        }
      }
    });
    ...
    // 첫째줄 헤더 생성
    var dataItem         = {};
    dataItem.saleDay         = messages["tableDay.saleDay"];
  ...
  $scope.searchTableDayOfWeekList = function () {
	...
	$http({
		method: 'POST',
		url: '/sale/status/table/dayofweek/list.sb',
		...
		var grid = wijmo.Control.getControl("#tableDayOfWeekGrid");
			...
			if($scope.arrTblCol != "" || $scope.arrTblCol != null){
		    	...
		    	for(var i = 0; i < $scope.arrTblCol.length; i++){
		    		grid.columns.setAt(4+(i*3), new wijmo.grid.Column({header: messages["tableDayOfWeek.realSaleAmt"], binding: 'realSaleAmtT'+i, width: 80, align: 'right', isReadOnly: 'true', aggregate: 'Sum'}));
		    	    grid.columns.setAt(5+(i*3), new wijmo.grid.Column({header: messages["tableDayOfWeek.realSaleCnt"], binding: 'realSaleCntT'+i, width: 80, align: 'center', isReadOnly: 'true', aggregate: 'Sum'}));
		    	    grid.columns.setAt(6+(i*3), new wijmo.grid.Column({header: messages["tableDayOfWeek.guestCnt"], binding: 'guestCnt1T'+i, width: 80, align: 'center', isReadOnly: 'true', aggregate: 'Sum'}));
  ...
  $scope.isChkDt = function() {
	  $scope.tableNo = wijmo.Control.getControl("#srchTableDayOfWeekTableNo");
  ...
  $scope.getStoreTableList = function () {
    ...
    var comboParams     = {};
    comboParams.storeCd = $("#tableDayOfWeekSelectStoreCd").val();
    // 파라미터 (comboFg, comboId, gridMapId, url, params, option, callback)
    $scope._queryCombo("combo", "srchTableDayOfWeekTableNo", null, url, comboParams, "A", null); // 명칭관리 조회시 url 없이 그룹코드만 넘긴다.
  };
...
//엑셀 다운로드
  $scope.excelDownloadTableDayOfWeek = function () {
