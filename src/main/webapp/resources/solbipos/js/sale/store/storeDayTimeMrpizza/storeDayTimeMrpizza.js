/****************************************************************
 *
 * 파일명 : storeDayTimeMrpizza.js
 * 설  명 : 미스터피자 > 매장분석 > 매장-일별시간대 JavaScript
 *
 ...
 *  매장-일별시간대 그리드 생성
 */
app.controller('storeDayTimeMrpizzaCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('storeDayTimeMrpizzaCtrl', $scope, $http, true));
    ...
    $scope.initGrid = function (s, e) {
       ...
       $scope._makePickColumns("storeDayTimeMrpizzaCtrl");
    ...
    $scope.$on("storeDayTimeMrpizzaCtrl", function (event, data) {
        ...
        // 매장-일별시간대 리스트 조회
        $scope.storeDayTimeMrpizzaList();
        // 기능수행 종료 : 반드시 추가
        event.preventDefault();
    });

    // 매장-일별시간대 리스트 조회
    $scope.storeDayTimeMrpizzaList = function () {
        ...
        if($scope.option2 === "store"){
            params.storeCds = $("#storeDayTimeMrpizzaStoreCd").val();
        } else {
            params.storeCds = '';
        }
        ...
        $scope._inquiryMain("/sale/store/storeDayTimeMrpizza/storeDayTimeMrpizza/getStoreDayTimeMrpizzaList.sb", params, function (){
