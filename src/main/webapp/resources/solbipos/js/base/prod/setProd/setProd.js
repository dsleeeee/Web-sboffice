...
app.controller('setProdCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.initGrid = function (s, e) {
    ...
    s.formatItem.addHandler(function (s, e) {
      if (e.panel === s.cells) {
        ...
        if (col.binding === 'regSdselGrp' || col.binding === 'regSdselGrpSingle') {
            wijmo.addClass(e.cell, 'wijLink');
        } else if (col.binding === 'nSideProdYn') {
          if (item.sideProdYn === 'Y') {
            wijmo.addClass(e.cell, 'wijLink');
          }
        }
      }
    ...
    s.beginningEdit.addHandler(function (s, e) {
      var col = s.columns[e.col];
      if (col.binding === 'regSdselGrp' || col.binding === 'nSideProdYn') {
        e.cancel = true;
      }
    });
      ...
      if( ht.cellType === wijmo.grid.CellType.Cell) {
        ...
        if ( col.binding === 'nSideProdYn') {
          if(selectedRow.sideProdYn === 'Y'){
            $scope._popConfirm(messages["setProd.delSdselGrpMsg"], function() {
              selectedRow.sideProdYn = 'N';
              // 파라미터 설정
              var params = [];
              params.push(selectedRow);
              $scope._save('/base/prod/setProd/setProd/saveSideProdYn.sb', params, function() {
                ...
                $("#setProd").html("");
                var groupScope = agrid.getScope('setProeSideMenuSelectGroupCtrl');
                ...
                $("#sideClassTitle").html("");
                var prodScope = agrid.getScope('setProeSideMenuSelectProdCtrl');
  ...
  // <-- 검색 호출 -->
  $scope.$on('setProdCtrl', function(event, data) {
    $("#setProd").html("");
    var groupScope = agrid.getScope('setProeSideMenuSelectGroupCtrl');
    groupScope._gridDataInit();   // 선택분류 그리드 초기화
  ...
  $scope.searchProd = function(){
    ...
    // 파라미터
    var params = {};
    params.prodCd = $("#setProdCd").val();
    params.prodNm = $("#setProdNm").val();
    params.useYn = $scope.useYn;

    // 조회 수행 : 조회URL, 파라미터, 콜백함수, 팝업결과표시여부
    $scope._inquiryMain('/base/prod/setProd/setProd/getProdList.sb', params,function() {
      $("#btnAddSelGroup").hide();
      $("#btnDelSelGroup").hide();
      $("#btnSaveSelGroup").hide();
    }, false);
  };
  // <-- //검색 호출 -->

  // 저장
  $scope.setProdSave = function() {
 ...
 * 사이드메뉴 선택그룹 그리드 생성
 */
app.controller('setProeSideMenuSelectGroupCtrl', ['$scope', '$http', function ($scope, $http) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('setProeSideMenuSelectGroupCtrl', $scope, $http, false));
      ...
      if( ht.cellType === wijmo.grid.CellType.Cell) {
        ...
        if ( col.binding === 'sdselGrpCd') {
          if (selectedRow.sdselGrpCd !== '' && selectedRow.sdselGrpCd !== undefined && selectedRow.sdselGrpCd !== '자동채번') {
            ...
            $scope._broadcast('setProeSideMenuSelectClassCtrl', selectedRow);
            var prodGrid = agrid.getScope('setProeSideMenuSelectProdCtrl');
  ...
  // <-- 검색 호출 -->
  $scope.$on('setProeSideMenuSelectGroupCtrl', function(event, data) {
    // 초기 버튼 셋팅
    $("#btnAddSelGroup").show();
    $("#btnDelSelGroup").show();
    ...
    $("#setProd").html(" [" + data.prodCd + "]" + data.prodNm);
    // 선택그룹 그리드 조회
    $scope.srchSelGroup(data);
  ...
  // 선택그룹 그리드 조회
  $scope.srchSelGroup = function(data){

    // 파라미터
    var params = {};
    if(data.sdselGrpCd == undefined || data.sdselGrpCd == null || data.sdselGrpCd == '' || data.gubun !== data.sdselTypeFg) {
      params.sdselGrpCd = "";
  ...
  $scope.deleteRow = function() {
    $scope._popConfirm(messages["cmm.choo.delete"], function() {
      ...
      $scope._save('/base/prod/sideMenu/menuGrp/save.sb', params, function() {
        ...
        prodScope._gridDataInit();   // 그리드 초기화
        // 저장 후 그리드 재조회
        $scope._broadcast("setProeSideMenuSelectGroupCtrl");
      });
    });
  };
 ...
 * 사이드메뉴 선택분류 그리드 생성
 */
app.controller('setProeSideMenuSelectClassCtrl', ['$scope', '$http', 'sdselGrpCd', function ($scope, $http, sdselGrpCd) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('setProeSideMenuSelectClassCtrl', $scope, $http, false));
      ...
      if( ht.cellType === wijmo.grid.CellType.Cell) {
        ...
        if(col.binding === 'sdselClassCd') {
          if(selectedRow.sdselClassCd !== '' && selectedRow.sdselClassCd !== undefined && selectedRow.sdselClassCd !== '자동채번') {
            ...
            $scope._broadcast('setProeSideMenuSelectProdCtrl', params);
          }
        }
      }
    });
  ...
  // 선택분류 그리드 조회
  $scope.$on('setProeSideMenuSelectClassCtrl', function(event, data) {
    // scope 영역에 변수 Set
  ...
  $scope.deleteRow = function() {
    $scope._popConfirm(messages["cmm.choo.delete"], function() {
      ...
      $scope._save('/base/prod/sideMenu/menuClass/save.sb', params, function() {
        ...
        // 선택분류 리스트 재조회
        var grpGrid = agrid.getScope('setProeSideMenuSelectGroupCtrl');
        var selectedSelGroup = grpGrid.getSelectedSelGroup();
        $scope._broadcast('setProeSideMenuSelectClassCtrl', selectedSelGroup);
 ...
 * 사이드메뉴 선택상품 그리드 생성
 */
app.controller('setProeSideMenuSelectProdCtrl', ['$scope', '$http', 'sdselClassCd', function ($scope, $http, sdselClassCd) {
  // 상위 객체 상속 : T/F 는 picker
  angular.extend(this, new RootController('setProeSideMenuSelectProdCtrl', $scope, $http, false));
  ...
  // 선택상품 그리드 조회
  $scope.$on('setProeSideMenuSelectProdCtrl', function(event, data) {
    // scope 영역에 변수 Set
