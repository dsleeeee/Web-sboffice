 ...
 * 상품명 조회 그리드 생성
 */
app.controller('prodNmCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodNmCtrl', $scope, $http, true));
    ...
    $scope.$on("prodNmCtrl", function (event, data) {
        // 조회
        $scope.getProdNmList();
    ...
    $scope.getProdNmList = function () {
        ...
        // 상품명 탭 리스트 조회
        $scope._inquiryMain("/base/multilingual/prodLang/getProdNmList.sb", params);
    };
        ...
        $scope._popConfirm(messages["cmm.choo.save"], function() {
            ...
            for (var i = 0; i < params.length; i++) {
                ...
                if (nvl(item.prodEnNm + '', '').getByteLengthForOracle() > 100) {
                    $scope._popMsg(messages["prodLang.en"] + " " + messages["prodLang.valueSize.chk.msg"]);  // 영문 데이터 중 문자열의 길이가 너무 긴 데이터가 있습니다.
                    return false;
                }

                if (nvl(item.prodCnNm + '', '').getByteLengthForOracle() > 100) {
                    $scope._popMsg(messages["prodLang.cn"] + " " + messages["prodLang.valueSize.chk.msg"]);  // 중문 데이터 중 문자열의 길이가 너무 긴 데이터가 있습니다.
                    return false;
                }

                if (nvl(item.prodJpNm + '', '').getByteLengthForOracle() > 100) {
                    $scope._popMsg(messages["prodLang.jp"] + " " + messages["prodLang.valueSize.chk.msg"]);  // 일문 데이터 중 문자열의 길이가 너무 긴 데이터가 있습니다.
                    return false;
                }
            }

            $scope._save("/base/multilingual/prodLang/saveProdNm.sb", params, function(result) {
                // 재조회
                $scope.getProdNmList();
            });
        });
    ...
    $scope.excelDownload = function(){
        ...
        $scope.$broadcast('loadingPopupActive', messages["cmm.progress"]); // 데이터 처리중 메시지 팝업 열기
            $timeout(function () {
                ...
                }, messages["prodLang.prodNm"]  + '_' +  getCurDateTime() + '.xlsx', function () {
                    $timeout(function () {
    ...
    $scope.sampleDownload = function () {
        ...
        var vScope = agrid.getScope('prodNmExcelCtrl');
 ...
 * 상품명 양식다운로드 그리드 생성
 */
app.controller('prodNmExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

    // 상위 객체 상속 : T/F 는 picker
    angular.extend(this, new RootController('prodNmExcelCtrl', $scope, $http, false));
    ...
    $scope.sampleDownload = function (data) {
        ...
        $scope._inquiryMain("/base/multilingual/prodLang/getProdNmList.sb", params, function (){
            ...
            $timeout(function()	{
                wijmo.grid.xlsx.FlexGridXlsxConverter.saveAsync(	$scope.flex,
                    ...
                    messages["prodLang.prodNm"] + '_엑셀업로드_양식_' + getCurDateTime() + '.xlsx',
                    function () {
                        $timeout(function () {
                            $scope.$broadcast('loadingPopupInactive'); //데이터 처리중 메시지 팝업 닫기
                        }, 10);
    ...
    $scope.saveData = function (jsonData) {
        ...
        $http({
            method : 'POST', //방식
            url    : '/base/multilingual/prodLang/saveProdNm.sb', /* 통신할 URL */
            ...
            if ($scope._httpStatusCheck(response, true)) {
                if (parseInt($scope.progressCnt) >= parseInt($scope.totalRows)) {
                    // 재조회
                    agrid.getScope('prodNmCtrl').getProdNmList();
                }
            }
