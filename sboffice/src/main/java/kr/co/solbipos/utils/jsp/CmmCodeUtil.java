 ...
 * @author 정용길
 * jsp 에서 직접 사용
 * 공통 코드 comboBox 에 직접 사용하는 data 형식으로 돌려줌
 */
@Slf4j
...
public class CmmCodeUtil {
    ...
    /**
      ...
      * @return
      */
    public String getCommCode(String comCdFg) {

        CommonCode commonCode = new CommonCode();
        commonCode.setComCdFg(comCdFg);

        List<DefaultMap<CommonCode>> codeList = null;

        try {
            // 요청한 코드가 레디스에 있는지 확인
            if( cmmCodeService.hasCode(comCdFg) ) {
                // 레디스에서 조회
                commonCode = cmmCodeService.getCodeList(comCdFg);
            }
            // 디비에서 조회 후 레디스에 세팅
            else {
                codeList = sampleService.selectCommonCodeList(comCdFg);
                ...
                cmmCodeService.setCodeList(commonCode); // 레디스에 공통코드 세팅
            }
        }
        catch (Exception e) {
            log.warn("redis error.... 공통코드 디비에서 조회 : {}", comCdFg);
        ...
        return assmblObj(commonCode.getCodeList(), "comCdNm", "comCd");
    }
    ...
    public <E> String assmblObj(List<DefaultMap<E>> source, String name[], String value[]) {
        ...
        source.stream().forEach( x -> {
            list.add( assmbl(x, name, value) );
        });
        
        return convertToJson(list);
    }
    
    public <E> String addStr(DefaultMap<E> obj, String arr[]) {
        return addStr(obj, arr, "");
    }
    
    public <E> String addStr(DefaultMap<E> obj, String arr[], String separator) {
        if( ObjectUtils.isEmpty(obj) || ObjectUtils.isEmpty(arr) ) {
            return "";
        }
        
        String returnStr = "";
        for( int i = 0; i < arr.length; i++ ) {
            returnStr += obj.getStr(arr[i]);
            if( i == arr.length - 1 ) {
            }
            else {
