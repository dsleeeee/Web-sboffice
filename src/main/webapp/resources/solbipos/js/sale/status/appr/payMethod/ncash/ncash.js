...
app.controller('apprNcashCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $scope.initGrid = function (s, e) {
    ...
    $scope._makePickColumns("apprNcashCtrl");
  ...
  $scope.searchApprNcashList = function (isPageChk) {
	  ...
	  var startDt = new Date(wijmo.Globalize.format($scope.srchApprNcashStartDate.value, 'yyyy-MM-dd'));
	  var endDt = new Date(wijmo.Globalize.format($scope.srchApprNcashEndDate.value, 'yyyy-MM-dd'));
    ...
    $scope.srchPosNo  	  = $("#apprNcashSelectPosCd").val();
    $scope.srchSaleYn	  = $scope.saleYnModel;
  ...
  $scope.isChkDt = function() {
    $scope.srchApprNcashStartDate.isReadOnly = $scope.isChecked;
    $scope.srchApprNcashEndDate.isReadOnly = $scope.isChecked;
  };

  //매장선택 모듈 팝업 사용시 정의
  // 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
  // _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
  $scope.apprNcashSelectStoreShow = function () {
    $scope._broadcast('apprNcashSelectStoreCtrl');
  };

  	//포스선택 모듈 팝업 사용시 정의
	// 함수명 : 모듈에 넘기는 파라미터의 targetId + 'Show'
	// _broadcast : 모듈에 넘기는 파라미터의 targetId + 'Ctrl'
	$scope.apprNcashSelectPosShow = function () {
		$scope._broadcast('apprNcashSelectPosCtrl');
	};

  //엑셀 다운로드
  $scope.excelDownloadNcash = function () {
	...
	// 파라미터
    var params       = {};
    $scope._broadcast('apprNcashExcelCtrl',params);
  };
  ...
  //매장의 포스(pos) 리스트 조회
	$scope.getPosNmList = function () {
		...
		comboParams.storeCd = $("#posNcashSelectStoreCd").val();
	};
...
app.controller('apprNcashExcelCtrl', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {

	// 상위 객체 상속 : T/F 는 picker
	angular.extend(this, new RootController('apprNcashExcelCtrl', $scope, $http, $timeout, true));
	...
	// 다른 컨트롤러의 broadcast 받기
	$scope.$on("apprNcashExcelCtrl", function (event, data) {

		if(data != undefined && $scope.isSearch) {
			$scope.searchapprNcashExcelList();
			// 기능수행 종료 : 반드시 추가
			event.preventDefault();
		} else{
	...
	// 신용카드 승인현황 리스트 조회
	  $scope.searchapprNcashExcelList = function () {
		...
		// 조회 수행 : 조회URL, 파라미터, 콜백함수
		$scope._inquiryMain("/sale/status/appr/ncash/excelList.sb", params, function() {
			...
			$timeout(function () {
				...
				}, messages["dailyReport.appr"]+'_'+messages["dailyReport.appr"]+'_'+messages["dailyReport.apprNcash"]+'_'+getToday()+'.xlsx', function () {
					$timeout(function () {
	...
	// 선택한 승인구분에 따른 리스트 항목 visible
		$scope.editDataGrid = function () {
	        var grid = wijmo.Control.getControl("#apprNcashExcelGrid");
	        var columns = grid.columns;
